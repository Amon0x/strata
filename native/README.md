...
`strata_surface_frame` updates a Surface and prepares packet v7. A bytes sink borrows the packet only
...
`strata_headless` is the non-windowed application host. It drives the same C ABI and packet-v7
...
programs configure an application, activate `.strata`, create/frame a Surface, decode packet v7,
