...
struct PaintStyle final {
    ...
    /** True when `fill` is the parse option's resolved value for the `currentColor` keyword. */
    bool fill_uses_current_color = false;
    bool has_stroke = false;
    Color stroke{0U, 0U, 0U, 255U};
    /** True when `stroke` is the parse option's resolved value for the `currentColor` keyword. */
...
struct ParseLimits final {
    ...
    std::size_t maximum_path_segments = 4096U;
};
...
/**
 * Parses the deliberately static SVG subset documented in Strata's SVG guide.
 *
