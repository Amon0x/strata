...
class AbiError final : public std::runtime_error {
  public:
    AbiError(const strata_result result, const std::string_view operation,
        ...
        : std::runtime_error(describe(result, operation, diagnostic)), status_(result.status),
          diagnostic_id_(result.diagnostic_id), diagnostic_(std::move(diagnostic)) {}

    [[nodiscard]] strata_status status() const noexcept {
        return status_;
    }
    [[nodiscard]] std::uint64_t diagnostic_id() const noexcept {
  ...
  private:
    [[nodiscard]] static std::string describe(const strata_result result,
...
inline void require_ok(const strata_result result, const std::string_view operation) {
    if (result.status != STRATA_STATUS_OK)
        throw AbiError(result, operation);
}

[[nodiscard]] inline strata_scale_policy_config
scale_policy_defaults(const strata_scale_policy_kind kind) {
...
[[nodiscard]] inline strata_scale_context
resolve_scale_context(const strata_scale_policy_config& policy,
                      const std::int64_t framebuffer_width, const std::int64_t framebuffer_height) {
    strata_scale_context context{};
    context.struct_size = sizeof(context);
    require_ok(
        strata_resolve_scale_context(&policy, framebuffer_width, framebuffer_height, &context),
...
[[nodiscard]] inline strata_theme_layout_size
theme_layout_size(const strata_theme_layout_size_kind kind = STRATA_THEME_SIZE_AUTO,
                  const double value = 0.0) noexcept {
...
[[nodiscard]] inline strata_theme_layout_size
theme_layout_clamp(const strata_theme_layout_size* minimum,
                   const strata_theme_layout_size* preferred,
                   const strata_theme_layout_size* maximum) noexcept {
...
[[nodiscard]] inline strata_theme_motion_easing theme_motion_easing(
    const strata_theme_motion_easing_kind kind = STRATA_THEME_MOTION_EASING_LINEAR) noexcept {
...
[[nodiscard]] inline strata_theme_motion_easing
theme_cubic_bezier(const double x1, const double y1, const double x2, const double y2) noexcept {
    strata_theme_motion_easing result =
        theme_motion_easing(STRATA_THEME_MOTION_EASING_CUBIC_BEZIER);
    result.x1 = x1;
    result.y1 = y1;
    result.x2 = x2;
    result.y2 = y2;
...
[[nodiscard]] inline strata_theme_motion_timing
theme_motion_timing(const strata_string_view name,
...
[[nodiscard]] inline strata_theme_animation_spec
theme_named_animation(const strata_string_view name) noexcept {
    return strata_theme_animation_spec{STRATA_THEME_ANIMATION_NAMED, 0U, name, nullptr};
}

[[nodiscard]] inline strata_theme_animation_spec
theme_inline_animation(const strata_theme_declared_animation* animation) noexcept {
    return strata_theme_animation_spec{STRATA_THEME_ANIMATION_INLINE, 0U, strata_string_view{},
                                       animation};
}
...
[[nodiscard]] inline std::optional<std::string> optional_copied(const strata_string_view value) {
    return value.size == 0U ? std::nullopt : std::optional<std::string>(copied(value));
}

[[nodiscard]] inline DiagnosticSeverity
diagnostic_severity(const strata_diagnostic_severity severity) noexcept {
    switch (severity) {
    case STRATA_DIAGNOSTIC_INFO:
        return DiagnosticSeverity::info;
    case STRATA_DIAGNOSTIC_WARNING:
        return DiagnosticSeverity::warning;
    case STRATA_DIAGNOSTIC_ERROR:
        return DiagnosticSeverity::error;
    case STRATA_DIAGNOSTIC_FATAL:
...
[[nodiscard]] inline Diagnostic copy_diagnostic(const strata_diagnostic& value) {
    ...
    if (result.source_id.has_value()) {
        result.range = SourceRange{
            value.range.byte_start,   value.range.byte_end, value.range.line_start,
            value.range.column_start, value.range.line_end, value.range.column_end,
        };
    }
...
inline void capture_diagnostics(void* const user_data,
                                const strata_diagnostics_snapshot* const snapshot) noexcept {
...
[[nodiscard]] inline ProfilerCounter copy_profiler_counter(const strata_profiler_counter& value) {
    return ProfilerCounter{copied(value.name), value.value};
}

[[nodiscard]] inline ProfilerSection copy_profiler_section(const strata_profiler_section& value) {
    return ProfilerSection{
        value.id,           value.parent_id,     copied(value.name),
        copied(value.path), value.sample_count,  value.last_sample_frame_index,
        value.last_nanos,   value.average_nanos, value.p50_nanos,
        value.p95_nanos,    value.p99_nanos,     value.maximum_nanos,
    };
}

[[nodiscard]] inline ProfilerSpike copy_profiler_spike(const strata_profiler_spike& value) {
    ...
    if (value.stack_depth != 0U) {
        result.stack_section_ids.assign(value.stack_section_ids,
...
inline void capture_profiler(void* const user_data,
                             const strata_profiler_snapshot* const snapshot) noexcept {
...
[[nodiscard]] inline std::optional<Diagnostic> find_diagnostic(const strata_runtime* const runtime,
                                                               const std::uint64_t id) noexcept {
    if (runtime == nullptr || id == 0U)
        return std::nullopt;
    ...
    if (found == capture.value.records.end())
        return std::nullopt;
    try {
        return *found;
    } catch (...) {
...
struct RuntimeCallbacks final {
    explicit RuntimeCallbacks(RuntimeOptions options,
                              const strata_diagnostic_sink forwarded_diagnostic = {})
        : options(std::move(options)), forwarded_diagnostic(forwarded_diagnostic) {}
...
inline void runtime_diagnostic(void* const user_data,
                               const strata_diagnostic* const diagnostic) noexcept {
    ...
    if (callbacks.forwarded_diagnostic.emit != nullptr &&
        ...
        try {
            callbacks.forwarded_diagnostic.emit(callbacks.forwarded_diagnostic.user_data,
                                                diagnostic);
        } catch (...) {
    ...
    try {
        const Diagnostic owned = copy_diagnostic(*diagnostic);
        {
            ...
            const auto retained =
                std::ranges::find(callbacks.diagnostics, owned.id, &Diagnostic::id);
        ...
        if (callbacks.options.diagnostic)
            callbacks.options.diagnostic(owned);
    } catch (...) {
        // A diagnostic observer cannot fail the ABI operation that produced the diagnostic.
    }
}

[[nodiscard]] inline std::optional<Diagnostic>
find_diagnostic(const RuntimeCallbacks* const callbacks, const std::uint64_t id) noexcept {
    if (callbacks == nullptr || id == 0U)
        return std::nullopt;
    const std::scoped_lock lock(callbacks->diagnostic_mutex);
    const auto found = std::ranges::find(callbacks->diagnostics, id, &Diagnostic::id);
    if (found == callbacks->diagnostics.end())
        return std::nullopt;
    try {
        return *found;
    } catch (...) {
...
inline strata_status resource_load(void* const user_data, const strata_string_view resource_id,
                                   strata_bytes_view* const out_bytes) noexcept {
    if (user_data == nullptr || out_bytes == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    auto& bridge = *static_cast<ResourceBridge*>(user_data);
    try {
        const std::optional<std::vector<std::uint8_t>> loaded =
            bridge.adapter.load(std::string_view(resource_id.data, resource_id.size));
        if (!loaded.has_value())
...
inline strata_status durable_load(void* const user_data, const strata_string_view application_id,
                                  strata_bytes_view* const out_bytes) noexcept {
    if (user_data == nullptr || out_bytes == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    ...
    try {
        const auto loaded =
            bridge.adapter.load(std::string_view(application_id.data, application_id.size));
        if (!loaded.has_value())
            return STRATA_STATUS_NOT_FOUND;
        bridge.borrowed = *loaded;
        *out_bytes = strata_bytes_view{bridge.borrowed.data(), bridge.borrowed.size()};
...
inline strata_status durable_write(void* const user_data, const strata_string_view application_id,
                                   const strata_bytes_view bytes) noexcept {
    if (user_data == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    auto& bridge = *static_cast<DurableBridge*>(user_data);
    try {
        bridge.adapter.write(std::string_view(application_id.data, application_id.size),
...
inline strata_status async_begin(void* const user_data, const std::uint64_t request_id,
                                 const strata_string_view binding, const strata_string_view owner,
                                 const strata_string_view payload_json) noexcept {
    if (user_data == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
...
inline strata_status clipboard_read(void* const user_data,
                                    strata_string_view* const out_text) noexcept {
    if (user_data == nullptr || out_text == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
...
inline strata_status clipboard_write(void* const user_data,
                                     const strata_string_view text) noexcept {
    if (user_data == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    auto& bridge = *static_cast<ClipboardBridge*>(user_data);
    try {
...
inline strata_status ime_set_cursor_rect(void* const user_data, const strata_rect rect) noexcept {
    if (user_data == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    auto& bridge = *static_cast<ImeBridge*>(user_data);
    try {
...
inline strata_status effect_emit(void* const user_data, const strata_string_view effect_id,
                                 const strata_string_view payload_json) noexcept {
    if (user_data == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
...
inline strata_status source_load(void* const user_data, const strata_string_view importer_source_id,
                                 const strata_string_view import_path,
                                 strata_module_source* const out_source) noexcept {
    if (user_data == nullptr || out_source == nullptr)
        return STRATA_STATUS_INVALID_ARGUMENT;
    auto& bridge = *static_cast<SourceLoaderBridge*>(user_data);
    try {
        bridge.borrowed =
            (*bridge.load)(std::string_view(importer_source_id.data, importer_source_id.size),
                           std::string_view(import_path.data, import_path.size));
        if (!bridge.borrowed.has_value())
            return STRATA_STATUS_NOT_FOUND;
        *out_source = strata_module_source{
            sizeof(strata_module_source),
            strata_string_view{bridge.borrowed->source_id.data(),
                               bridge.borrowed->source_id.size()},
...
struct RuntimeControl final {
    explicit RuntimeControl(strata_runtime* value,
                            std::shared_ptr<RuntimeCallbacks> callbacks = {}) noexcept
        : value(value), callbacks(std::move(callbacks)) {}
    ~RuntimeControl() {
        if (value != nullptr)
...
[[nodiscard]] inline std::optional<Diagnostic> find_diagnostic(const RuntimeControl* const control,
                                                               const std::uint64_t id) noexcept {
    if (control == nullptr || id == 0U)
        return std::nullopt;
    if (const std::optional<Diagnostic> retained = find_diagnostic(control->callbacks.get(), id);
        retained.has_value()) {
        return retained;
    }
...
inline void clear_retained_diagnostics(RuntimeControl* const control) noexcept {
    if (control == nullptr || control->callbacks == nullptr)
        return;
...
publish_host_snapshot(const std::shared_ptr<RuntimeControl>& control, const std::string_view id,
    ...
    const strata_result retained = strata_runtime_get_host_snapshot_generation(
        control->value, strata_string_view{id.data(), id.size()}, &retained_generation);
    if (retained.status != STRATA_STATUS_NOT_FOUND) {
        if (retained.status != STRATA_STATUS_OK) {
            throw AbiError(retained, "host snapshot generation read",
        ...
        control->next_host_snapshot_generation =
            std::max(control->next_host_snapshot_generation, retained_generation + 1U);
    }
    ...
    const strata_result published = strata_runtime_publish_host_snapshot(control->value, &snapshot);
    if (published.status != STRATA_STATUS_OK) {
        throw AbiError(published, "host snapshot publication",
...
inline void capture_string(void* const user_data, const strata_string_view text) noexcept {
    ...
    try {
        if (text.size == 0U)
            ...
            capture.value.assign(text.data, text.size);
    } catch (...) {
        capture.failed = true;
    }
}

inline void require_result(const strata_result result, const std::string_view operation,
...
[[nodiscard]] inline ActivationInfo activation_info(const strata_activation_info& value) {
    const auto status = [&value] {
        switch (value.status) {
        case STRATA_ACTIVATION_ACTIVATED:
            return ActivationStatus::activated;
        case STRATA_ACTIVATION_REJECTED_GENERATION:
            return ActivationStatus::rejected_generation;
        case STRATA_ACTIVATION_REJECTED_COMPILE:
            return ActivationStatus::rejected_compile;
        case STRATA_ACTIVATION_REJECTED_UNIT:
            return ActivationStatus::rejected_unit;
    ...
    return ActivationInfo{
        ...
        value.has_active_generation != 0U ? std::optional<std::uint64_t>(value.active_generation)
                                          : std::nullopt,
        value.diagnostic_count,
    };
}

[[nodiscard]] inline ActionDispatchInfo
dispatch_info(const strata_action_dispatch_info& value) noexcept {
    const auto status = [&value] {
        switch (value.status) {
        case STRATA_ACTION_DISPATCH_NO_ACTION:
            return ActionDispatchStatus::no_action;
        case STRATA_ACTION_DISPATCH_HANDLED:
            return ActionDispatchStatus::handled;
        case STRATA_ACTION_DISPATCH_FORWARDED:
            return ActionDispatchStatus::forwarded;
        case STRATA_ACTION_DISPATCH_IGNORED:
            return ActionDispatchStatus::ignored;
        case STRATA_ACTION_DISPATCH_UNHANDLED:
            return ActionDispatchStatus::unhandled;
        case STRATA_ACTION_DISPATCH_FAILED:
            return ActionDispatchStatus::failed;
        default:
            return ActionDispatchStatus::failed;
        }
    }();
...
[[nodiscard]] inline strata_action_dispatch_config
action_dispatch(const ActionDispatch& action) noexcept {
    const std::string_view source =
        action.source_key.has_value() ? std::string_view(*action.source_key) : std::string_view{};
    const std::string_view state_scope =
        action.state_scope.has_value() ? std::string_view(*action.state_scope) : std::string_view{};
...
class Snapshot final {
  public:
    ...
    [[nodiscard]] strata_snapshot_info info() const {
        ...
        result.struct_size = sizeof(result);
        detail::require_result(strata_snapshot_get_info(value_, &result), "snapshot info read",
                               owner_.get());
        return result;
    }
  ...
  private:
    ...
    void reset() noexcept {
        if (value_ != nullptr)
            strata_snapshot_release(std::exchange(value_, nullptr));
        owner_.reset();
    }
...
class ApplicationStateSnapshot final {
  ...
  private:
    friend class Runtime;
    ApplicationStateSnapshot(std::shared_ptr<detail::RuntimeControl> owner,
                             strata_application_state_snapshot* value) noexcept
        : owner_(std::move(owner)), value_(value) {}
    void reset() noexcept {
        if (value_ != nullptr) {
...
class Runtime final {
  public:
    ...
    explicit Runtime(RuntimeOptions options) {
        if (!options.clock)
            throw std::invalid_argument("Runtime requires a monotonic clock");
    ...
    explicit Runtime(const strata_runtime_config& config) {
        ...
        auto callbacks =
            std::make_shared<detail::RuntimeCallbacks>(RuntimeOptions{}, config.diagnostics);
    ...
    [[nodiscard]] Snapshot snapshot() {
        ...
        require(strata_runtime_create_snapshot(native_handle(), &value),
    ...
    [[nodiscard]] ApplicationStateSnapshot application_state() {
        ...
        require(strata_runtime_create_application_state_snapshot(native_handle(), &value),
                "application state snapshot creation");
        return ApplicationStateSnapshot(control_, value);
    }

    [[nodiscard]] bool restore_application_state(const ApplicationStateSnapshot& snapshot) {
        std::uint32_t changed = 0U;
        require(
            strata_runtime_restore_application_state(native_handle(), snapshot.value_, &changed),
    ...
    [[nodiscard]] DiagnosticsSnapshot diagnostics() const {
        ...
        require(strata_runtime_read_diagnostics(native_handle(), &sink),
    ...
    void set_profiler_capture(const bool enabled) {
        require(strata_runtime_set_profiler_capture(native_handle(), enabled ? 1U : 0U),
    ...
    [[nodiscard]] strata_host_snapshot_info host_snapshot_info() const {
        ...
        require(strata_runtime_get_host_snapshot_info(native_handle(), &info),
                "host snapshot info read");
        return info;
    }

    [[nodiscard]] std::optional<std::string> host_value_json(const std::string_view path) {
        ...
        const strata_result result = strata_runtime_read_host_value_json(
            native_handle(), strata_string_view{path.data(), path.size()}, &sink);
    ...
    [[nodiscard]] std::string active_unit_json() {
        ...
        require(strata_runtime_read_active_unit_json(native_handle(), &sink), "active unit read");
        if (capture.failed)
            throw std::bad_alloc();
        return std::move(capture.value);
    }
    ...
    [[nodiscard]] strata_runtime_memory_info memory_info() const {
        ...
        info.struct_size = sizeof(info);
        require(strata_runtime_get_memory_info(native_handle(), &info), "runtime memory telemetry");
        return info;
    }

    /** The materials this application declares a source for on one backend. */
    [[nodiscard]] std::vector<MaterialDeclaration>
    material_declarations(const std::string_view backend) const {
        const strata_string_view backend_view{backend.data(), backend.size()};
        std::size_t count = 0U;
        require(strata_runtime_read_material_declarations(native_handle(), backend_view, nullptr,
                                                          0U, &count),
                "material declaration count");
        if (count == 0U)
            return {};
        std::vector<strata_material_declaration> native(count);
        require(strata_runtime_read_material_declarations(native_handle(), backend_view,
                                                          native.data(), native.size(), &count),
    ...
    [[nodiscard]] std::vector<EffectPassDeclaration>
    effect_pass_declarations(const std::string_view backend) const {
        const strata_string_view backend_view{backend.data(), backend.size()};
        std::size_t count = 0U;
        require(strata_runtime_read_effect_pass_declarations(native_handle(), backend_view, nullptr,
                                                             0U, &count),
                "effect declaration count");
        if (count == 0U)
            return {};
        std::vector<strata_effect_pass_declaration> native(count);
        require(strata_runtime_read_effect_pass_declarations(native_handle(), backend_view,
                                                             native.data(), native.size(), &count),
        ...
        for (std::size_t index = 0U; index < count; ++index) {
            declarations.push_back(EffectPassDeclaration{
                ...
                native[index].index,
                native[index].kind == STRATA_EFFECT_PASS_SHADER   ? EffectPassKind::shader
                : native[index].kind == STRATA_EFFECT_PASS_SHADOW ? EffectPassKind::shadow
                                                                  : EffectPassKind::blur,
                native[index].radius,
    ...
    void configure_application(const strata_application_config& config) {
        require(strata_runtime_configure_application(native_handle(), &config),
                "application configuration");
    }

    void set_resource_adapter(ResourceAdapter adapter) {
        if (!adapter.load) {
            throw std::invalid_argument("resource adapter requires a load callback");
        }
        ...
        require(strata_runtime_set_resource_adapter(native_handle(), &native),
                "resource adapter installation");
        control_->resource = std::move(bridge);
    }

    [[nodiscard]] std::optional<std::vector<std::uint8_t>> resource(const std::string_view id) {
        detail::ByteCapture capture;
        const strata_bytes_sink sink{
            sizeof(strata_bytes_sink),
            &capture,
            &detail::capture_bytes,
        };
    ...
    void set_durable_store(DurableStoreAdapter adapter) {
        ...
        require(strata_runtime_set_durable_store_adapter(native_handle(), &native),
    ...
    void set_durable_store(const strata_durable_store_adapter& adapter) {
        require(strata_runtime_set_durable_store_adapter(native_handle(), &adapter),
                "durable store installation");
    }
    ...
    void set_async_host(AsyncHostAdapter adapter) {
        ...
        require(strata_runtime_set_async_host_adapter(native_handle(), &native),
                "async host installation");
        control_->async = std::move(bridge);
    }

    void set_async_host(const strata_async_host_adapter& adapter) {
        require(strata_runtime_set_async_host_adapter(native_handle(), &adapter),
                "async host installation");
    }
    ...
    void set_clipboard_adapter(ClipboardAdapter adapter) {
        ...
        require(strata_runtime_set_clipboard_adapter(native_handle(), &native),
    ...
    void set_clipboard_text(const std::string_view text) {
        require(strata_runtime_clipboard_write(native_handle(),
    ...
    void set_ime_adapter(ImeAdapter adapter) {
        ...
        require(strata_runtime_set_ime_adapter(native_handle(), &native),
                "IME adapter installation");
        control_->ime = std::move(bridge);
    }

    void set_ime_active(const bool active) {
        require(strata_runtime_ime_set_active(native_handle(), active ? 1U : 0U),
                "IME active-state publication");
    }

    void set_ime_cursor_rect(const Rect rect) {
        require(strata_runtime_ime_set_cursor_rect(
    ...
    void set_effect_adapter(EffectAdapter adapter) {
        if (!adapter.emit)
            throw std::invalid_argument("effect adapter requires an emit callback");
        ...
        require(strata_runtime_set_effect_adapter(native_handle(), &native),
                "effect adapter installation");
        control_->effect = std::move(bridge);
    }

    void emit_effect(const EffectRequest& effect) {
        require(strata_runtime_emit_effect_json(
                    native_handle(), strata_string_view{effect.id.data(), effect.id.size()},
                    strata_string_view{effect.payload_json.data(), effect.payload_json.size()}),
                "effect emission");
    }

    [[nodiscard]] std::optional<std::string>
    durable_shell_value_json(const std::string_view key) const {
        detail::StringCapture capture;
        const strata_value_json_sink sink{
            sizeof(strata_value_json_sink),
            &capture,
    ...
    void write_durable_shell_value_json(const std::string_view key,
                                        const std::string_view value_json) {
        require(strata_runtime_write_durable_shell_value_json(
                    native_handle(), strata_string_view{key.data(), key.size()},
                    strata_string_view{value_json.data(), value_json.size()}),
    ...
    void async_progress(const std::uint64_t request_id, const strata_async_progress& progress) {
        require(strata_runtime_async_progress(native_handle(), request_id, &progress),
                "async progress publication");
    }

    void async_succeed(const std::uint64_t request_id, const std::string_view value_json) {
        require(strata_runtime_async_succeed_json(
                    native_handle(), request_id,
                    strata_string_view{value_json.data(), value_json.size()}),
                "async success publication");
    }

    void async_fail(const std::uint64_t request_id, const std::string_view message,
                    const std::string_view code = {}) {
        require(strata_runtime_async_fail(native_handle(), request_id,
    ...
    [[nodiscard]] std::uint64_t publish_host_snapshot(const std::string_view id,
                                                      const std::string_view value_json) {
    ...
    [[nodiscard]] ActivationInfo activate(const SourceActivation& activation) {
        ...
        const strata_activation_config config{
            ...
            strata_string_view{activation.entry_source_id.data(),
                               activation.entry_source_id.size()},
    ...
    [[nodiscard]] ActivationInfo activate(const std::uint64_t generation,
    ...
    [[nodiscard]] strata_activation_info activate(const strata_activation_config& config) {
        ...
        info.struct_size = sizeof(info);
        require(strata_runtime_compile_and_activate(native_handle(), &config, &info),
                "application activation");
        return info;
    }

    [[nodiscard]] strata_activation_info activate(const strata_compiled_activation_config& config) {
        strata_activation_info info{};
        info.struct_size = sizeof(info);
        require(strata_runtime_activate_compiled_module(native_handle(), &config, &info),
    ...
    [[nodiscard]] ActionDispatchInfo dispatch(const ActionDispatch& action) {
        ...
        info.struct_size = sizeof(info);
        require(strata_runtime_dispatch_action_json(native_handle(), &config, &info),
                "runtime action dispatch");
        return detail::dispatch_info(info);
    }

    [[nodiscard]] std::optional<std::string>
    source_map_entry_json(const std::string_view compiled_path) {
        detail::StringCapture capture;
        const strata_value_json_sink sink{
            sizeof(strata_value_json_sink),
            &capture,
        ...
        const strata_result result = strata_runtime_read_source_map_entry_json(
            native_handle(), strata_string_view{compiled_path.data(), compiled_path.size()}, &sink);
        if (result.status == STRATA_STATUS_NOT_FOUND)
            return std::nullopt;
    ...
    [[nodiscard]] std::string source_map_entries_json(const std::string_view source_id,
        ...
        require(strata_runtime_read_source_map_entries_at_json(
                    native_handle(), strata_string_view{source_id.data(), source_id.size()}, line,
                    column, &sink),
...
class Surface final {
  public:
    ...
    [[nodiscard]] strata_surface* native_handle() const noexcept {
        return value_;
    }
    [[nodiscard]] bool live() const noexcept {
    ...
    [[nodiscard]] bool adopt_environment(const SurfaceEnvironment& environment) {
        ...
        require(strata_surface_adopt_environment(value_, &native, &adopted),
    ...
    [[nodiscard]] InputBatchInfo enqueue(const std::span<const InputEvent> events) {
        ...
        for (const InputEvent& event : events)
            native.push_back(event.native());
        strata_surface_input_batch_info info{};
        info.struct_size = sizeof(info);
        require(strata_surface_enqueue_input(value_, native.data(), native.size(), &info),
    ...
    [[nodiscard]] ActionDispatchInfo dispatch(const ActionDispatch& action) {
        ...
        strata_action_dispatch_info info{};
        info.struct_size = sizeof(info);
        require(strata_surface_dispatch_action_json(value_, &config, &info),
                "surface action dispatch");
        return detail::dispatch_info(info);
    }

    [[nodiscard]] bool set_focus_containment(const std::optional<std::string_view> key) {
        std::uint32_t contained = 0U;
        const std::string_view value = key.value_or(std::string_view{});
        require(strata_surface_set_focus_containment(
    ...
    [[nodiscard]] DiagnosticsSnapshot diagnostics() const {
        ...
        require(strata_surface_read_diagnostics(value_, &sink), "surface diagnostics read");
        if (capture.failed || !capture.emitted) {
    ...
    void set_profiler_capture(const bool enabled) {
        require(strata_surface_set_profiler_capture(value_, enabled ? 1U : 0U),
    ...
    void record_host_frame(const ProfilerHostFrame& telemetry) {
        ...
        require(strata_surface_record_host_frame(value_, &native), "surface host-frame telemetry");
    }
    ...
    [[nodiscard]] bool inspector_select(const std::string_view key) {
        ...
        require(strata_surface_inspector_select(value_, strata_string_view{key.data(), key.size()},
                                                &selected),
    ...
    [[nodiscard]] bool inspector_pick(const Point point) {
        std::uint32_t selected = 0U;
        require(strata_surface_inspector_pick(value_, point.x, point.y, &selected),
    ...
    [[nodiscard]] std::string inspector_selection_json() const {
        ...
        require(strata_surface_read_inspector_selection_json(value_, &sink),
    ...
    [[nodiscard]] std::vector<std::uint8_t> prepare_release_packet() {
        ...
        require(strata_surface_prepare_release_packet(value_, &sink),
    ...
    void acknowledge_release_packet() {
        require(strata_surface_acknowledge_release_packet(value_),
    ...
    [[nodiscard]] bool set_theme(const strata_theme& theme) {
        std::uint32_t changed = 0U;
        require(strata_surface_set_theme(value_, &theme, &changed), "surface root theme selection");
        return changed != 0U;
    }

    [[nodiscard]] bool unregister_theme(const std::string_view name) {
        std::uint32_t removed = 0U;
        require(strata_surface_unregister_theme(
                    value_, strata_string_view{name.data(), name.size()}, &removed),
                "surface theme removal");
        return removed != 0U;
    }

    [[nodiscard]] bool set_scoped_theme(const std::string_view node_key,
                                        const strata_theme& theme) {
        std::uint32_t changed = 0U;
        require(strata_surface_set_scoped_theme(
                    value_, strata_string_view{node_key.data(), node_key.size()}, &theme, &changed),
    ...
    [[nodiscard]] bool clear_scoped_theme(const std::string_view node_key) {
        std::uint32_t removed = 0U;
        require(strata_surface_clear_scoped_theme(
    ...
    [[nodiscard]] bool animate_scroll_to(const strata_scroll_animation_request& request) {
        ...
        require(strata_surface_animate_scroll_to(value_, &request, &started),
                "surface scroll animation");
        return started != 0U;
    }

    [[nodiscard]] bool
    animate_scroll_to(const std::string_view key, const std::optional<double> x,
                      const std::optional<double> y, const std::string_view timing = "standard",
  ...
  private:
    ...
    void require(const strata_result result, const std::string_view operation) const {
        detail::require_result(result, operation, owner_.get());
    }
...
inline Surface Runtime::create_surface(const SurfaceOptions& options) const {
    ...
    for (const ImageResource& image : options.images) {
        images.push_back(strata_surface_image_resource{
            ...
            image.sampling == ImageSampling::nearest ? STRATA_IMAGE_SAMPLING_NEAREST
    ...
    const strata_surface_config config{
        ...
        strata_string_view{options.id.data(), options.id.size()},
        options.root_role == SurfaceRootRole::screen ? STRATA_SURFACE_ROOT_SCREEN
...
inline Surface Runtime::create_surface(const strata_surface_config& config) const {
    ...
    require(strata_runtime_create_surface(native_handle(), &config, &value), "surface creation");
