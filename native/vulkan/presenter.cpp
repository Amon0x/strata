...
void capture_bytes(void* const user_data, const strata_bytes_view bytes) noexcept {
...
material_declarations(strata_runtime* const runtime) {
    constexpr std::string_view backend = "hlsl";
    const strata_string_view backend_view{backend.data(), backend.size()};
    std::size_t count = 0U;
    require_ok(
        strata_runtime_read_material_declarations(runtime, backend_view, nullptr, 0U, &count),
        "Vulkan material declaration count");
    std::vector<strata_material_declaration> native(count);
    if (count != 0U) {
        require_ok(strata_runtime_read_material_declarations(runtime, backend_view, native.data(),
                                                             native.size(), &count),
                   "Vulkan material declaration read");
    }
...
effect_pass_declarations(strata_runtime* const runtime) {
    constexpr std::string_view backend = "hlsl";
    const strata_string_view backend_view{backend.data(), backend.size()};
    std::size_t count = 0U;
    require_ok(
        strata_runtime_read_effect_pass_declarations(runtime, backend_view, nullptr, 0U, &count),
        "Vulkan effect declaration count");
    std::vector<strata_effect_pass_declaration> native(count);
    if (count != 0U) {
        require_ok(strata_runtime_read_effect_pass_declarations(
                       runtime, backend_view, native.data(), native.size(), &count),
                   "Vulkan effect declaration read");
    }
...
[[nodiscard]] std::optional<std::vector<std::uint8_t>> resource(strata_runtime* const runtime,
    ...
    const strata_result result =
        strata_runtime_read_resource(runtime, strata_string_view{id.data(), id.size()}, &sink);
    if (result.status == STRATA_STATUS_NOT_FOUND)
        return std::nullopt;
    require_ok(result, "Vulkan program resource read");
    if (capture.failed)
        throw std::bad_alloc();
...
struct Presenter::Impl final {
    ...
    Impl(strata_runtime* const runtime, Device device, PresenterOptions options)
        : runtime(runtime), source_loader(std::move(options.load_program_source)),
          renderer(device) {
        if (runtime == nullptr) {
            throw std::invalid_argument("Vulkan presenter requires a live Runtime");
        }
    }
    ...
    [[nodiscard]] std::string load_source(const std::string_view resource_id) {
        if (resource_id.empty())
            return {};
        if (source_loader)
            return source_loader(resource_id);
        const std::optional<std::vector<std::uint8_t>> bytes = resource(runtime, resource_id);
        if (!bytes.has_value()) {
            throw std::runtime_error("Vulkan program resource is unavailable: " +
    ...
    void synchronize_programs() {
        ...
        for (const LoadedEffectPass& effect : effects) {
            renderer.declare_effect_pass(
                effect.declaration.effect_id, effect.declaration.index,
                static_cast<std::uint32_t>(effect.declaration.kind), effect.declaration.radius,
    ...
    [[nodiscard]] bool reload_program_source(const std::string_view resource_id,
        ...
        if (resource_id.empty()) {
            throw std::invalid_argument("Vulkan program resource id must not be empty");
        }
        bool declared = false;
    ...
    void attach(const std::string_view layer_id, strata_surface* const surface) {
        if (layer_id.empty()) {
            throw std::invalid_argument("Vulkan presenter layer id must not be empty");
        }
        if (surface == nullptr) {
            throw std::invalid_argument("Vulkan presenter requires a live Surface");
        }
        const auto found = layers.find(layer_id);
        if (found != layers.end()) {
            if (!found->second.stream->matches(surface)) {
                throw std::invalid_argument(
                    "Vulkan presenter layer id is attached to another Surface");
            }
            return;
        }
        layers.emplace(std::string(layer_id),
                       Layer{std::make_unique<host::SurfacePacketStream>(surface)});
    }

    [[nodiscard]] PresentedFrame present(const std::string_view layer_id,
                                         strata_surface* const surface, const RenderTarget& target,
                                         const std::int64_t time_nanoseconds,
                                         FrameOptions options) {
        if (!programs_synchronized)
...
Presenter::Presenter(Runtime& runtime, Device device, PresenterOptions options)
    : Presenter(runtime.native_handle(), device, std::move(options)) {}

Presenter::Presenter(strata_runtime* const runtime, Device device, PresenterOptions options)
    : impl_(std::make_unique<Impl>(runtime, device, std::move(options))) {}

Presenter::~Presenter() = default;
Presenter::Presenter(Presenter&&) noexcept = default;
Presenter& Presenter::operator=(Presenter&&) noexcept = default;

void Presenter::synchronize_programs() { impl_->synchronize_programs(); }

bool Presenter::reload_program_source(const std::string_view resource_id,
                                      const std::string_view hlsl_source) {
...
void Presenter::attach(const std::string_view layer_id, strata_surface* const surface) {
    impl_->attach(layer_id, surface);
}

PresentedFrame Presenter::present(const std::string_view layer_id, Surface& surface,
                                  const RenderTarget& target, const std::int64_t time_nanoseconds,
                                  const FrameOptions options) {
    return present(layer_id, surface.native_handle(), target, time_nanoseconds, options);
}

PresentedFrame Presenter::present(const std::string_view layer_id, strata_surface* const surface,
                                  const RenderTarget& target, const std::int64_t time_nanoseconds,
                                  const FrameOptions options) {
    return impl_->present(layer_id, surface, target, time_nanoseconds, options);
}

void Presenter::detach(const std::string_view layer_id) { impl_->detach(layer_id); }

void Presenter::discard(const std::string_view layer_id) noexcept { impl_->discard(layer_id); }
