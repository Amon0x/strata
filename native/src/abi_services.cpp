...
strata_result strata_runtime_set_resource_adapter(
    ...
    if (adapter == nullptr) {
        return runtime_failure(
            *runtime,
            ...
            "Resource adapters cannot be detached; install a newer nonzero generation instead."
        );
    }
    if (adapter->struct_size < sizeof(strata_resource_adapter) || adapter->load == nullptr ||
        adapter->generation == 0U) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_RESOURCE_ADAPTER",
            "A resource adapter requires the complete ABI structure, load callback, and nonzero generation."
        );
    }
    if (runtime->resources.has_value() &&
        adapter->generation <= runtime->resources->generation) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.RESOURCE.NON_MONOTONIC_GENERATION",
            "A replacement resource adapter generation must be strictly newer than the active generation."
        );
    }

    // Everything after validation is a no-fail commit. Prior materialized resources remain owned
    // and releasable until each gated Surface completes its strong reload transaction.
    static_assert(std::is_nothrow_copy_assignable_v<strata_resource_adapter>);
    static_assert(noexcept(
        std::declval<std::optional<strata_resource_adapter>&>() =
        std::declval<const strata_resource_adapter&>()
    ));
    runtime->resources = *adapter;
    for (strata_surface* const surface : runtime->surfaces) {
        if (surface == nullptr || surface->release_packet_prepared) continue;
        surface->required_resource_generation = adapter->generation;
        surface->resource_reload_required = true;
    }
    return strata::core::result(STRATA_STATUS_OK);
}

strata_result strata_runtime_read_resource(
    strata_runtime* const runtime,
    ...
    if (!valid_view(resource_id, false) || sink == nullptr ||
        ...
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_RESOURCE_READ",
    ...
    try {
        ...
        if (!strata::core::valid_utf8(id)) throw std::invalid_argument("resource id must be valid UTF-8");
        strata_bytes_view borrowed{nullptr, 0U};
        const strata_status status = runtime->resources->load(
            runtime->resources->user_data,
        ...
        if (borrowed.size != 0U) copied.assign(borrowed.data, borrowed.data + borrowed.size);
        ...
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.RESOURCE.INVALID_RESULT",
            error.what()
        );
    } catch (...) {
        return runtime_failure(
...
strata_result strata_runtime_set_clipboard_adapter(
    strata_runtime* const runtime,
    ...
    if (adapter->struct_size < sizeof(strata_clipboard_adapter) ||
        adapter->read == nullptr || adapter->write == nullptr) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
...
strata_result strata_runtime_clipboard_read(
    strata_runtime* const runtime,
    ...
    try {
        ...
        const strata_status status = runtime->clipboard->read(runtime->clipboard->user_data, &borrowed);
        if (status != STRATA_STATUS_OK) {
            return runtime_failure(
                *runtime,
                status,
                "STRATA.CLIPBOARD.READ_FAILED",
                "The host clipboard adapter rejected a read."
            );
        }
        if (!valid_view(borrowed, true)) throw std::invalid_argument("clipboard adapter returned an invalid string view");
        const std::string copied = copied_string(borrowed);
        if (!strata::core::valid_utf8(copied)) throw std::invalid_argument("clipboard adapter returned invalid UTF-8");
...
strata_result strata_runtime_clipboard_write(
    strata_runtime* const runtime,
    ...
    try {
        ...
        const strata_status status = runtime->clipboard->write(
            runtime->clipboard->user_data,
            strata_string_view{copied.data(), copied.size()}
        );
...
strata_result strata_runtime_set_ime_adapter(
    strata_runtime* const runtime,
    const strata_ime_adapter* const adapter
) {
    if (runtime == nullptr) return invalid_argument();
    if (adapter != nullptr &&
        (adapter->struct_size < sizeof(strata_ime_adapter) ||
         adapter->set_active == nullptr || adapter->set_cursor_rect == nullptr)) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_IME_ADAPTER",
...
strata_result strata_runtime_ime_set_active(
    strata_runtime* const runtime,
    ...
    try {
        ...
        return status == STRATA_STATUS_OK
                   ? strata::core::result(STRATA_STATUS_OK)
                   : runtime_failure(*runtime, status, "STRATA.IME.UPDATE_FAILED", "The host IME adapter rejected active state.");
    } catch (...) {
        return runtime_failure(*runtime, STRATA_STATUS_INTERNAL_ERROR, "STRATA.ABI.CALLBACK_FAILED", "IME active-state update failed inside the C ABI boundary.");
    }
}

strata_result strata_runtime_ime_set_cursor_rect(
    strata_runtime* const runtime,
    ...
    try {
        const strata_status status = runtime->ime->set_cursor_rect(runtime->ime->user_data, logical_rect);
        return status == STRATA_STATUS_OK
                   ? strata::core::result(STRATA_STATUS_OK)
                   : runtime_failure(*runtime, status, "STRATA.IME.UPDATE_FAILED", "The host IME adapter rejected cursor geometry.");
    } catch (...) {
        return runtime_failure(*runtime, STRATA_STATUS_INTERNAL_ERROR, "STRATA.ABI.CALLBACK_FAILED", "IME cursor update failed inside the C ABI boundary.");
    }
}

strata_result strata_runtime_set_effect_adapter(
    strata_runtime* const runtime,
    ...
    if (adapter->struct_size < sizeof(strata_effect_adapter) || adapter->emit == nullptr) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_EFFECT_ADAPTER",
...
strata_result strata_runtime_emit_effect_json(
    strata_runtime* const runtime,
    ...
    try {
        ...
        if (!strata::core::valid_utf8(id)) throw std::invalid_argument("effect id must be valid UTF-8");
        const strata::data::JsonValue payload = payload_json.size == 0U
                                                   ? strata::data::JsonValue{}
                                                   : strata::data::parse_json(copied_string(payload_json));
        const std::string canonical = strata::data::encode_canonical_json(payload);
        const strata_status status = static_cast<strata_status>(
            runtime->host_services.emit_effect(id, canonical)
        );
        return status == STRATA_STATUS_OK
                   ? strata::core::result(STRATA_STATUS_OK)
                   : runtime_failure(*runtime, status, "STRATA.EFFECT.EMIT_FAILED", "The host effect adapter rejected an effect.");
    } catch (const strata::data::JsonError&) {
        return runtime_failure(*runtime, STRATA_STATUS_INVALID_ARGUMENT, "STRATA.EFFECT.INVALID_JSON", "Effect payload is not valid strict JSON.");
    } catch (const std::invalid_argument& error) {
        return runtime_failure(*runtime, STRATA_STATUS_INVALID_UTF8, "STRATA.EFFECT.INVALID_ID", error.what());
    } catch (const std::bad_alloc&) {
        return runtime_failure(*runtime, STRATA_STATUS_OUT_OF_MEMORY, "STRATA.CORE.OUT_OF_MEMORY", "Effect emission exhausted memory.");
    } catch (...) {
        return runtime_failure(*runtime, STRATA_STATUS_INTERNAL_ERROR, "STRATA.ABI.CALLBACK_FAILED", "Effect emission failed inside the C ABI boundary.");
    }
}

strata_result strata_runtime_set_durable_store_adapter(
    strata_runtime* const runtime,
    ...
    if (!runtime->core.has_application() || runtime->durable_store.has_value() ||
        ...
        adapter->struct_size < sizeof(strata_durable_store_adapter) ||
        adapter->load == nullptr || adapter->write == nullptr) {
        return runtime_failure(
            *runtime,
            STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_DURABLE_STORE_ADAPTER",
            "Durable store installation is one-time and requires a configured application, no created Surface, and complete load/write callbacks."
        );
    }
    ...
    runtime->core.application().configure_durable_store(strata::runtime::DurableStoreAdapter{
        [runtime](const std::string_view application_id) -> std::optional<std::string> {
            ...
            if (status == STRATA_STATUS_NOT_FOUND) return std::nullopt;
            if (status != STRATA_STATUS_OK) {
                throw std::runtime_error("host durable load returned status " + std::to_string(status));
            }
            if (bytes.data == nullptr && bytes.size != 0U) {
                throw std::invalid_argument("durable load returned null non-empty bytes");
            }
            std::string copied = bytes.size == 0U
                ? std::string{}
                : std::string(
        ...
        [runtime](const std::string_view application_id, const std::string_view payload) {
            ...
            if (status != STRATA_STATUS_OK) {
                throw std::runtime_error("host durable write returned status " + std::to_string(status));
            }
        },
    });
    return strata::core::result(STRATA_STATUS_OK);
...
strata_result strata_runtime_read_durable_shell_value_json(
    strata_runtime* const runtime,
    ...
    if (!runtime->core.has_application() || !valid_view(key, false) || sink == nullptr ||
        ...
        return runtime_failure(
            *runtime, STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_DURABLE_READ",
    ...
    if (value == nullptr) return strata::core::result(STRATA_STATUS_NOT_FOUND);
    try {
        ...
        return runtime_failure(
            *runtime, STRATA_STATUS_INTERNAL_ERROR,
            "STRATA.ABI.CALLBACK_FAILED",
...
strata_result strata_runtime_write_durable_shell_value_json(
    strata_runtime* const runtime,
    const strata_string_view key,
    ...
    if (!runtime->core.has_application() || !valid_view(key, false) ||
        ...
        return runtime_failure(
            *runtime, STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_DURABLE_WRITE",
    ...
    try {
        runtime->core.application().durability().set_shell_value(
            copied_string(key),
            strata::runtime::value_from_json(
                strata::data::parse_json(copied_string(value_json))
            )
        );
        return strata::core::result(STRATA_STATUS_OK);
...
strata_result strata_runtime_flush_durable_state(strata_runtime* const runtime) {
    ...
    return issue.has_value()
        ? runtime_failure(
              *runtime, STRATA_STATUS_INTERNAL_ERROR, issue->code.c_str(), issue->message.c_str()
          )
        : strata::core::result(STRATA_STATUS_OK);
}

strata_result strata_runtime_set_async_host_adapter(
    strata_runtime* const runtime,
    const strata_async_host_adapter* const adapter
) {
    if (runtime == nullptr) return invalid_argument();
    if (!runtime->core.has_application() || runtime->async_host.has_value() ||
        adapter == nullptr || adapter->struct_size < sizeof(strata_async_host_adapter) ||
        adapter->begin == nullptr || adapter->cancel == nullptr) {
        return runtime_failure(
            *runtime, STRATA_STATUS_INVALID_ARGUMENT,
            "STRATA.ABI.INVALID_ASYNC_HOST_ADAPTER",
            "Async host installation is one-time and requires a configured application and complete begin/cancel callbacks."
        );
    }
...
strata_result strata_runtime_async_progress(
    strata_runtime* const runtime,
    ...
    if (!runtime->core.has_application() || progress == nullptr ||
        progress->struct_size < sizeof(strata_async_progress) ||
        progress->has_total > 1U || progress->reserved != 0U ||
        !valid_view(progress->message, true)) {
    ...
    try {
        ...
        return strata::core::result(
            accepted ? STRATA_STATUS_OK : STRATA_STATUS_INVALID_ARGUMENT
        );
    } catch (const std::bad_alloc&) {
...
strata_result strata_runtime_async_succeed_json(
    strata_runtime* const runtime,
    const std::uint64_t request_id,
    const strata_string_view value_json
) {
...
strata_result strata_runtime_async_fail(
    strata_runtime* const runtime,
    const std::uint64_t request_id,
    ...
    try {
        const bool accepted = runtime->core.application().async().post_fail(
            request_id,
            strata::runtime::AsyncFailure{copied_string(message), copied_string(code)}
        );
        return strata::core::result(
            accepted ? STRATA_STATUS_OK : STRATA_STATUS_INVALID_ARGUMENT
        );
