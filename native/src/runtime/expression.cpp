...
[[nodiscard]] JsonValue required(const JsonValue value, const std::string_view field) {
    ...
    if (!found) throw std::runtime_error("runtime IR is missing field '" + std::string(field) + "'");
    return found;
}
...
[[nodiscard]] std::string_view string_field(
    ...
    if (!text.has_value()) throw std::runtime_error("runtime IR field must be a string");
    return *text;
}
...
[[nodiscard]] JsonArray array_field(const JsonValue value, const std::string_view field) {
    ...
    if (!array.has_value()) throw std::runtime_error("runtime IR field must be an array");
    return *array;
}
...
[[nodiscard]] JsonObject object_field(const JsonValue value, const std::string_view field) {
    ...
    if (!object.has_value()) throw std::runtime_error("runtime IR field must be an object");
    return *object;
}

[[nodiscard]] double json_number(const JsonValue value) {
    if (const std::optional<double> number = value.number(); number.has_value()) return *number;
...
[[nodiscard]] ColorValue parse_color(const std::string_view rgba) {
    if (rgba.size() != 8U) throw std::runtime_error("portable IR color must contain RGBA bytes");
    std::uint8_t channels[4]{};
    for (std::size_t index = 0U; index < 4U; ++index) {
        const int high = hexadecimal(rgba[index * 2U]);
        const int low = hexadecimal(rgba[index * 2U + 1U]);
        if (high < 0 || low < 0) throw std::runtime_error("portable IR color is not hexadecimal");
...
[[nodiscard]] int compare_keys(const Value& left, const Value& right) {
    ...
    if (left.duration() != nullptr && right.duration() != nullptr) {
        return left.duration()->nanoseconds < right.duration()->nanoseconds
                   ? -1
...
[[nodiscard]] const ValueList* collection_items(const ExpressionValue& value) {
    if (const Value* scalar = value.value()) return scalar->list();
    if (const auto* collection = value.collection()) return (*collection)->items.list();
    return nullptr;
}
...
[[nodiscard]] Value materialized_value(const ExpressionValue& value) {
    if (const Value* scalar = value.value()) return *scalar;
    if (const auto* collection = value.collection()) return (*collection)->items;
    return Value{};
}
...
struct CollectionDependencyTrace final : ExpressionDependencyObserver {
    ...
    void lexical(
        ...
        if (parent != nullptr) parent->lexical(name, value);
        if (!value.cacheable()) {
            cacheable = false;
            return;
        }
        const auto [stored, inserted] = lexical_values.insert_or_assign(
            std::string(name),
            value
        );
        if (inserted) order.emplace_back(false, stored->first);
    }

    void host(const ExpressionHostDependency& dependency) override {
        if (parent != nullptr) parent->host(dependency);
        const auto [stored, inserted] = host_values.insert_or_assign(
            canonical_host_dependency_path(dependency.path),
            dependency
        );
        if (inserted) order.emplace_back(true, stored->first);
    }
...
struct LambdaDependencyFilter final : ExpressionDependencyObserver {
    LambdaDependencyFilter(
        ExpressionDependencyObserver* source_parent,
    ...
    void lexical(
        ...
        if (parent != nullptr && name != parameter) parent->lexical(name, value);
    }
...
[[nodiscard]] std::pair<std::size_t, std::size_t> collection_counts(const ExpressionValue& value) {
    if (const auto* collection = value.collection()) return {(*collection)->total, (*collection)->matched};
...
[[nodiscard]] std::optional<ActionOrigin> action_origin(
    ...
    if (!source_id.has_value() || !start || !end) return std::nullopt;
    const auto position = [](const JsonValue value, const std::string_view field) {
        ...
        return number.has_value() && *number > 0 &&
                       *number <= static_cast<std::int64_t>(UINT32_MAX)
...
ExpressionValue::ExpressionValue(std::shared_ptr<const CollectionViewValue> value) : storage_(std::move(value)) {}
ExpressionValue::ExpressionValue(std::shared_ptr<const LambdaValue> value) : storage_(std::move(value)) {}
ExpressionValue::ExpressionValue(std::shared_ptr<const ActionValue> value) : storage_(std::move(value)) {}
ExpressionValue::ExpressionValue(std::shared_ptr<const ExpressionListValue> value) : storage_(std::move(value)) {}
ExpressionValue::ExpressionValue(std::shared_ptr<const ExpressionObjectValue> value) : storage_(std::move(value)) {}
const Value* ExpressionValue::value() const noexcept {
    if (const Value* scalar = std::get_if<Value>(&storage_)) return scalar;
    if (const auto* list_value = list()) return &(**list_value).materialized;
    if (const auto* object_value = object()) return &(**object_value).materialized;
    return nullptr;
}
const Value* ExpressionValue::data_value() const noexcept {
    if (const Value* materialized = value()) return materialized;
...
bool ExpressionDependencyValue::cacheable() const noexcept {
    if (kind == ExpressionDependencyValueKind::unsupported) return false;
    ...
    if (kind == ExpressionDependencyValueKind::executable_object) {
        return field_names.size() == field_values.size() && std::ranges::all_of(
            field_values,
            &ExpressionDependencyValue::cacheable
        );
    }
    return true;
}
...
ExpressionValue restore_expression_dependency(const ExpressionDependencyValue& value) {
    ...
    case ExpressionDependencyValueKind::executable_object: {
        ...
        for (std::size_t index = 0U; index < value.field_values.size(); ++index) {
            ExpressionValue restored = restore_expression_dependency(
                value.field_values[index]
            );
            materialized.emplace_back(
                value.field_names[index],
                materialized_value(restored)
            );
...
std::optional<ExpressionDependencyValue> expression_scope_dependency(
    const ExpressionScope& scope,
    const std::string_view name
) {
...
std::string canonical_host_dependency_path(
...
ExpressionValue ExpressionRuntime::evaluate_in(
    const JsonValue expression,
    const ExpressionScope& scope
) {
...
void ExpressionRuntime::clear_diagnostics() { diagnostics_.clear(); }

ExpressionDependencyObserver* ExpressionRuntime::exchange_dependency_observer(
    ExpressionDependencyObserver* const observer
) noexcept {
    return std::exchange(dependency_observer_, observer);
}

ExpressionHostDependency ExpressionRuntime::read_host_dependency(
    const std::span<const HostPathSegment> path,
    const ExpressionScope& scope
) const {
    ...
    if (contextual == scope.contextual_host_roots.end()) {
        ...
        return ExpressionHostDependency{
            ...
            resolution.has_value()
                ? std::optional<Value>(std::move(resolution->value)) : std::nullopt,
            resolution.has_value()
                ? std::optional<std::string>(std::move(resolution->snapshot_id)) : std::nullopt,
    ...
    for (std::size_t index = 1U; index < path.size() && current != nullptr; ++index) {
        ...
        if (segment.kind == HostPathSegmentKind::field) {
            ...
            current = segment.index.has_value() &&
                              *segment.index < current->list()->values.size()
...
ExpressionValue ExpressionRuntime::evaluate(
    const JsonValue expression,
    const ExpressionScope& scope
) {
    struct ActiveScopeRestore final {
        ...
        ~ActiveScopeRestore() { slot = previous; }
    } restore{active_scope_, active_scope_};
    active_scope_ = &scope;
    const std::string_view kind = string_field(expression, "kind");
    if (kind == "literal") return evaluate_literal(expression);
    if (kind == "variable" || kind == "property" || kind == "index") {
        if (std::optional<HostAccess> access = host_access(expression, scope);
            access.has_value()) {
            const ExpressionHostDependency dependency = read_host_dependency(
                access->path,
                scope
            );
            ...
            if (!dependency.value.has_value() && kind == "property") {
                ...
                if (std::optional<HostAccess> receiver = host_access(
                        required(expression, "receiver"), scope
                    ); receiver.has_value()) {
                    const ExpressionHostDependency receiver_dependency = read_host_dependency(
                        receiver->path,
                        scope
                    );
                    const Value* value = receiver_dependency.value.has_value()
                        ? &*receiver_dependency.value : nullptr;
                    computed_property =
            ...
            if (!computed_property) {
                if (dependency_observer_ != nullptr) dependency_observer_->host(dependency);
                if (dependency.value.has_value()) return ExpressionValue(*dependency.value);
    ...
    if (kind == "variable") {
        ...
        if (binding == "host") {
            ...
            report(expression, "STRATA.DSL.RUNTIME_MISSING_BINDING", "Binding '" + name + "' is not available.", "local state, component parameter, or host root");
        }
        return ExpressionValue{};
    }
    if (kind == "list") {
    ...
    if (kind == "action") return evaluate_action(expression, scope);
    if (kind == "helper") return evaluate_helper(expression, scope);
    if (kind == "property") {
        ...
        if (const auto* list_value = receiver.list()) {
            ...
            if (name == "isEmpty") return ExpressionValue(Value((**list_value).values.empty()));
        }
        if (const Value* value = receiver.value()) {
            if (const Value* field = value->field(name)) return ExpressionValue(*field);
            if (const ValueList* list = value->list()) {
                if (name == "size" || name == "length") return ExpressionValue(Value(static_cast<double>(list->values.size())));
                if (name == "isEmpty") return ExpressionValue(Value(list->values.empty()));
            }
            if (const std::string* text = value->string()) {
                if (name == "length") return ExpressionValue(Value(static_cast<double>(utf16_length(*text))));
                if (name == "isEmpty") return ExpressionValue(Value(text->empty()));
            }
        } else if (const auto* view_pointer = receiver.collection()) {
            const CollectionViewValue& view = **view_pointer;
            if (name == "items") return ExpressionValue(view.items);
            if (name == "size" || name == "length" || name == "matched") return ExpressionValue(Value(static_cast<double>(view.matched)));
            if (name == "total") return ExpressionValue(Value(static_cast<double>(view.total)));
            if (name == "rangeStart") return ExpressionValue(Value(static_cast<double>(view.range_start)));
            if (name == "rangeEnd") return ExpressionValue(Value(static_cast<double>(view.range_end_exclusive)));
            if (name == "isEmpty") return ExpressionValue(Value(view.matched == 0U));
            if (name == "cacheHits") return ExpressionValue(Value(static_cast<double>(view.cache_hits.load(std::memory_order_relaxed))));
            if (name == "rebuilds") return ExpressionValue(Value(static_cast<double>(view.rebuilds)));
        }
        report(expression, "STRATA.DSL.RUNTIME_MISSING_PROPERTY", "Property '" + name + "' is not available on this value.");
        return ExpressionValue{};
    }
    if (kind == "index") {
        const ExpressionValue receiver = evaluate(required(expression, "receiver"), scope);
        const Value index = require_value(evaluate(required(expression, "index"), scope), expression);
        if (const auto* list_value = receiver.list()) {
        ...
        if (const auto* object_value = receiver.object()) {
            const std::string name = index.string() != nullptr ? *index.string() : display_string(index);
    ...
    if (kind == "conditional") {
        const Value condition = require_value(evaluate(required(expression, "condition"), scope), expression);
        return evaluate(required(expression, truthy(condition) ? "then" : "else"), scope);
    }
    if (kind == "unary") {
        const Value operand = require_value(evaluate(required(expression, "operand"), scope), expression);
        const std::string_view operation = string_field(expression, "operator");
        if (operation == "not") return ExpressionValue(Value(!truthy(operand)));
        if (operation == "negate") {
            if (operand.number() != nullptr) return ExpressionValue(Value(-*operand.number()));
            if (operand.duration() != nullptr) return ExpressionValue(Value(DurationValue{-operand.duration()->nanoseconds}));
        }
        report(expression, "STRATA.DSL.RUNTIME_TYPE_MISMATCH", "Unary operator received an incompatible value.");
        return ExpressionValue(Value(0.0));
    }
    if (kind == "group") return evaluate(required(expression, "expression"), scope);
    if (kind == "binary") {
        const std::string_view operation = string_field(expression, "operator");
        const Value left = require_value(evaluate(required(expression, "left"), scope), expression);
        if (operation == "and" && !truthy(left)) return ExpressionValue(Value(false));
        if (operation == "or" && truthy(left)) return ExpressionValue(Value(true));
        if (operation == "coalesce" && left.kind() != ValueKind::null_value) return ExpressionValue(left);
        const Value right = require_value(evaluate(required(expression, "right"), scope), expression);
        if (operation == "and" || operation == "or") return ExpressionValue(Value(truthy(right)));
        if (operation == "coalesce") return ExpressionValue(right);
        if (operation == "equal") return ExpressionValue(Value(left == right));
        if (operation == "not_equal") return ExpressionValue(Value(!(left == right)));
        ...
        const double left_numeric = left_number != nullptr
                                        ? *left_number
                                        : left.duration() != nullptr
                                            ? static_cast<double>(left.duration()->nanoseconds)
                                            : 0.0;
        const double right_numeric = right_number != nullptr
                                         ? *right_number
        ...
        if ((operation == "less" || operation == "less_equal" || operation == "greater" ||
             operation == "greater_equal") && numeric) {
            return ExpressionValue(Value(
                operation == "less"          ? left_numeric < right_numeric
        ...
        if (numeric) {
            double result = 0.0;
            if (operation == "add") result = left_numeric + right_numeric;
            else if (operation == "subtract") result = left_numeric - right_numeric;
            else if (operation == "multiply") result = left_numeric * right_numeric;
            else if (operation == "divide") result = right_numeric == 0.0 ? 0.0 : left_numeric / right_numeric;
            else if (operation == "modulo") result = right_numeric == 0.0 ? 0.0 : std::fmod(left_numeric, right_numeric);
            else {
                report(expression, "STRATA.DSL.RUNTIME_UNKNOWN_OPERATOR", "Binary operator '" + operation + "' is not supported.");
                return ExpressionValue{};
            }
        ...
        report(expression, "STRATA.DSL.RUNTIME_TYPE_MISMATCH", "Binary operator received incompatible values.");
        return ExpressionValue(Value(0.0));
    }
    if (kind == "error") return ExpressionValue{};
    ...
    report(expression, "STRATA.DSL.RUNTIME_UNKNOWN_EXPRESSION", "Portable expression kind '" + kind + "' is not supported.");
    return ExpressionValue{};
}
...
ExpressionValue ExpressionRuntime::evaluate_literal(const JsonValue expression) {
    ...
    if (kind == "null") return ExpressionValue{};
    if (kind == "boolean") return ExpressionValue(Value(*required(literal, "value").boolean()));
    ...
    report(expression, "STRATA.DSL.RUNTIME_UNKNOWN_LITERAL", "Portable literal kind '" + kind + "' is not supported.");
    return ExpressionValue{};
}

ExpressionValue ExpressionRuntime::evaluate_action(
    const JsonValue expression,
    const ExpressionScope& scope
) {
    ...
    if (contract == nullptr) {
        report(expression, "STRATA.DSL.RUNTIME_UNKNOWN_ACTION", "Action '" + id + "' is not registered.");
        return ExpressionValue{};
    }
    Value payload;
    ...
    try {
        ...
        auto action = std::make_shared<const Action>(
            contract,
            std::move(payload),
            origin
        );
        ...
        if (id.starts_with("state.")) {
            ...
            const std::string* state_name = state_name_value != nullptr
                                                ? state_name_value->string()
                                                : nullptr;
            if (state_name != nullptr) {
                const auto binding = scope.state_bindings.find(*state_name);
                if (binding != scope.state_bindings.end()) state_binding = binding->second;
            }
        }
        ...
        if (origin.has_value() && origin->line.has_value()) {
            const DiagnosticPosition start{
                *origin->line, origin->column.value_or(1U), std::nullopt,
            };
        ...
        report(RuntimeDiagnostic{
            ...
            origin.has_value() && origin->component_path.has_value()
...
ExpressionValue ExpressionRuntime::evaluate_helper(
    const JsonValue expression,
    const ExpressionScope& scope
) {
    const std::string_view name = string_field(expression, "name");
    ...
    if (name == "sequence") return ExpressionValue(composed_action(expression, scope, ActionCompositionMode::sequence));
    if (name == "parallel") return ExpressionValue(composed_action(expression, scope, ActionCompositionMode::parallel));
    ...
    if (name == "count" || name == "any" || name == "all") {
        ...
        if (!source_expression) return ExpressionValue(name == "count" ? Value(0.0) : Value(false));
        const ExpressionValue source = evaluate(source_expression, scope);
        const ValueList* items = collection_items(source);
        if (items == nullptr) {
            report(expression, "STRATA.DSL.RUNTIME_COLLECTION_SOURCE", "Collection aggregate requires a list or derived view.");
        ...
        const auto predicate_value = predicate_expression
            ? evaluate(predicate_expression, scope)
            : ExpressionValue{};
        const auto* predicate = predicate_value.lambda();
        std::size_t matches = 0U;
        for (const Value& item : items->values) {
            if (predicate == nullptr || truthy(evaluate_lambda(**predicate, item))) ++matches;
            if (name == "any" && matches != 0U) break;
        }
        if (name == "count") return ExpressionValue(Value(static_cast<double>(matches)));
        if (name == "any") return ExpressionValue(Value(matches != 0U));
    ...
    if (name == "min" || name == "max") {
        ...
        for (const JsonValue entry : array_field(expression, "arguments")) {
            ...
            result = first ? number : name == "min" ? std::min(result, number) : std::max(result, number);
            first = false;
        }
    ...
    if (name == "clamp") {
        ...
        const double minimum = minimum_argument.number() != nullptr ? *minimum_argument.number() : value;
        const double maximum = maximum_argument.number() != nullptr ? *maximum_argument.number() : value;
        if (minimum > maximum) {
            report(expression, "STRATA.DSL.RUNTIME_INVALID_RANGE", "clamp minimum exceeds its maximum.");
    ...
    if (name == "abs" || name == "floor" || name == "ceil" || name == "round") {
        ...
        return ExpressionValue(Value(
            name == "abs" ? std::abs(number) : name == "floor" ? std::floor(number) : std::ceil(number)
        ));
    }
    if (name == "length" || name == "size" || name == "isEmpty") {
        ...
        std::size_t size = value.string() != nullptr ? utf16_length(*value.string())
                           ...
                           : value.object() != nullptr ? value.object()->fields.size()
                                                       : 0U;
        return ExpressionValue(name == "isEmpty" ? Value(size == 0U) : Value(static_cast<double>(size)));
    }
    if (name == "join") {
    ...
    if (name == "lower" || name == "upper" || name == "trim" || name == "title") {
        ...
        if (name == "lower") value = lower_ascii(std::move(value));
        else if (name == "upper") value = upper_ascii(std::move(value));
        else if (name == "trim") {
            ...
            for (std::string word; words >> word;) {
                ...
                if (!titled.empty()) titled.push_back(' ');
                titled += word;
            }
    ...
    if (name == "contains") {
        ...
        if (value.string() != nullptr) return ExpressionValue(Value(value.string()->contains(display_string(needle))));
        if (value.list() != nullptr) return ExpressionValue(Value(std::ranges::find(value.list()->values, needle) != value.list()->values.end()));
        return ExpressionValue(Value(false));
    }
    if (name == "startsWith" || name == "endsWith") {
        const std::string value = display_string(argument(expression, scope, "value", 0U));
        const std::string part = display_string(argument(expression, scope, name == "startsWith" ? "prefix" : "suffix", 1U));
        return ExpressionValue(Value(name == "startsWith" ? value.starts_with(part) : value.ends_with(part)));
    }
    if (name == "format") {
        const auto& arguments = array_field(expression, "arguments");
        if (arguments.empty()) return ExpressionValue(Value(""));
        std::string formatted = display_string(require_value(evaluate(required(arguments[0], "value"), scope), arguments[0]));
        for (std::size_t index = 1U; index < arguments.size(); ++index) {
            const std::string marker = "{" + std::to_string(index - 1U) + "}";
            const std::string replacement = display_string(require_value(evaluate(required(arguments[index], "value"), scope), arguments[index]));
    ...
    if (name == "rgb" || name == "rgba") {
        ...
        const std::uint8_t alpha = name == "rgba"
                                       ? color_channel(argument(expression, scope, "alpha", 3U))
                                       : UINT8_MAX;
    ...
    if (name == "effect") {
        ...
        for (const JsonValue entry : array_field(expression, "arguments")) {
            ...
            if (!encoded_name.has_value() || *encoded_name == "name") continue;
            arguments.emplace_back(
                std::string(*encoded_name),
                require_value(evaluate(required(entry, "value"), scope), entry)
            );
        }
        return ExpressionValue(Value(std::vector<std::pair<std::string, Value>>{
            {"arguments", Value(std::move(arguments))},
            {"name", argument(expression, scope, "name", 0U)},
        }));
    }
    report(expression, "STRATA.DSL.RUNTIME_UNKNOWN_HELPER", "Helper '" + name + "' is not available at runtime.", "registered helper");
    return ExpressionValue{};
}

Value ExpressionRuntime::require_value(
    const ExpressionValue& evaluated,
    const JsonValue expression
) {
    if (const Value* value = evaluated.value()) return *value;
    if (const auto* collection = evaluated.collection()) return (*collection)->items;
    report(expression, "STRATA.DSL.RUNTIME_TYPE_MISMATCH", "Expression did not produce a scalar runtime value.");
    return Value{};
}

Value ExpressionRuntime::argument(
    const JsonValue helper,
    const ExpressionScope& scope,
    const std::string_view name,
...
JsonValue ExpressionRuntime::argument_expression(
    const JsonValue helper,
    const std::string_view name,
    const std::size_t position
) const {
...
Value ExpressionRuntime::evaluate_lambda(const LambdaValue& lambda, const Value& input) {
    ...
    struct DependencyObserverRestore final {
        ...
        ~DependencyObserverRestore() { slot = previous; }
...
std::optional<ExpressionRuntime::HostAccess> ExpressionRuntime::host_access(
    const JsonValue expression,
    const ExpressionScope& scope
) {
    const std::string_view kind = string_field(expression, "kind");
    ...
    if (kind != "property" && kind != "index") return std::nullopt;
    std::optional<HostAccess> receiver = host_access(required(expression, "receiver"), scope);
    if (!receiver.has_value()) return std::nullopt;
    if (kind == "property") {
        receiver->path.push_back(HostPathSegment::named(
            std::string(string_field(expression, "name"))
        ));
        return receiver;
    }
    const Value lookup = require_value(
        evaluate(required(expression, "index"), scope),
        expression
    );
...
std::shared_ptr<const CollectionViewValue> ExpressionRuntime::collection_view(
    const JsonValue helper,
    const ExpressionScope& scope
) {
    ...
    if (!source_expression) {
        report(helper, "STRATA.DSL.RUNTIME_COLLECTION_SOURCE", "Collection helper requires a source list.");
        return std::shared_ptr<const CollectionViewValue>(new CollectionViewValue{
            CollectionViewImmutableIdentity{
                Value(std::vector<Value>{}), 0U, 0U, 0U, 0U, std::string(operation),
            },
        });
    }
    ...
    for (CollectionCacheEntry& cached : collection_cache_) {
        ...
        for (const CollectionDependencyRead& read : cached.dependency_order) {
            if (read.kind == CollectionDependencyKind::lexical) {
                ...
                const ExpressionHostDependency current = read_host_dependency(
                    stored->second.path,
                    scope
                );
        ...
        if (!dependencies_current) continue;
        if (dependency_observer_ != nullptr) {
            for (const CollectionDependencyRead& read : cached.dependency_order) {
                if (read.kind == CollectionDependencyKind::lexical) {
                    dependency_observer_->lexical(
    ...
    if (source_items == nullptr) {
        report(helper, "STRATA.DSL.RUNTIME_COLLECTION_SOURCE", "Collection helper requires a list or derived collection view.");
        return std::shared_ptr<const CollectionViewValue>(new CollectionViewValue{
            CollectionViewImmutableIdentity{
                Value(std::vector<Value>{}), 0U, 0U, 0U, 0U, std::string(operation),
            },
    ...
    for (std::size_t index = 1U; index < arguments.size(); ++index) {
        ...
        if (string_field(argument_value, "kind") != "lambda") {
            scalar_arguments.push_back(require_value(evaluate(argument_value, scope), argument_value));
        }
    }

    JsonValue lambda_expression;
    if (operation == "filter" || operation == "takeWhile") lambda_expression = argument_expression(helper, "predicate", 1U);
    else if (operation == "map") lambda_expression = argument_expression(helper, "transform", 1U);
    else if (operation == "sortBy" || operation == "distinctBy" || operation == "groupBy") lambda_expression = argument_expression(helper, "selector", 1U);
    ExpressionValue lambda_value = lambda_expression
        ? evaluate(lambda_expression, scope)
        : ExpressionValue{};
    ...
    if (operation == "filter") {
        for (const Value& value : source_items->values) {
            if (lambda_pointer != nullptr && truthy(evaluate_lambda(**lambda_pointer, value))) result.push_back(value);
        }
    } else if (operation == "map") {
        for (const Value& value : source_items->values) {
            result.push_back(lambda_pointer != nullptr ? evaluate_lambda(**lambda_pointer, value) : Value{});
        }
    } else if (operation == "sortBy") {
        const bool descending = !scalar_arguments.empty() && scalar_arguments.back().boolean() != nullptr && *scalar_arguments.back().boolean();
        ...
        std::stable_sort(decorated.begin(), decorated.end(), [this, descending](
            const auto& left,
            const auto& right
        ) {
        ...
        for (const Value& value : source_items->values) {
            const Value key = lambda_pointer != nullptr ? evaluate_lambda(**lambda_pointer, value) : value;
        ...
        for (const Value& value : source_items->values) {
            const Value key = lambda_pointer != nullptr ? evaluate_lambda(**lambda_pointer, value) : Value{};
            auto found = std::ranges::find_if(groups, [&key](const auto& group) { return group.first == key; });
            if (found == groups.end()) {
        ...
        for (auto& [key, items] : groups) {
            result.emplace_back(std::vector<std::pair<std::string, Value>>{
                {"key", key}, {"items", Value(std::move(items))},
            });
        }
        ...
        for (const Value& value : source_items->values) {
            if (lambda_pointer == nullptr || !truthy(evaluate_lambda(**lambda_pointer, value))) break;
            result.push_back(value);
        }
    } else if (operation == "window" || operation == "page") {
        const std::size_t first = !scalar_arguments.empty() ? bounded_index(scalar_arguments[0]).value_or(0U) : 0U;
        const std::size_t amount = scalar_arguments.size() > 1U
                                       ? std::min(bounded_index(scalar_arguments[1]).value_or(0U), maximum_derived_items)
                                       : 0U;
        const std::size_t offset = operation == "page" && amount != 0U && first <= std::numeric_limits<std::size_t>::max() / amount
                                       ? first * amount
                                       : operation == "page" ? std::numeric_limits<std::size_t>::max() : first;
        if (offset < source_items->values.size()) {
            const std::size_t end = std::min(source_items->values.size(), offset + std::min(amount, source_items->values.size() - offset));
            result.insert(
                result.end(),
    ...
    if (result.size() > maximum_derived_items) {
        ...
        report(helper, "STRATA.DSL.RUNTIME_COLLECTION_BOUND_EXCEEDED", "Collection helper output exceeded the runtime bound.");
    }
    if (operation == "map" && std::ranges::any_of(result, [](const Value& value) {
            if (value.object() == nullptr) return false;
            const Value* key = value.field("key");
            if (key == nullptr) key = value.field("id");
            return key == nullptr || key->kind() == ValueKind::null_value ||
                   key->kind() == ValueKind::list || key->kind() == ValueKind::object;
        })) {
        report(
            helper,
            "STRATA.DSL.RUNTIME_COLLECTION_UNSTABLE_KEY",
            "Mapped record results must expose a stable 'key' or 'id' field before they are repeated.",
    ...
    if ((operation == "window" || operation == "page") && !scalar_arguments.empty()) {
        ...
        if (operation == "page" && scalar_arguments.size() > 1U) {
            ...
            range_start = amount != 0U && range_start <= std::numeric_limits<std::size_t>::max() / amount
                              ? range_start * amount
                              : std::numeric_limits<std::size_t>::max();
        }
    }
    range_start = std::min(range_start, matched);
    const std::size_t unbounded_end = std::numeric_limits<std::size_t>::max() - range_start < result.size()
                                          ? std::numeric_limits<std::size_t>::max()
    ...
    auto view = std::shared_ptr<const CollectionViewValue>(new CollectionViewValue{
        CollectionViewImmutableIdentity{
            Value(std::move(result)), total, matched, range_start, range_end,
            std::string(operation),
        },
    });
    if (dependencies.cacheable) {
        if (collection_cache_.size() >= 1024U) collection_cache_.erase(collection_cache_.begin());
...
std::shared_ptr<const ActionValue> ExpressionRuntime::composed_action(
    const JsonValue helper,
    const ExpressionScope& scope,
    ...
    for (const JsonValue argument_value : array_field(helper, "arguments")) {
        ...
        if (evaluated.action() == nullptr) {
            report(helper, "STRATA.DSL.RUNTIME_ACTION_COMPOSITION", "Action composition requires typed actions.");
            return std::make_shared<const ActionValue>(ActionValue{});
        }
        children.push_back(*evaluated.action());
    }
    if (children.empty()) {
        report(helper, "STRATA.DSL.RUNTIME_ACTION_COMPOSITION", "Action composition must not be empty.");
        return std::make_shared<const ActionValue>(ActionValue{});
    }
    const std::string id = mode == ActionCompositionMode::sequence ? "action.sequence" : "action.parallel";
    const auto contract = actions_.contract(id);
    if (contract == nullptr) {
        report(helper, "STRATA.DSL.RUNTIME_UNKNOWN_ACTION", "Framework composition action is not registered.");
        return std::make_shared<const ActionValue>(ActionValue{});
    }
...
void ExpressionRuntime::report(
    ...
    const std::string diagnostic_path = active_scope_ != nullptr &&
                                        !active_scope_->component_path.empty()
        ? active_scope_->component_path
        : path.has_value() ? std::string(*path) : std::string{};
    report(RuntimeDiagnostic{
        std::move(code),
        std::move(message),
        diagnostic_path,
...
void ExpressionRuntime::report(RuntimeDiagnostic diagnostic) {
    ...
    fingerprint.push_back('\0');
    if (diagnostic.expected.has_value()) fingerprint += *diagnostic.expected;
    if (!reported_diagnostics_.insert(std::move(fingerprint)).second) return;
    diagnostics_.push_back(std::move(diagnostic));
}

bool truthy(const Value& value) noexcept {
    switch (value.kind()) {
    case ValueKind::null_value: return false;
    case ValueKind::boolean: return *value.boolean();
    case ValueKind::number: return *value.number() != 0.0;
    case ValueKind::duration: return value.duration()->nanoseconds != 0;
    case ValueKind::string: return !value.string()->empty();
...
std::string display_string(const Value& value) {
    switch (value.kind()) {
    case ValueKind::null_value: return {};
    ...
    case ValueKind::duration: return std::to_string(value.duration()->nanoseconds);
    case ValueKind::string: return *value.string();
    case ValueKind::color: {
