...
/** Strict UTF-8 plus the XML 1.0 character domain; SVG resource text never uses replacement. */
[[nodiscard]] bool valid_xml_text(const std::string_view source) noexcept {
    std::size_t offset = 0U;
    while (offset < source.size()) {
        ...
        if (continuation_count > source.size() - offset - 1U)
            return false;
        for (std::size_t continuation = 0U; continuation < continuation_count; ++continuation) {
            const auto next = static_cast<std::uint8_t>(source[offset + continuation + 1U]);
            if ((next & 0xC0U) != 0x80U)
                return false;
            codepoint = codepoint << 6U | (next & 0x3FU);
        }
        if ((continuation_count == 2U && codepoint < 0x800U) ||
            (continuation_count == 3U && codepoint < 0x10000U) || codepoint > 0x10FFFFU ||
            (codepoint >= 0xD800U && codepoint <= 0xDFFFU)) {
            return false;
        }
        if (codepoint != 0x09U && codepoint != 0x0AU && codepoint != 0x0DU && codepoint < 0x20U) {
            return false;
        }
...
class XmlParser final {
  public:
    ...
    [[nodiscard]] XmlNode document() {
        ...
        if (!valid_xml_text(source_))
            fail(0U, "SVG is not valid UTF-8 XML text");
...
[[nodiscard]] AffineTransform multiply(const AffineTransform& left, const AffineTransform& right) {
    const AffineTransform result{
    ...
    if (!std::isfinite(result.a) || !std::isfinite(result.b) || !std::isfinite(result.c) ||
        !std::isfinite(result.d) || !std::isfinite(result.e) || !std::isfinite(result.f)) {
        throw ParseError(0U, "SVG transform arithmetic overflow");
    }
    return result;
}
...
[[nodiscard]] AffineTransform parse_transform(const Attribute& attribute) {
    ...
    while (true) {
        ...
        if (name == "matrix" && values.size() == 6U) {
            ...
            const double radians = std::fmod(values[0], 360.0) * std::numbers::pi / 180.0;
            ...
            const double radians = std::fmod(values[0], 180.0) * std::numbers::pi / 180.0;
...
[[nodiscard]] PaintStyle parse_paint(const XmlNode& node, PaintStyle style,
    ...
    if (const Attribute* const value = find_attribute(node, "fill")) {
        const std::string keyword = lowercase(trim(value->value));
        if (keyword == "none") {
            style.has_fill = false;
            style.fill_uses_current_color = false;
        } else {
            style.has_fill = true;
            style.fill_uses_current_color = keyword == "currentcolor";
    ...
    if (const Attribute* const value = find_attribute(node, "stroke")) {
        ...
        if (keyword == "none") {
            ...
            style.stroke_uses_current_color = false;
        } else {
            style.has_stroke = true;
            style.stroke_uses_current_color = keyword == "currentcolor";
    ...
    if (const Attribute* const value = find_attribute(node, "opacity")) {
        ...
        if (style.opacity > 0.0 && style.opacity < 1.0 && style.has_fill && style.has_stroke) {
            fail(*value, "fill-and-stroke element opacity requires isolated compositing and is not "
                         "in the static subset");
        }
    } else {
        style.opacity = 1.0;
    }
...
class PathBuilder final {
  ...
  private:
    void add(PathSegment segment) {
        const auto finite = [](const Point point) noexcept {
            return std::isfinite(point.x) && std::isfinite(point.y);
        };
        if (!finite(segment.control_a) || !finite(segment.control_b) || !finite(segment.to)) {
            throw ParseError(0U, "SVG path arithmetic overflow");
        }
...
void append_arc(PathBuilder& builder, const Point from, Point radii, const double rotation_degrees,
    ...
    const auto checked = [](const double value) {
        if (!std::isfinite(value))
            throw ParseError(0U, "SVG arc arithmetic overflow");
        return value;
    };
    const auto checked_point = [&checked](const Point point) {
        checked(point.x);
        checked(point.y);
        return point;
    };
    ...
    const Point half_delta =
        checked_point(Point{from.x * 0.5 - to.x * 0.5, from.y * 0.5 - to.y * 0.5});
    const Point prime = checked_point(Point{
        cosine * half_delta.x + sine * half_delta.y,
        -sine * half_delta.x + cosine * half_delta.y,
    });
    double radius_x_squared = checked(radii.x * radii.x);
    double radius_y_squared = checked(radii.y * radii.y);
    const double prime_x_squared = checked(prime.x * prime.x);
    const double prime_y_squared = checked(prime.y * prime.y);
    const double correction =
        checked(prime_x_squared / radius_x_squared + prime_y_squared / radius_y_squared);
    if (correction > 1.0) {
        const double factor = checked(std::sqrt(correction));
        radii = checked_point(scale(radii, factor));
        radius_x_squared = checked(radii.x * radii.x);
        radius_y_squared = checked(radii.y * radii.y);
    }
    const double first_denominator_term = checked(radius_x_squared * prime_y_squared);
    const double second_denominator_term = checked(radius_y_squared * prime_x_squared);
    const double denominator = checked(first_denominator_term + second_denominator_term);
    const double radius_product = checked(radius_x_squared * radius_y_squared);
    const double numerator = checked(
        std::max(0.0, checked(radius_product - first_denominator_term - second_denominator_term)));
    const double sign = large_arc == sweep ? -1.0 : 1.0;
    const double coefficient =
        denominator == 0.0 ? 0.0 : checked(sign * std::sqrt(checked(numerator / denominator)));
    const Point center_prime = checked_point(Point{
        coefficient * radii.x * prime.y / radii.y,
        coefficient * -radii.y * prime.x / radii.x,
    });
    const Point midpoint =
        checked_point(Point{from.x * 0.5 + to.x * 0.5, from.y * 0.5 + to.y * 0.5});
    const Point center = checked_point(Point{
        cosine * center_prime.x - sine * center_prime.y + midpoint.x,
        sine * center_prime.x + cosine * center_prime.y + midpoint.y,
    });
    const Point start_vector = checked_point(Point{
        (prime.x - center_prime.x) / radii.x,
        (prime.y - center_prime.y) / radii.y,
    });
    const Point end_vector = checked_point(Point{
        (-prime.x - center_prime.x) / radii.x,
        (-prime.y - center_prime.y) / radii.y,
    });
    double start_angle = checked(vector_angle(start_vector));
    double delta = checked(vector_angle(end_vector) - start_angle);
    if (!sweep && delta > 0.0)
        delta = checked(delta - 2.0 * std::numbers::pi);
    if (sweep && delta < 0.0)
        delta = checked(delta + 2.0 * std::numbers::pi);
    const double piece_count = checked(std::ceil(std::abs(delta) / (std::numbers::pi * 0.5)));
    if (piece_count <= 0.0) {
        builder.line(to);
        return;
    }
    if (piece_count > 4.0)
        throw ParseError(0U, "SVG arc subdivision overflow");
    const std::size_t pieces = static_cast<std::size_t>(piece_count);
    const double piece_delta = checked(delta / static_cast<double>(pieces));

    const auto point_at = [&](const double angle) -> Point {
        return checked_point(Point{
    ...
    for (std::size_t piece = 0U; piece < pieces; ++piece) {
        const double end_angle = checked(start_angle + piece_delta);
        const double alpha = checked(4.0 / 3.0 * std::tan(piece_delta * 0.25));
...
void validate_transformed_geometry(const XmlNode& node, const DrawCommand& command) {
    double minimum_x = std::numeric_limits<double>::infinity();
    double minimum_y = std::numeric_limits<double>::infinity();
    double maximum_x = -std::numeric_limits<double>::infinity();
    double maximum_y = -std::numeric_limits<double>::infinity();
    const auto validate_point = [&](const Point point) {
        minimum_x = std::min(minimum_x, point.x);
        minimum_y = std::min(minimum_y, point.y);
        maximum_x = std::max(maximum_x, point.x);
        maximum_y = std::max(maximum_y, point.y);
        const Point transformed = command.transform.apply(point);
        if (!std::isfinite(transformed.x) || !std::isfinite(transformed.y)) {
            fail(node, "SVG geometry and transform produce non-finite coordinates");
        }
    };
    for (const PathSegment& segment : command.path.segments) {
        validate_point(segment.to);
        if (segment.verb == PathVerb::cubic) {
            validate_point(segment.control_a);
            validate_point(segment.control_b);
        }
    }
    double stroke_expansion = 0.0;
    if (command.paint.has_stroke && command.paint.stroke_width > 0.0) {
        stroke_expansion = command.paint.stroke_width * 0.5 *
            (command.paint.line_join == LineJoin::miter ? command.paint.miter_limit : 1.0);
        const double scale_bound = std::max(std::hypot(command.transform.a, command.transform.b),
                                            std::hypot(command.transform.c, command.transform.d));
        if (!std::isfinite(stroke_expansion) ||
            !std::isfinite(command.paint.stroke_width * scale_bound) ||
            !std::isfinite(stroke_expansion * scale_bound)) {
            fail(node, "SVG stroke arithmetic exceeds the finite geometry range");
        }
    }
    ...
    if (!std::isfinite(minimum_x) || !std::isfinite(minimum_y) || !std::isfinite(maximum_x) ||
        !std::isfinite(maximum_y) || !std::isfinite(maximum_x - minimum_x) ||
        !std::isfinite(maximum_y - minimum_y)) {
        fail(node, "SVG geometry bounds exceed the finite coordinate range");
    }
    if (maximum_x - minimum_x <= 1.0e-12) {
        minimum_x -= 0.5;
        maximum_x += 0.5;
        if (!std::isfinite(minimum_x) || !std::isfinite(maximum_x)) {
            fail(node, "SVG degenerate horizontal bounds exceed the finite coordinate range");
        }
    }
    if (maximum_y - minimum_y <= 1.0e-12) {
        ...
        if (!std::isfinite(minimum_y) || !std::isfinite(maximum_y)) {
            fail(node, "SVG degenerate vertical bounds exceed the finite coordinate range");
        }
    }
    for (const Point corner : {Point{minimum_x, minimum_y}, Point{maximum_x, minimum_y},
                               Point{maximum_x, maximum_y}, Point{minimum_x, maximum_y}}) {
        const Point transformed = command.transform.apply(corner);
        if (!std::isfinite(transformed.x) || !std::isfinite(transformed.y)) {
            fail(node, "SVG transformed paint bounds exceed the finite coordinate range");
        }
    }
}
...
void visit(const XmlNode& node, const AffineTransform& parent_transform,
    ...
    validate_transformed_geometry(node, command);
...
Document parse(const std::string_view source, const ParseOptions& options) {
    ...
    if (view_box != nullptr) {
        ...
        if (!std::isfinite(document.view_box.minimum_x + document.view_box.width) ||
            !std::isfinite(document.view_box.minimum_y + document.view_box.height)) {
            fail(*view_box, "SVG viewBox exceeds the finite coordinate range");
        }
    }
