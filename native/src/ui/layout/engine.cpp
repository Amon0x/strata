...
[[nodiscard]] const RetainedNode* anchored_node(const RetainedTree& tree,
...
[[nodiscard]] const RetainedNode* ordinary_anchor_node(const RetainedTree& tree,
    ...
    return candidate_style.participates && candidate_style.kind != LayoutKind::portal ? candidate
                                                                                      : nullptr;
}

[[nodiscard]] Rect anchor_placement(const Rect anchor, const Rect viewport, const Size size,
                                    const LayoutStyle& style) noexcept {
    const auto aligned = [alignment = style.anchor_align](const double start, const double extent,
    ...
    if (style.anchor_flip) {
        if (side == LayoutAnchorSide::bottom) {
            ...
            if (size.height > following && preceding > following)
                side = LayoutAnchorSide::top;
        } else if (side == LayoutAnchorSide::top) {
            const double preceding = anchor.y - viewport.y - style.anchor_gap;
            const double following = viewport.bottom() - anchor.bottom() - style.anchor_gap;
            if (size.height > preceding && following > preceding)
                side = LayoutAnchorSide::bottom;
            ...
            if (size.width > following && preceding > following)
                side = LayoutAnchorSide::left;
        } else {
            const double preceding = anchor.x - viewport.x - style.anchor_gap;
            const double following = viewport.right() - anchor.right() - style.anchor_gap;
            if (size.width > preceding && following > preceding)
                side = LayoutAnchorSide::right;
        }
    }
...
LayoutEngine::LayoutEngine(IntrinsicMeasure intrinsic_measure)
    : intrinsic_measure_(intrinsic_measure ? std::move(intrinsic_measure)
                                           : IntrinsicMeasure(default_intrinsic)) {}

const LayoutResult& LayoutEngine::result() const noexcept {
    return result_;
}
...
bool LayoutEngine::requires_layout(const RetainedTree& tree,
...
const LayoutResult& LayoutEngine::layout(RetainedTree& tree, const LayoutEnvironment& environment,
    ...
    if (root == nullptr)
        throw std::invalid_argument("layout requires a retained root");
    ...
    if (content_size_transitions_.active_count() != 0U) {
        ...
        for (const std::uint64_t identity : content_size_transitions_.active_identities()) {
            for (const RetainedNode* node = tree.find_identity(identity); node != nullptr;
                 node = node->parent()) {
    ...
    const auto schedule_measurement = [&pending_measurements, &scheduled_measurements,
        ...
        pending_measurements.emplace(std::pair(depth, node->identity()), node);
    };
    for (RetainedNode* dirty : tree.dirty_nodes()) {
        const DirtySet& reasons = dirty->dirty();
        if (!reasons.contains(DirtyReason::structure) && !reasons.contains(DirtyReason::layout) &&
            !reasons.contains(DirtyReason::text) && !reasons.contains(DirtyReason::style) &&
            !reasons.contains(DirtyReason::scale) && !reasons.contains(DirtyReason::editor)) {
            continue;
        }
    ...
    while (!pending_measurements.empty()) {
        ...
        const Constraints constraints =
            current == root ? Constraints::fixed(root_bounds.width, root_bounds.height)
    ...
    if (measured == nullptr) {
        ...
        if (cached_root == measurement_cache_.end() || cached_root->second.measured == nullptr) {
            measured = measure(*root, Constraints::fixed(root_bounds.width, root_bounds.height),
                               environment, next.operations);
        } else {
            measured = cached_root->second.propagated != nullptr ? cached_root->second.propagated
    ...
    if (!measurement_frontier_.empty()) {
        ...
        for (const auto& [identity, frontier] : measurement_frontier_) {
            ...
            std::size_t depth = 0U;
            for (const RetainedNode* current = frontier->node; current != nullptr;
                 current = current->parent()) {
                ++depth;
            }
            for (RetainedNode* ancestor = frontier->node->parent(); ancestor != nullptr;
                 ancestor = ancestor->parent()) {
                --depth;
                affected.insert_or_assign(ancestor->identity(), AffectedAncestor{ancestor, depth});
            }
        }
        ...
        std::ranges::sort(ordered, std::greater{}, &AffectedAncestor::depth);
        for (const AffectedAncestor& ancestor : ordered) {
            auto cached = measurement_cache_.find(ancestor.node->identity());
            if (cached == measurement_cache_.end() || cached->second.measured == nullptr) {
                continue;
            }
        ...
        std::erase_if(measurement_frontier_, [this](const auto& entry) {
            for (const RetainedNode* ancestor = entry.second->node->parent(); ancestor != nullptr;
                 ancestor = ancestor->parent()) {
                if (measurement_frontier_.contains(ancestor->identity()))
                    return true;
            }
            return false;
        });
    ...
    if (const auto root_frontier = measurement_frontier_.find(root->identity());
        ...
        arrange(current_root != measurement_cache_.end() && current_root->second.measured != nullptr
                    ? current_root->second.measured
                    : measured,
                root_bounds, std::nullopt, {}, environment, next);
    } else {
    ...
    for (const auto& [identity, frontier] : measurement_frontier_) {
        ...
        if (retained == arrangement_cache_.end()) {
            throw std::logic_error("incremental measurement frontier lost its arrangement cache");
        }
    ...
    while (!pending_anchors_.empty()) {
        ...
        for (std::size_t index = 0U; index < pending_anchors_.size(); ++index) {
            ...
            const RetainedNode* target = ordinary_anchor_node(
                tree, *pending.measured->node, pending.measured->style.anchor_target);
            if (target == nullptr) {
                ready = index;
                missing_target = true;
                break;
        ...
        if (!ready.has_value()) {
            ...
            for (PendingAnchor& pending : unresolved) {
                const std::string fingerprint = pending.measured->node->description().source_path +
                ...
                arrange(pending.measured, pending.fallback_bounds, pending.inherited_clip,
                        pending.pin_context, environment, next);
            }
            continue;
        }
        PendingAnchor pending = std::move(pending_anchors_[*ready]);
        pending_anchors_.erase(pending_anchors_.begin() + static_cast<std::ptrdiff_t>(*ready));
        const LayoutStyle& style = pending.measured->style;
        const RetainedNode* target =
            ordinary_anchor_node(tree, *pending.measured->node, style.anchor_target);
        const LayoutRecord* anchor_record = target != nullptr && arranged_in_current_pass(*target)
        ...
        if (style.match_anchor_width &&
            ...
            pending.measured = measure(*pending.measured->node,
                                       Constraints{
        ...
        arrange(pending.measured,
                anchor_placement(anchor_record->bounds, pending.containing_bounds,
                                 pending.measured->measured_size, style),
                pending.inherited_clip, pending.pin_context, environment, next);
    }
    for (std::size_t index = 0U; index < pending_portals_.size(); ++index) {
        PendingPortal pending = pending_portals_[index];
        const LayoutStyle& style = pending.measured->style;
        const RetainedNode* anchor_node =
            anchored_node(tree, *pending.measured->node, style.anchor_target);
        const LayoutRecord* anchor_record =
            anchor_node != nullptr && arranged_in_current_pass(*anchor_node)
        ...
        const LayoutRecord* parent_record =
            parent != nullptr ? next.find(parent->identity()) : nullptr;
        ...
        if (anchor_record != nullptr || point_anchor.has_value()) {
            if (style.match_anchor_width && anchor_record != nullptr &&
                anchor_record->bounds.width != pending.measured->measured_size.width) {
                pending.measured = measure(*pending.measured->node,
                                           Constraints{
                                               anchor_record->bounds.width,
            ...
            const Rect anchor_bounds =
                point_anchor.has_value() ? *point_anchor : anchor_record->bounds;
            portal_bounds =
                anchor_placement(anchor_bounds, viewport, pending.measured->measured_size, style);
        }
        arrange(pending.measured, portal_bounds, pending.inherited_clip, pending.pin_context,
                environment, next);
    }
    pending_portals_.clear();
    ...
    const auto record_unarranged = [&](const auto& self, const RetainedNode& node) -> void {
        if (!arranged_in_current_pass(node)) {
            ...
            record.portal_target =
                style.kind == LayoutKind::portal ? style.portal_target : std::string{};
            record.detached_from_parent_clip =
                style.kind == LayoutKind::portal && style.detach_from_parent_clip;
    ...
    for (const collection::VirtualAnchorUpdate& update :
        ...
        static_cast<void>(tree.set_arrangement_value(
            update.identity, "strata.scroll.offset",
    ...
    if (consume_dirty)
        tree.consume_layout_dirty();
    std::erase_if(next.records, [&tree](const auto& entry) {
        return tree.find_identity(entry.first) == nullptr;
    });
    std::erase_if(measurement_cache_,
                  [&next](const auto& entry) { return !next.records.contains(entry.first); });
    std::erase_if(arrangement_cache_,
                  [&next](const auto& entry) { return !next.records.contains(entry.first); });
...
bool LayoutEngine::arranged_in_current_pass(const RetainedNode& node) const noexcept {
    if (current_arranged_records_.contains(node.identity()))
        return true;
    for (const RetainedNode* ancestor = &node; ancestor != nullptr; ancestor = ancestor->parent()) {
        if (current_arranged_subtree_roots_.contains(ancestor->identity()))
            return true;
    }
    return false;
}
...
LayoutStyle LayoutEngine::resolved_style(const RetainedNode& node) const {
    ...
    const MotionComputedValues* values =
        motion_ != nullptr ? motion_->computed_values(node.identity()) : nullptr;
    if (values == nullptr)
        return style;
    const auto animated_size = [values](const MotionProperty property, LayoutSize& size) {
        if (const MotionLayoutValue* value = values->layout(property); value != nullptr) {
            const LayoutSize::Kind kind =
                value->unit == MotionLayoutUnit::percent ? LayoutSize::Kind::percent
                : value->unit == MotionLayoutUnit::fill  ? LayoutSize::Kind::fill
    ...
    const auto optional_fixed = [values](const MotionProperty property,
    ...
    const auto placement = [values](const MotionProperty property,
    ...
    if (style.placement.has_value() || values->find(MotionProperty::placement_x) != nullptr ||
        values->find(MotionProperty::placement_y) != nullptr) {
        if (!style.placement.has_value())
            style.placement.emplace();
...
Point LayoutEngine::resolved_scroll_offset(const RetainedNode& node, Point fallback) noexcept {
