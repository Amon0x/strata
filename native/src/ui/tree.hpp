...
struct MaterializationRange final {
    ...
    [[nodiscard]] friend bool operator==(const MaterializationRange&, const MaterializationRange&) = default;
};

/** One generated row build, published by Surface after that row is materialized. */
struct DescriptionMaterialization final {
    DescriptionMaterialization(
...
public:
    struct Node final {
        ...
        [[nodiscard]] const runtime::Value* retained_value(
...
public:
     ...
     */
    [[nodiscard]] std::shared_ptr<const DescriptionNode> realized_child_description(
    ...
    [[nodiscard]] bool realization_current(
        const std::shared_ptr<const DescriptionChildren>& provider,
        const std::shared_ptr<const Theme>& projected_theme,
        const std::optional<std::string>& projected_theme_scope,
        std::uint64_t theme_generation,
...
public:
    ...
    using PersistenceWriter = std::function<void(
        std::string_view key,
        std::string_view field,
    ...
    [[nodiscard]] ReconcileStats reconcile(
    ...
    [[nodiscard]] const std::vector<RetainedNode*>* find_state_scope(std::string_view scope) const noexcept;
    ...
    [[nodiscard]] std::shared_ptr<const RetainedDescriptionSnapshot> description_snapshot(
    ) const;
    ...
    [[nodiscard]] bool set_retained_value(
        std::uint64_t identity,
        std::string name,
        runtime::Value value,
    ...
    [[nodiscard]] bool set_presentation_value(
        std::uint64_t identity,
        std::string name,
        runtime::Value value
    );
...
private:
    [[nodiscard]] std::unique_ptr<RetainedNode> reconcile_node(
    ...
    void persist_retained_value(
        const RetainedNode& node,
        std::string_view name,
        const runtime::Value& value
    );
    [[nodiscard]] bool field_persisted(
        std::string_view type,
        std::string_view name
    ) const;
...
[[nodiscard]] runtime::StateScopeSet attached_description_state_scopes(
    const RetainedTree& tree
);

[[nodiscard]] bool expression_value_equal(
