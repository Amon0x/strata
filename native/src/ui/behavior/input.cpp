...
BehaviorInputScope::BehaviorInputScope(InputDispatchContext& dispatch,
                                       const DescriptionBehavior& attachment,
                                       InputOperationResult& result) noexcept
    : router_(dispatch.router_), node_(dispatch.node()), attachment_(attachment),
      ...
      phase_(dispatch.phase()), target_(&dispatch.node() == dispatch.target()),
      pointer_target_(dispatch.pointer_target()), result_(result), dispatch_(&dispatch) {
    ++router_.behavior_dispatch_count_;
}

BehaviorInputScope::BehaviorInputScope(InputRouter& router, RetainedNode& node,
                                       const DescriptionBehavior& attachment,
                                       const PointerInputEvent& event,
                                       const BehaviorInputEventPhase phase, const bool target,
                                       RetainedNode* const pointer_target,
                                       InputOperationResult& result) noexcept
    : router_(router), node_(node), attachment_(attachment), event_(&event), phase_(phase),
      target_(target), pointer_target_(pointer_target), result_(result) {
    ++router_.behavior_dispatch_count_;
}
...
BehaviorInputScope::BehaviorInputScope(InputRouter& router, RetainedNode& node,
                                       ...
                                       const BehaviorInputEventPhase phase, const bool target,
                                       InputOperationResult& result) noexcept
    : router_(router), node_(node), attachment_(attachment), key_(key), modifiers_(modifiers),
      phase_(phase), target_(target), result_(result) {
    ++router_.behavior_dispatch_count_;
}
...
BehaviorInputScope::BehaviorInputScope(InputRouter& router, RetainedNode& node,
                                       ...
                                       InputOperationResult& result) noexcept
    : router_(router), node_(node), attachment_(attachment), phase_(phase), result_(result) {
    ++router_.behavior_dispatch_count_;
}

RetainedNode& BehaviorInputScope::node() noexcept {
    return node_;
}
const RetainedNode& BehaviorInputScope::node() const noexcept {
    return node_;
}
const DescriptionBehavior& BehaviorInputScope::attachment() const noexcept {
    return attachment_;
}
const PointerInputEvent& BehaviorInputScope::event() const noexcept {
    return *event_;
}
const PointerInputEvent* BehaviorInputScope::pointer() const noexcept {
    return event_;
}
std::string_view BehaviorInputScope::key() const noexcept {
    return key_;
}
const KeyModifiers& BehaviorInputScope::modifiers() const noexcept {
    return event_ != nullptr ? event_->modifiers : modifiers_;
}
BehaviorInputEventPhase BehaviorInputScope::phase() const noexcept {
    return phase_;
}
bool BehaviorInputScope::target() const noexcept {
    return target_;
}
InputDispatchContext* BehaviorInputScope::dispatch() const noexcept {
    return dispatch_;
}

WidgetInputScope BehaviorInputScope::widget_scope() const noexcept {
    return WidgetInputScope(router_, node_, result_, {},
                            event_ != nullptr ? event_->modifiers : KeyModifiers{}, event_,
                            pointer_target_, 0U, std::nullopt, {}, dispatch_);
}
...
bool BehaviorInputScope::press_matches(const bool include_descendants) const noexcept {
    ...
    if (found == router_.pressed_pointer_targets_.end() || router_.tree_ == nullptr)
        return false;
    const RetainedNode* pressed = router_.tree_->find_identity(found->second.identity);
    const bool release_inside =
        pointer_target_ == &node_ ||
        (pointer_target_ != nullptr &&
         (include_descendants ? InputRouter::descendant_of(*pointer_target_, node_)
                              : router_.passive_pointer_path(node_, pointer_target_)));
    const bool selection_dragged = found->second.moved_beyond_slop && pressed != nullptr &&
                                   (router_.static_text_selectable(*pressed) ||
                                    router_.editors_.contains(pressed->identity()));
    return release_inside && !selection_dragged && pressed != nullptr &&
           (pressed == &node_ ||
...
GestureClaimState BehaviorInputScope::gesture_claim_state() const noexcept {
    if (dispatch_ != nullptr)
        return dispatch_->gesture_claim_state();
    if (router_.tree_ == nullptr)
...
void BehaviorInputScope::mark_long_press_emitted() {
    for (auto& [pointer_id, press] : router_.pressed_pointer_targets_) {
        if (event_ != nullptr && pointer_id != event_->pointer_id)
            continue;
        if (router_.tree_ == nullptr)
            continue;
...
void BehaviorInputScope::set_retained(std::string name, runtime::Value value,
                                      const DirtyReason reason) {
    set_retained(node_, std::move(name), std::move(value), reason);
}

void BehaviorInputScope::set_retained(RetainedNode& node, std::string name, runtime::Value value,
                                      const DirtyReason reason) {
    if (router_.tree_ == nullptr)
        return;
    const bool changed = router_.tree_->set_retained_value(node.identity(), std::move(name),
...
bool BehaviorInputScope::emit(const std::string_view event_kind, runtime::Value event_value) {
...
bool BehaviorInputScope::emit_action(const std::string_view action_id, runtime::Value payload,
