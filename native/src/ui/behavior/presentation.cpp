...
[[nodiscard]] bool hovered(const RetainedNode& node, const DescriptionBehavior&,
...
void hover_overlay(const DescriptionBehavior&, WidgetRenderScope& scope) {
    if (scope.visual().hover_overlay.has_value())
        scope.rounded_rect(scope.layout().bounds, *scope.visual().hover_overlay);
}

[[nodiscard]] double option_number(const runtime::Value* options, const std::string_view name,
...
[[nodiscard]] bool drag_preview_active(const RetainedNode& node, const DescriptionBehavior&,
                                       const InputRouter& input) {
...
void drag_preview_overlay(const DescriptionBehavior& attachment, WidgetRenderScope& scope) {
    ...
    const double opacity = std::clamp(option_number(preview_options, "opacity", 0.82), 0.0, 1.0);
    ...
    const RenderColor accent =
        state->accepted ? RenderColor{91U, 226U, 154U, 255U} : RenderColor{239U, 103U, 112U, 255U};
    ...
    if (state->commands != nullptr) {
        for (const RenderCommand& command : *state->commands)
            scope.append(command, opacity);
    }
    scope.pop_clip();
    scope.pop_transform();
    if (state->commands == nullptr || state->commands->empty()) {
        scope.rounded_rect(preview,
    ...
    scope.rounded_rect(badge, accent, RenderBorder{1.0, RenderColor{245U, 249U, 255U, 235U}, true},
                       6.0);
    if (!state->accepted) {
        scope.solid_rect(Rect{badge.x + 3.0, badge.y + 5.0, 6.0, 2.0},
                         RenderColor{255U, 255U, 255U, 235U});
    }
}

[[nodiscard]] bool drop_target_active(const RetainedNode& node, const DescriptionBehavior&,
                                      const InputRouter& input) {
...
void drop_target_overlay(const DescriptionBehavior& attachment, WidgetRenderScope& scope) {
    ...
    if (horizontal) {
        ...
        scope.rounded_rect(
            Rect{before ? bounds.x : bounds.right() - half, bounds.y, half, bounds.height}, tint,
            std::nullopt, 4.0);
        ...
        scope.rounded_rect(Rect{x - 5.0, line.bottom() - 3.0, 10.0, 3.0}, color, std::nullopt, 1.5);
        return;
    }
    const bool before = state->placement == "before";
    ...
    scope.rounded_rect(
        Rect{bounds.x, before ? bounds.y : bounds.bottom() - half, bounds.width, half}, tint,
        std::nullopt, 4.0);
    ...
    scope.rounded_rect(Rect{line.right() - 3.0, y - 5.0, 3.0, 10.0}, color, std::nullopt, 1.5);
}
...
void reorder_target_overlay(const DescriptionBehavior& attachment, WidgetRenderScope& scope) {
    ...
    const std::string axis = axis_value != nullptr && axis_value->string() != nullptr
                                 ...
                                 : "VERTICAL";
    const bool horizontal = axis == "HORIZONTAL" || axis == "horizontal";
    const ReorderInsertion resolved =
        resolve_reorder_insertion(scope.node(), scope.layout_result(), state->position, horizontal);
    const Rect bounds = scope.layout().bounds;
    const double insertion = resolved.coordinate;
    const RenderColor color{103U, 225U, 164U, 255U};
    scope.rounded_rect(
        horizontal ? Rect{insertion - 1.5, bounds.y + 2.0, 3.0, std::max(0.0, bounds.height - 4.0)}
...
void register_builtin_behavior_presenters(BehaviorRegistry& registry) {
    ...
    registry.register_present_phase(
        "strata.drop-target", BehaviorPresentPhase{drop_target_active, drop_target_overlay, true});
    registry.register_present_phase(
        "strata.reorder-target",
        BehaviorPresentPhase{drop_target_active, reorder_target_overlay, true});
}

bool has_behavior_overlay(const BehaviorRegistry& registry, const RetainedNode& node,
                          const InputRouter& input, const bool detached) {
    for (const DescriptionBehavior& attachment : node.description().behaviors) {
        ...
        if (lifecycle->present.has_overlay(node, attachment, input))
            return true;
    }
    return false;
}

void append_behavior_overlays(const BehaviorRegistry& registry, const RetainedNode& node,
