...
[[nodiscard]] const runtime::Value* field(const runtime::Value* value,
...
[[nodiscard]] std::int64_t integer(const runtime::Value* value,
...
[[nodiscard]] std::string text(const runtime::Value* value, std::string fallback = {}) {
...
long_press_duration(const BehaviorInputScope& scope) noexcept {
...
[[nodiscard]] bool activation_pointer(BehaviorInputScope& scope) {
    ...
    if (event == nullptr || !pointer_policy(scope))
        return false;
    ...
    const bool accepted_phase =
        scope.phase() == BehaviorInputEventPhase::target ||
                ...
                object({
                    ...
                    {"startedAt",
                     runtime::Value(static_cast<double>(
                         event->timestamp_nanos > 0 ? event->timestamp_nanos
    ...
    if (event->type != PointerEventType::release || duration.has_value() ||
        ...
        scope.long_press_emitted() || scope.gesture_claim_state() != GestureClaimState::unclaimed) {
        return false;
    }

    const std::int64_t now = event->timestamp_nanos > 0 ? event->timestamp_nanos
                                                        : scope.widget_scope().frame_time_nanos();
    ...
    const std::int64_t elapsed =
        now >= previous_time ? now - previous_time : std::numeric_limits<std::int64_t>::max();
    ...
    scope.set_retained(std::string(activate_click),
                       object({
                       ...
                       DirtyReason::input);
    const std::int64_t required_clicks =
        std::clamp<std::int64_t>(integer(scope.option("clickCount"), 1), 1, 3);
...
[[nodiscard]] bool activation_advance(BehaviorInputScope& scope) {
    ...
    if (started <= 0 || now < started || now - started < *duration)
        return false;
    scope.set_retained(std::string(activate_session),
                       object({
...
[[nodiscard]] Point retained_point(const runtime::Value* value,
                                   const Point fallback = {}) noexcept {
    return Point{number(field(value, "x"), fallback.x), number(field(value, "y"), fallback.y)};
}

[[nodiscard]] Point clamp_movement(BehaviorInputScope& scope, Point offset) noexcept {
    ...
    if (node_layout == nullptr || parent_layout == nullptr)
        return offset;
...
[[nodiscard]] bool movement_phase(const BehaviorInputScope& scope) {
    ...
    for (RetainedNode* current = widget.pointer_target();
         current != nullptr && current != &scope.node(); current = current->parent()) {
        if (current->description().key == handle_key)
            return true;
    }
    return false;
}
...
[[nodiscard]] bool movable_pointer(BehaviorInputScope& scope) {
    const PointerInputEvent* event = scope.pointer();
    if (event == nullptr || !boolean(scope.option("enabled"), true))
        return false;
    const runtime::Value* session = scope.retained(movement_session);
    if (event->type == PointerEventType::press) {
        if (!movement_phase(scope) || event->button != 0)
            return false;
    ...
    if (event->type == PointerEventType::move) {
        if (!scope.press_moved_beyond_slop())
            return false;
        if (!scope.claim_gesture())
            return false;
        Point next{
            number(field(session, "offsetX"), 0.0) + event->position.x -
                number(field(session, "startX"), event->position.x),
...
void set_runtime_size(BehaviorInputScope& scope, const double width, const double height) {
    ...
    static_cast<void>(scope.emit(
        "resized", object({{"width", runtime::Value(width)}, {"height", runtime::Value(height)}})));
}
...
[[nodiscard]] bool resize_pointer(BehaviorInputScope& scope) {
    ...
    if (event.type == PointerEventType::press) {
        ...
        scope.set_retained(
            ...
            object({
                ...
                {"height",
                 runtime::Value(number(field(previous, "height"), layout->bounds.height))},
                {"hadPrevious",
                 runtime::Value(previous != nullptr && previous->object() != nullptr)},
                {"previousWidth",
                 runtime::Value(number(field(previous, "width"), layout->bounds.width))},
                {"previousHeight",
                 runtime::Value(number(field(previous, "height"), layout->bounds.height))},
            }),
            DirtyReason::input);
    ...
    if (event.type == PointerEventType::move) {
        ...
        const double width =
            spec.horizontal ? std::clamp(number(field(session, "width"), 0.0) + event.position.x -
                                             number(field(session, "startX"), event.position.x),
                                         spec.min_width, spec.max_width)
                            : number(field(session, "width"), 0.0);
        const double height =
            spec.vertical ? std::clamp(number(field(session, "height"), 0.0) + event.position.y -
                                           number(field(session, "startY"), event.position.y),
    ...
    if (event.type == PointerEventType::cancel) {
        ...
        if (boolean(field(session, "hadPrevious"), false)) {
            set_runtime_size(scope, number(field(session, "previousWidth"), 0.0),
...
[[nodiscard]] bool resize_key(BehaviorInputScope& scope) {
    if (scope.phase() != BehaviorInputEventPhase::target)
        return false;
    if (scope.key() == "escape") {
        if (scope.retained(resize_session) == nullptr)
            return false;
        scope.set_retained(std::string(resize_session), runtime::Value{}, DirtyReason::input);
        return true;
    }
    ...
    if (spec.horizontal && scope.key() == "left")
        width -= 4.0;
    else if (spec.horizontal && scope.key() == "right")
        width += 4.0;
    else if (spec.vertical && scope.key() == "up")
        height -= 4.0;
    else if (spec.vertical && scope.key() == "down")
        height += 4.0;
    else
        return false;
...
[[nodiscard]] bool split_vertical(const RetainedNode& pane) noexcept {
    ...
    const runtime::Value* value =
        found != pane.description().properties.end() ? found->second.value() : nullptr;
    return normalized(text(value, "horizontal")) == "vertical";
}

[[nodiscard]] double pane_number(const RetainedNode& pane, const std::string_view name,
                                 const double fallback) noexcept {
    const auto found = pane.description().properties.find(name);
    return number(found != pane.description().properties.end() ? found->second.value() : nullptr,
                  fallback);
}
...
[[nodiscard]] double current_split_ratio(const RetainedNode& pane) noexcept {
    ...
    if (controlled != pane.description().properties.end() &&
...
void change_split(BehaviorInputScope& scope, RetainedNode& pane, double value) {
    ...
    const bool is_controlled = controlled != pane.description().properties.end() &&
                               controlled->second.value() != nullptr &&
                               controlled->second.value()->number() != nullptr;
    if (!is_controlled) {
        scope.set_retained(pane, std::string(split_ratio), runtime::Value(value),
                           DirtyReason::layout);
    }
...
[[nodiscard]] bool split_pointer(BehaviorInputScope& scope) {
    if (scope.phase() != BehaviorInputEventPhase::target || scope.pointer() == nullptr)
        return false;
    ...
    if (event.type == PointerEventType::press) {
        if (event.button != 0)
            return false;
        scope.set_retained(std::string(split_session),
                           object({
                               {"pointerId", runtime::Value(static_cast<double>(event.pointer_id))},
                               {"startX", runtime::Value(event.position.x)},
                               {"startY", runtime::Value(event.position.y)},
    ...
    if (event.type == PointerEventType::move) {
        ...
        if (extent <= 0.0)
            return true;
        const double delta =
            vertical ? event.position.y - number(field(session, "startY"), event.position.y)
...
[[nodiscard]] bool split_key(BehaviorInputScope& scope) {
    ...
    if (!vertical && scope.key() == "left")
        direction = -1.0;
    else if (!vertical && scope.key() == "right")
        direction = 1.0;
    else if (vertical && scope.key() == "up")
        direction = -1.0;
    else if (vertical && scope.key() == "down")
        direction = 1.0;
    else
        return false;
...
void register_builtin_behavior_inputs(BehaviorRegistry& registry) {
    registry.register_input_phase("strata.activate", BehaviorInputPhase{
    ...
    registry.register_input_phase("strata.resize", BehaviorInputPhase{
