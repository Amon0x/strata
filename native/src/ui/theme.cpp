...
[[nodiscard]] runtime::Value nullable_color(const std::optional<runtime::ColorValue>& value) {
...
void add_text_layout(ValueFields& fields, const ThemeTextLayoutStyle& style) {
    ...
    fields.insert_or_assign("fontStyleFlags",
                            runtime::Value(static_cast<double>(style.style_flags)));
...
[[nodiscard]] runtime::Value size(const Size& value) {
    return object(
        {{"height", runtime::Value(value.height)}, {"width", runtime::Value(value.width)}});
}
...
[[nodiscard]] runtime::Value layout_size_value(const LayoutSize& value) {
    ...
    case LayoutSize::Kind::clamp:
        return object({
            {"max",
             value.maximum != nullptr ? layout_size_value(*value.maximum) : runtime::Value{}},
            {"min",
             value.minimum != nullptr ? layout_size_value(*value.minimum) : runtime::Value{}},
            {"preferred", value.preferred != nullptr ? layout_size_value(*value.preferred)
                                                     : runtime::Value("auto")},
        });
    }
    return runtime::Value("auto");
...
[[nodiscard]] std::string_view serialized_layout_kind_name(const LayoutKind value) noexcept {
    ...
    case LayoutKind::grid:
        return "GRID";
    case LayoutKind::panel:
        return "PANEL";
    case LayoutKind::overlay:
        return "OVERLAY";
    case LayoutKind::spacer:
        return "SPACER";
    case LayoutKind::scroll:
        return "SCROLL";
    case LayoutKind::portal:
        return "PORTAL";
    }
    return "PANEL";
}
...
void add_layout(ValueFields& fields, const ThemeWidgetLayoutStyle& style) {
    ...
    fields.insert_or_assign("intrinsicSize", style.intrinsic_size.has_value()
                                                 ? size(*style.intrinsic_size)
                                                 : runtime::Value{});
    ...
    fields.insert_or_assign("alignItems",
                            runtime::Value(std::string(align_name(style.align_items))));
    fields.insert_or_assign("justifyContent",
                            runtime::Value(std::string(justify_name(style.justify_content))));
    fields.insert_or_assign("alignContent",
                            runtime::Value(std::string(justify_name(style.align_content))));
    fields.insert_or_assign("alignSelf",
                            style.align_self.has_value()
                                ? runtime::Value(std::string(align_name(*style.align_self)))
                                : runtime::Value{});
    fields.insert_or_assign("justifySelf",
    ...
    fields.insert_or_assign("gridColumn",
                            style.grid_column.has_value()
                                ? runtime::Value(static_cast<double>(*style.grid_column))
                                : runtime::Value{});
    fields.insert_or_assign("gridRow", style.grid_row.has_value()
                                           ? runtime::Value(static_cast<double>(*style.grid_row))
                                           : runtime::Value{});
    ...
    fields.insert_or_assign("viewportInsetsFromInsideBorder",
    ...
    fields.insert_or_assign("anchorPoint", style.anchor_point.has_value()
                                               ? point(*style.anchor_point)
                                               : runtime::Value{});
    fields.insert_or_assign(
        "anchorSide", runtime::Value(style.anchor_side == LayoutAnchorSide::top     ? "TOP"
                                     : style.anchor_side == LayoutAnchorSide::right ? "RIGHT"
                                     : style.anchor_side == LayoutAnchorSide::left  ? "LEFT"
                                                                                    : "BOTTOM"));
    fields.insert_or_assign(
        "anchorAlign", runtime::Value(style.anchor_align == LayoutAnchorAlign::center ? "CENTER"
                                      : style.anchor_align == LayoutAnchorAlign::end  ? "END"
                                                                                      : "START"));
    fields.insert_or_assign("anchorGap", runtime::Value(style.anchor_gap));
    fields.insert_or_assign("anchorCrossOffset", runtime::Value(style.anchor_cross_offset));
    ...
    fields.insert_or_assign(
        "virtualAxis",
        runtime::Value(list.axis == LayoutAxis::horizontal ? "HORIZONTAL" : "VERTICAL"));
    fields.insert_or_assign("virtualItemExtent", runtime::Value(list.item_extent));
    fields.insert_or_assign("virtualItemCount",
                            runtime::Value(static_cast<double>(list.item_count())));
    ...
    if (list.item_members != nullptr) {
        ...
        for (const std::vector<std::string>& band : *list.item_members) {
            ...
            for (const std::string& key : band)
                values.emplace_back(runtime::KeyValue{key});
    ...
    if (list.item_extents.has_value()) {
        ...
        for (const double value : list.item_extents->values())
            extents.emplace_back(value);
...
[[nodiscard]] runtime::Value motion_attachment(const ThemeMotionAttachment& value,
    ...
    if (value.continuity_trigger.has_value()) {
        fields.emplace("continuityTrigger",
                       runtime::Value(std::string(motion_trigger_name(*value.continuity_trigger))));
    }
    return object(std::move(fields));
}

[[nodiscard]] std::string qualified_animation_prefix(const std::string_view theme_namespace,
...
[[nodiscard]] std::string animation_owner_namespace(const std::string_view theme_name,
    ...
    return scope_key.has_value()
               ...
               : "$global|" + std::to_string(theme_name.size()) + "#" + std::string(theme_name);
}

[[nodiscard]] std::string motion_reference(const ThemeAnimationSet& motion,
    ...
    const bool local =
        std::ranges::any_of(motion.declared_animations,
                            [&name](const CompiledMotion& value) { return value.name == name; });
    return local ? std::string(prefix) + "N" + std::to_string(name.size()) + "#" + name : name;
}

void add_motion(ValueFields& fields, const ThemeAnimationSet& motion,
                const std::string_view prefix) {
    ...
    for (const ThemeMotionAttachment& attachment : motion.attachments) {
        fields.insert_or_assign(
            ...
            motion_attachment(attachment, motion_reference(motion, prefix, attachment.animation,
                                                           std::string("trigger/") +
                                                               std::string(motion_trigger_name(
                                                                   attachment.trigger)))));
    }
    ...
    for (const ThemeMotionChannel& channel : motion.channels) {
        ValueFields declaration{
            {"animation", runtime::Value(motion_reference(motion, prefix, channel.animation,
                                                          std::string("channel/") + channel.id))},
            {"id", runtime::Value(channel.id)},
        };
        if (channel.interaction.has_value()) {
            declaration.emplace("interaction", runtime::Value(std::string(
                                                   motion_interaction_name(*channel.interaction))));
        }
    ...
    if (motion.resolved_properties.has_value()) {
        ...
        fields.insert_or_assign("animateChanges",
                                object({
    ...
    if (motion.content_size.has_value()) {
        fields.insert_or_assign("animateContentSize",
                                object({
...
void apply_motion(ValueFields& fields, const ThemeNullable<ThemeAnimationSet>& value,
                  const std::string_view prefix) {
    if (!value.has_value())
        return;
    if (!value->has_value())
        disable_motion(fields, "none");
    else if (empty_motion(**value))
        disable_motion(fields, "present-empty");
    else
        add_motion(fields, **value, prefix);
}
...
[[nodiscard]] std::string node_variant(const DescriptionNode& node) {
    ...
    if (value.string() != nullptr && !value.string()->empty())
        return *value.string();
    if (value.key() != nullptr && !value.key()->value.empty())
        return value.key()->value;
    return std::string(default_widget_variant);
}

[[nodiscard]] const runtime::Value* property(const DescriptionNode& node,
...
[[nodiscard]] const runtime::Value* style_field(const DescriptionNode& node,
                                                const std::string_view name) noexcept {
    const runtime::Value* layout = property(node, "$layout");
    if (layout != nullptr && layout->object() != nullptr) {
        if (const runtime::Value* value = layout->field(name); value != nullptr)
            return value;
    }
...
[[nodiscard]] bool mapped_color_property(const std::string_view name) noexcept {
    return name == "background" || name == "foreground" || name == "color" ||
           name == "hoverOverlay" || name == "activeOverlay" || name == "track" || name == "fill" ||
           name == "thumb" || name == "selection" || name == "scrim" || name == "hintColor" ||
           name == "selectionColor" || name == "caretColor";
}
...
[[nodiscard]] bool mapped_number_property(const std::string_view name) noexcept {
    return name == "radius" || name == "indicatorSize" || name == "indicatorInset" ||
           ...
           name == "trackGap" || name == "activeTrackGap" || name == "thumbSize" ||
           name == "thumbWidth" || name == "thumbHeight" || name == "activeThumbWidth" ||
           name == "activeThumbHeight" || name == "thumbRadius" || name == "padding" ||
           name == "margin" || name == "gap";
}

[[nodiscard]] std::optional<runtime::Value> resolve_token(const std::string_view property_name,
...
[[nodiscard]] runtime::Value motion_policy_value(const ThemeMotionPolicy& policy) {
    ...
    const auto repeat_value = [](const MotionRepeat& repeat) {
        return object({
            ...
            {"kind", runtime::Value(repeat.kind == MotionRepeatKind::forever ? "forever"
                                    : repeat.kind == MotionRepeatKind::count ? "count"
                                                                             : "none")},
        });
    };
    const auto fill_value = [](const MotionFillMode fill) {
        switch (fill) {
        case MotionFillMode::none:
            return "none";
        case MotionFillMode::forwards:
            return "forwards";
        case MotionFillMode::backwards:
            return "backwards";
        case MotionFillMode::both:
            return "both";
        }
        return "both";
    };
...
void report_timing_if_unknown(const Theme& theme, const DescriptionNode& node,
                              const runtime::Value* value, const UnknownThemeTiming& callback) {
...
void report_unknown_timings(const Theme& theme, const DescriptionNode& node,
    ...
    for (const std::string_view field_name : {
        ...
        if (declaration == nullptr || declaration->object() == nullptr)
            continue;
        const runtime::Value* timing = declaration->field("timing");
        if (timing == nullptr)
            timing = declaration->field("policy");
...
resolve_node(const std::shared_ptr<const DescriptionNode>& source, const ThemeCatalog& catalog,
             ...
             const ThemeTypePredicate& themed_type, const UnknownThemeTiming& unknown_timing,
             ThemeMaterializationStats& stats, ThemeMaterializationCache* const cache) {
    if (source == nullptr)
        return nullptr;
    std::shared_ptr<const Theme> effective = inherited;
    std::optional<std::string> scope_namespace = inherited_scope_namespace;
    const std::shared_ptr<const Theme>* scoped =
        source->key.has_value() ? catalog.scoped_theme(*source->key) : nullptr;
    if (scoped != nullptr) {
        effective = *scoped;
        scope_namespace = *source->key;
    } else if (const std::string* reference = text(property(*source, "theme"));
               reference != nullptr) {
    ...
    if (cache != nullptr) {
        if (std::shared_ptr<const DescriptionNode> cached =
                cache->find(source, effective, scope_namespace, catalog.generation());
            cached != nullptr) {
            return cached;
        }
    }
    ...
    const auto resolve_authored_fields =
        [&effective, &resolved, &stats, &authored_color_field,
         &authored_foreground_field](const runtime::Value* container,
                                     const bool merge_all) -> std::optional<runtime::Value> {
        if (container == nullptr || container->object() == nullptr)
            return std::nullopt;
        ...
        for (const auto& [name, value] : container->object()->fields) {
            ...
            if (const runtime::ThemeTokenValue* token = value.theme_token(); token != nullptr) {
                const std::optional<runtime::Value> concrete =
                    resolve_token(name, *token, effective->tokens());
    ...
    for (const std::string_view container_name : {"visualStyle", "textVisualStyle"}) {
        const std::optional<runtime::Value> concrete =
            resolve_authored_fields(property(*source, container_name), true);
        if (concrete.has_value()) {
            properties.insert_or_assign(std::string(container_name),
                                        runtime::ExpressionValue(*concrete));
        }
    }
    if (const std::optional<runtime::Value> concrete =
            resolve_authored_fields(property(*source, "textStyle"), false);
        concrete.has_value()) {
    ...
    if (themed.has_value()) {
        ...
        for (const std::string_view motion_field : {
            ...
            if (property(*source, motion_field) != nullptr) {
                ...
                if (inherited_motion != resolved.end())
                    resolved.erase(inherited_motion);
    ...
    if (lazy) {
        ...
        range.end_exclusive =
            std::clamp(source->materialization->end_exclusive, range.start, child_count);
    }
    ...
    for (std::size_t index = range.start; index < range.end_exclusive; ++index) {
        resolved_children.emplace(index, resolve_node(source->children->at(index), catalog,
...
void validate_layout_size(const LayoutSize& value, const std::string_view label) {
    ...
    case LayoutSize::Kind::clamp:
        ...
        if (value.minimum != nullptr)
            validate_layout_size(*value.minimum, label);
        validate_layout_size(*value.preferred, label);
        if (value.maximum != nullptr)
            validate_layout_size(*value.maximum, label);
        break;
    }
}
...
void validate_theme_layout(const ThemeWidgetLayoutStyle& style) {
    if (style.kind > LayoutKind::portal || style.align_items > LayoutAlign::stretch ||
        ...
        style.anchor_side > LayoutAnchorSide::left || style.anchor_align > LayoutAnchorAlign::end ||
    ...
    for (const auto& [value, label] : {
        ...
        if (value->has_value())
            validate_layout_size(**value, label);
    }
    ...
    for (const LayoutSize& track : style.grid_columns)
        validate_layout_size(track, "theme grid column");
    for (const LayoutSize& track : style.grid_rows)
        validate_layout_size(track, "theme grid row");
    ...
    if (list.items == nullptr)
        throw std::invalid_argument("theme virtual list requires items");
    ...
    if (list.item_members != nullptr && list.item_members->size() != list.item_count()) {
    ...
    for (std::size_t index = 0U; index < list.item_count(); ++index) {
        ...
        if (blank(key) || !core::valid_utf8(key) || !keys.insert(key).second) {
            throw std::invalid_argument(
                "theme virtual item keys must be unique non-blank valid UTF-8");
        }
    }
    if (list.item_members != nullptr) {
        for (const std::vector<std::string>& band : *list.item_members) {
...
void validate_compiled_motion(const CompiledMotion& motion, const bool require_name = true) {
    ...
    for (const MotionTrack& track : motion.tracks) {
        ...
        if (!properties.insert(track.property).second || track.keyframes.empty()) {
            throw std::invalid_argument(
                "theme animation tracks must be unique and contain keyframes");
        }
        double previous = -1.0;
        for (const MotionKeyframe& frame : track.keyframes) {
            ...
            if (const double* number = std::get_if<double>(&frame.value);
            ...
            if (frame.easing.has_value())
                frame.easing->validate();
...
const runtime::ColorValue* ThemeTokens::color(const std::string_view name) const noexcept {
    if (name == "surface")
        return &surface;
    if (name == "surfaceRaised")
        return &surface_raised;
    if (name == "foreground")
        return &foreground;
    if (name == "mutedForeground")
...
void ThemeWidgetVisualStyle::validate() const {
    ...
    for (const auto& [value, label] : std::array{
             std::pair{std::optional<double>(disabled_opacity),
                       std::string_view("theme disabled opacity")},
...
void ThemeTextLayoutStyle::validate() const {
    ...
    for (const auto& [value, label] : std::array{
             ...
             std::pair{std::optional<double>(line_height_multiplier),
                       std::string_view("theme line-height multiplier")},
         }) {
        if (value.has_value() && (!std::isfinite(*value) || *value <= 0.0)) {
...
void ThemeAnimationSet::validate() const {
    const auto validate_spec = [this](const ThemeAnimationSpec& spec,
    ...
    for (const ThemeMotionAttachment& attachment : attachments) {
        ...
        if (attachment.continuity_trigger == attachment.trigger) {
            throw std::invalid_argument(
                "theme animation continuity trigger must differ from its trigger");
        }
    }
    ...
    for (const ThemeMotionChannel& channel : channels) {
        ...
        if (resolved == nullptr) {
            const auto found = std::ranges::find(declared_animations, *channel.animation.named(),
                                                 &CompiledMotion::name);
            if (found != declared_animations.end())
                resolved = &*found;
        }
    ...
    for (const ThemeMotionValueChannel& channel : value_channels) {
        ...
        if (const double* number = std::get_if<double>(&channel.target);
            number != nullptr && !std::isfinite(*number)) {
        ...
        if (!motion_property_interpolable(channel.property) ||
            ...
            throw std::invalid_argument(
                "theme motion value channel target is not interpolable for its property");
        }
    }
    if (resolved_properties.has_value()) {
        ...
        for (const MotionProperty property : resolved_properties->properties) {
            ...
            if (!properties.insert(property).second) {
                throw std::invalid_argument(
                    "theme resolved-property motion properties must be unique");
            }
        }
    }
    ...
    for (const ThemeMotionChannel& channel : channels) {
        sources.push_back(
            Source{"channel '" + channel.id + "'", properties_for(channel.animation)});
    }
    for (const ThemeMotionValueChannel& channel : value_channels) {
    ...
    const ThemeContentSizeMotion* effective_content =
        content_size.has_value() ? &*content_size : nullptr;
    ...
    if (effective_content != nullptr) {
        std::set<MotionProperty> properties;
        if (effective_content->animate_width)
            properties.insert(MotionProperty::width);
        if (effective_content->animate_height)
            properties.insert(MotionProperty::height);
    ...
    for (std::size_t first = 0U; first < sources.size(); ++first) {
        for (std::size_t second = first + 1U; second < sources.size(); ++second) {
            if (sources[first].attachment && sources[second].attachment)
                continue;
            std::vector<MotionProperty> overlap;
            std::ranges::set_intersection(sources[first].properties, sources[second].properties,
                                          std::back_inserter(overlap));
            if (!overlap.empty()) {
                throw std::invalid_argument("theme motion sources '" + sources[first].name +
                                            "' and '" + sources[second].name +
...
void ThemedWidgetStyle::validate() const {
    ...
    if (text_layout.has_value())
        text_layout->validate();
    if (layout.has_value())
        validate_theme_layout(*layout);
    if (motion.has_value() && motion->has_value())
        (**motion).validate();
}
...
ThemeMotionPolicy::ThemeMotionPolicy(const bool reduced_motion,
...
bool ThemeMotionPolicy::reduced_motion() const noexcept {
    return reduced_motion_;
}

const std::map<std::string, MotionTiming, std::less<>>&
ThemeMotionPolicy::timings() const noexcept {
    return timings_;
}
...
const MotionTiming&
ThemeMotionPolicy::timing_or_default(const std::string_view name) const noexcept {
    if (const MotionTiming* timing = find(name); timing != nullptr)
        return *timing;
...
Theme::Theme(std::string name, ThemeTokens tokens, std::shared_ptr<const Theme> parent,
             std::optional<ThemeMotionPolicy> motion_policy,
             std::map<ThemeWidgetKey, ThemedWidgetStyle> widget_styles)
    : name_(std::move(name)), tokens_(std::move(tokens)), parent_(std::move(parent)),
      motion_policy_(motion_policy.has_value() ? std::move(*motion_policy)
                     : parent_ != nullptr      ? parent_->motion_policy()
                                               : ThemeMotionPolicy{}),
...
const std::string& Theme::name() const noexcept {
    return name_;
}
const ThemeTokens& Theme::tokens() const noexcept {
    return tokens_;
}
const std::shared_ptr<const Theme>& Theme::parent() const noexcept {
    return parent_;
}
const ThemeMotionPolicy& Theme::motion_policy() const noexcept {
...
Theme::explicit_style(const std::string_view component_type, const std::string_view variant) const {
    ...
    return parent_ != nullptr ? parent_->explicit_style(component_type, variant) : std::nullopt;
}

ThemedWidgetStyle Theme::semantic_style(const std::string_view component_type,
                                        const std::string_view raw_variant) const {
    ...
    const runtime::ColorValue text_foreground = text_only && variant == "primary"  ? tokens_.accent
                                                : text_only && variant == "danger" ? tokens_.danger
                                                : text_only && variant == "subtle"
    ...
    if (text_only || structural || choice_group || variant == "subtle") {
        ...
        visual.background = std::optional<runtime::ColorValue>(
            variant == "primary" || variant == "danger" ? accent
            : variant == "secondary" || raised          ? tokens_.surface_raised
                                                        : tokens_.surface);
    ...
    visual.active_overlay =
        std::optional<runtime::ColorValue>(runtime::ColorValue{0U, 0U, 0U, 32U});
    ...
    if (!text_only)
        text_layout.primary_font = "strata:fonts/default-medium";
    text_layout.pixel_size = 12.0 * tokens_.density * (variant == "compact" ? 0.9 : 1.0);
    return ThemedWidgetStyle{
        std::move(visual), std::move(text_visual), std::move(text_layout),
        std::nullopt,      std::nullopt,
    };
}

ThemedWidgetStyle Theme::style(const std::string_view component_type,
                               const std::string_view variant) const {
    return resolved_style(component_type, variant).style;
}

ResolvedThemeWidgetStyle Theme::resolved_style(const std::string_view component_type,
                                               const std::string_view variant) const {
    ...
    if (std::optional<ResolvedThemeWidgetStyle> explicit_value =
            explicit_style(component_type, effective_variant);
...
Theme::derive(std::string name, ThemeTokens tokens,
              std::map<ThemeWidgetKey, ThemedWidgetStyle> widget_styles) const {
    return std::make_shared<const Theme>(std::move(name), std::move(tokens),
                                         std::make_shared<const Theme>(*this), motion_policy_,
...
const std::shared_ptr<const Theme>& ThemeCatalog::root() const noexcept {
    return root_;
}
...
bool ThemeCatalog::register_theme(Theme theme) {
    ...
    if (existing != themes_.end() && *existing->second == theme)
        return false;
...
ThemeCatalog::scoped_theme(const std::string_view node_key) const noexcept {
...
bool ThemeCatalog::set_scoped_theme(std::string node_key, Theme theme) {
    ...
    const auto found = scoped_themes_.find(node_key);
    if (found != scoped_themes_.end() && *found->second == theme)
        return false;
    scoped_themes_.insert_or_assign(std::move(node_key),
...
std::map<std::string, CompiledMotion, std::less<>> ThemeCatalog::declared_animations() const {
    ...
    const auto collect = [&result](const auto& self, const Theme& theme,
                                   const std::optional<std::string>& scope_namespace) -> void {
        const std::string owner_namespace =
            animation_owner_namespace(theme.name(), scope_namespace);
        for (const auto& [key, style] : theme.widget_styles()) {
            if (!style.motion.has_value() || !style.motion->has_value())
                continue;
            ...
            for (const CompiledMotion& declaration : (**style.motion).declared_animations) {
                ...
                qualified.name =
                    prefix + "N" + std::to_string(declaration.name.size()) + "#" + declaration.name;
            ...
            const auto collect_inline = [&result, &prefix](const CompiledMotion& declaration,
                                                           const std::string_view slot) {
                CompiledMotion qualified = declaration;
                qualified.name =
                    prefix + "I" + std::to_string(slot.size()) + "#" + std::string(slot);
                const auto [found, inserted] = result.emplace(qualified.name, qualified);
                if (!inserted && found->second != qualified) {
            ...
            for (const ThemeMotionAttachment& attachment : (**style.motion).attachments) {
                if (const CompiledMotion* inline_animation =
                        attachment.animation.inline_animation();
                    inline_animation != nullptr) {
                    collect_inline(*inline_animation,
                                   std::string("trigger/") +
                                       std::string(motion_trigger_name(attachment.trigger)));
                }
            }
        ...
        if (theme.parent() != nullptr)
            self(self, *theme.parent(), scope_namespace);
    };
...
std::uint64_t ThemeCatalog::generation() const noexcept {
    return generation_;
}
...
ThemeMaterializationCache::find(const std::shared_ptr<const DescriptionNode>& source,
    ...
    const std::shared_ptr<const DescriptionNode> materialized = found->second.materialized.lock();
...
materialize_theme_tree(const std::shared_ptr<const DescriptionNode>& root,
                       const ThemeCatalog& catalog, const ThemeTypePredicate& themed_type,
    ...
    if (cache != nullptr)
        cache->purge(catalog.generation());
    ThemeMaterializationResult result;
    result.root = resolve_node(root, catalog, catalog.root(), std::nullopt, themed_type,
                               unknown_timing, result.stats, cache);
    return result;
}

ThemeMaterializationResult materialize_theme_subtree(
    const std::shared_ptr<const DescriptionNode>& root, const ThemeCatalog& catalog,
    ...
    if (cache != nullptr)
        cache->purge(catalog.generation());
    ThemeMaterializationResult result;
    result.root = resolve_node(root, catalog, inherited_theme, inherited_scope_namespace,
                               themed_type, unknown_timing, result.stats, cache);
    return result;
}

bool theme_motion_reduced(const DescriptionNode& description,
                          const bool environment_reduced_motion) noexcept {
    if (environment_reduced_motion)
        return true;
...
MotionTiming theme_motion_timing(const DescriptionNode& description,
                                 const std::string_view name) noexcept {
    const runtime::Value* policy = property(description, "$motionPolicy");
    const runtime::Value* timings = policy != nullptr ? policy->field("timings") : nullptr;
    const auto decoded =
        [timings](const std::string_view timing_name) -> std::optional<MotionTiming> {
        ...
        try {
            if (easing->string() != nullptr) {
                ...
                parsed_easing = MotionEasing::cubic_bezier(*x1, *y1, *x2, *y2);
            } else {
                return std::nullopt;
            }
        ...
        if (const runtime::Value* repeated = value->field("repeat"); repeated != nullptr) {
            if (repeated->object() == nullptr)
                return std::nullopt;
            const std::string* kind = text(repeated->field("kind"));
            if (kind == nullptr)
                return std::nullopt;
            if (*kind == "forever")
                repeat.kind = MotionRepeatKind::forever;
            else if (*kind == "count") {
        ...
        if (const std::string* mode = text(value->field("fillMode")); mode != nullptr) {
            if (*mode == "none")
                fill = MotionFillMode::none;
            else if (*mode == "forwards")
                fill = MotionFillMode::forwards;
            else if (*mode == "backwards")
                fill = MotionFillMode::backwards;
            else if (*mode != "both")
                return std::nullopt;
        }
        const runtime::Value* reversed = value->field("reverse");
        if (reversed != nullptr && reversed->boolean() == nullptr)
            return std::nullopt;
        return MotionTiming{
...
bool theme_motion_timing_defined(const DescriptionNode& description,
                                 const std::string_view name) noexcept {
    const runtime::Value* policy = property(description, "$motionPolicy");
    const runtime::Value* timings = policy != nullptr ? policy->field("timings") : nullptr;
