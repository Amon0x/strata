...
[[nodiscard]] const runtime::Value* property(const DescriptionNode& description,
...
[[nodiscard]] const runtime::Value* style_field(const DescriptionNode& description,
                                                const std::string_view name) noexcept {
    for (const std::string_view container : {"textStyle", "$layout", "style"}) {
        if (const runtime::Value* style = property(description, container);
            ...
            if (const runtime::Value* value = style->field(name); value != nullptr)
                return value;
        }
    }
...
[[nodiscard]] std::optional<double> positive_number(const runtime::Value* value) noexcept {
    const double resolved = number(value, -1.0);
    return resolved > 0.0 ? std::optional<double>(resolved) : std::nullopt;
}

[[nodiscard]] std::optional<std::size_t> positive_count(const runtime::Value* value) noexcept {
    const double resolved = number(value, -1.0);
    if (resolved <= 0.0)
        return std::nullopt;
    return static_cast<std::size_t>(std::min(
        std::floor(resolved), static_cast<double>(std::numeric_limits<std::size_t>::max())));
}

[[nodiscard]] std::uint32_t unsigned_flags(const runtime::Value* value,
                                           const std::uint32_t fallback = 0U) noexcept {
    const double resolved = number(value, static_cast<double>(fallback));
    if (resolved <= 0.0)
        return 0U;
    return static_cast<std::uint32_t>(std::min(
        std::floor(resolved), static_cast<double>(std::numeric_limits<std::uint32_t>::max())));
}

[[nodiscard]] std::string text_option(const DescriptionNode& description,
                                      const std::string_view name, std::string fallback) {
...
[[nodiscard]] FontRasterization
font_rasterization_value(const runtime::Value* value,
                         const FontRasterization fallback = FontRasterization::grayscale) noexcept {
    if (value == nullptr || value->string() == nullptr)
        return fallback;
    return *value->string() == "MSDF" ? FontRasterization::msdf : FontRasterization::grayscale;
}
...
[[nodiscard]] const runtime::Value* object_field(const runtime::Value* object,
                                                 const std::string_view name) noexcept {
    return object != nullptr && object->object() != nullptr ? object->field(name) : nullptr;
}

[[nodiscard]] std::string font_value(const runtime::Value* value, std::string fallback) {
...
[[nodiscard]] std::vector<std::string> fallback_font_values(const runtime::Value* value) {
    ...
    for (const runtime::Value& entry : value->list()->values) {
        ...
        if (entry.string() != nullptr)
            ...
            id = entry.image()->id;
        if (!id.empty() && !std::ranges::contains(result, id))
            result.push_back(std::move(id));
    }
    return result;
}

[[nodiscard]] std::optional<std::uint32_t> packed_color(const runtime::Value* value) noexcept {
    if (value == nullptr || value->color() == nullptr)
        return std::nullopt;
    ...
    return static_cast<std::uint32_t>(color.red) << 24U |
           ...
           static_cast<std::uint32_t>(color.blue) << 8U | static_cast<std::uint32_t>(color.alpha);
}
...
TextLayout::TextLayout(std::shared_ptr<const TextLayoutData> storage) noexcept
    : storage_(std::move(storage)), shaped(storage_->shaped),
      glyph_font_ids(storage_->glyph_font_ids), glyph_pixel_sizes(storage_->glyph_pixel_sizes),
      resolved_runs(storage_->resolved_runs), lines(storage_->lines),
      wrap_width(storage_->wrap_width), clipped(storage_->clipped), truncated(storage_->truncated) {
}

TextLayout::TextLayout() : TextLayout(std::make_shared<const TextLayoutData>()) {}

TextLayout::TextLayout(const TextLayout& other) noexcept : TextLayout(other.storage_) {}

TextLayout::TextLayout(TextLayout&& other) noexcept : TextLayout(other.storage_) {}
...
bool text_layout_field(const std::string_view name) noexcept {
    static constexpr std::array<std::string_view, 16U> fields{
        "text",          "label",          "spans",
        "font",          "fallbackFonts",  "fontSize",
        "pixelSize",     "lineHeight",     "lineHeightMultiplier",
        "letterSpacing", "fontStyleFlags", "fontRasterization",
        "wrapWidth",     "wrapMode",       "overflow",
        "maxLines",
    };
    return std::ranges::contains(fields, name) || name == "alignment";
}

bool text_layout_projection_equal(const runtime::Value* current,
...
TextEngine::TextEngine(FontRegistry fonts) : fonts_(std::move(fonts)) {
    ...
    for (const auto& [font_id_value, face] : fonts_) {
        for (const std::string& detail : face.optional_diagnostics()) {
            diagnostics_.push_back(runtime::RuntimeDiagnostic{
                ...
                "Font '" + font_id_value +
                    "' remains usable, but an optional feature was "
                    "ignored: " +
                    detail,
                {},
                std::nullopt,
...
std::shared_ptr<const TextEngine>
TextEngine::load_control_font(const std::filesystem::path& root,
                              const resource::ResourceId& resource) {
    return std::make_shared<const TextEngine>(
        font::OpenTypeFont::parse(resource::load_binary_resource(root, resource)));
}

std::shared_ptr<const TextEngine>
TextEngine::load_default_fonts(const std::filesystem::path& root) {
    return std::make_shared<const TextEngine>(FontRegistry{
        {
            "strata:fonts/default-medium",
            font::OpenTypeFont::parse(resource::load_binary_resource(
                root, resource::ResourceId::parse("assets/strata/fonts/medium.ttf"))),
        },
        {
            "strata:fonts/default",
            font::OpenTypeFont::parse(resource::load_binary_resource(
                root, resource::ResourceId::parse("assets/strata/fonts/default.ttf"))),
        },
        {
            "strata:fonts/mono",
            font::OpenTypeFont::parse(resource::load_binary_resource(
                root, resource::ResourceId::parse("assets/strata/fonts/mono.ttf"))),
        },
    });
}

Size TextEngine::measure(const RetainedNode& node, const Constraints& constraints) const {
    const DescriptionNode& description = node.description();
    const runtime::Value* text_value = property(description, "text");
    if (text_value == nullptr)
        text_value = node.retained_value("$text");
    if (text_value == nullptr)
        text_value = property(description, "label");
    if (text_value == nullptr || text_value->string() == nullptr)
        return {};
    ...
    CacheKey selected =
        request_key(node, *text_value->string(), explicit_width ? bounded_width : std::nullopt);
...
const font::OpenTypeFont& TextEngine::font(const std::string_view id) const noexcept {
    ...
    return found != fonts_.end() ? found->second : fonts_.find("strata:fonts/default")->second;
}
...
std::string TextEngine::requested_font_id(const RetainedNode& node) const {
    ...
    if (value != nullptr) {
        if (value->string() != nullptr && !value->string()->empty())
            return *value->string();
        if (value->image() != nullptr && !value->image()->id.empty())
            return value->image()->id;
    }
...
double TextEngine::pixel_size(const RetainedNode& node) const noexcept {
    return std::max(0.001, number(style_field(node.description(), "pixelSize"),
...
font::ShapedText TextEngine::shape(const RetainedNode& node, const std::string_view text) const {
...
TextLayout TextEngine::layout(const RetainedNode& node, const std::string_view text) const {
    CacheKey key = request_key(node, text, std::nullopt);
    if (const auto measured = measured_requests_.find(node.identity());
        measured != measured_requests_.end() && measured->second.text == text &&
        !key.wrap_width.has_value()) {
...
TextLayout TextEngine::layout(const RetainedNode& node, const std::string_view text,
                              const TextLayoutOptions& options) const {
    return cached_layout(request_key(node, text, std::nullopt, &options), true);
}

void TextEngine::adopt_scale_context(const std::uint64_t scale_context) const {
    if (scale_context_generation_ == scale_context)
        return;
    if (scale_context_generation_.has_value()) {
        cache_.clear();
...
TextEngine::CacheKey TextEngine::request_key(const RetainedNode& node, const std::string_view text,
                                             const std::optional<double> measured_wrap_width,
                                             const TextLayoutOptions* const overrides) const {
    const DescriptionNode& description = node.description();
    const std::optional<double> authored_wrap =
        positive_number(style_field(description, "wrapWidth"));
    std::optional<double> resolved_wrap =
        authored_wrap.has_value() ? authored_wrap : measured_wrap_width;
    ...
    if (description.type != "RichText")
        return result;
    const runtime::Value* spans = property(description, "spans");
    if (spans == nullptr || spans->list() == nullptr)
        return result;
    ...
    for (std::size_t span_index = 0U; span_index < spans->list()->values.size(); ++span_index) {
        ...
        if (span_text == nullptr || span_text->string() == nullptr)
            continue;
        ...
        if (span_expressions != description.properties.end() &&
            ...
            const runtime::ExpressionValue* action =
                expression.object() != nullptr ? (**expression.object()).field("action") : nullptr;
            interactive =
                action != nullptr && action->action() != nullptr && *action->action() != nullptr;
        }
        ...
        result.style_runs.push_back(StyleRun{
            ...
            authored_fallbacks != nullptr ? fallback_font_values(authored_fallbacks)
                                          : result.fallback_fonts,
            std::max(0.001, number(run_field("pixelSize"),
            ...
            positive_number(run_field("lineHeight")).has_value()
                ...
                : result.line_height,
            std::max(0.001,
                     number(run_field("lineHeightMultiplier"), result.line_height_multiplier)),
            number(run_field("letterSpacing"), result.letter_spacing),
            unsigned_flags(run_field("fontStyleFlags"), result.font_style_flags),
            font_rasterization_value(run_field("fontRasterization"), result.font_rasterization),
...
const TextLayout& TextEngine::cached_layout(CacheKey key, const bool count_request) const {
    if (count_request)
        ++operation_counters_.requests;
    const auto lookup_started = std::chrono::steady_clock::now();
    const auto found = cache_.find(key);
    operation_counters_.cache_lookup_nanos +=
    ...
    if (found != cache_.end()) {
        ...
        if (count_request)
            ++operation_counters_.cache_hits;
        const TextLayout& restored = found->second;
        operation_counters_.cache_restore_nanos +=
            static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    operation_counters_.shaping_nanos +=
        static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                       std::chrono::steady_clock::now() - shaping_started)
                                       .count());
    record_missing_glyphs(key.font_id, layout_value.shaped);
    if (count_request)
        ++operation_counters_.cache_misses;
    const auto store_started = std::chrono::steady_clock::now();
    const TextLayout& stored =
        cache_.emplace(std::move(key), std::move(layout_value)).first->second;
    operation_counters_.cache_store_nanos +=
        static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
...
TextLayout TextEngine::build_layout(const CacheKey& key) const {
    ...
    const auto decode = [](const std::string_view value, std::size_t& offset) {
        ...
        if ((lead & 0xE0U) == 0xC0U)
            count = 1U, result = lead & 0x1FU;
        else if ((lead & 0xF0U) == 0xE0U)
            count = 2U, result = lead & 0x0FU;
        else
            count = 3U, result = lead & 0x07U;
    ...
    const auto fallback_order = [this](const std::string& requested,
        ...
        const auto append = [&base, this](const std::string& id) {
            ...
            if (std::ranges::find(base, &*found) == base.end())
                base.push_back(&*found);
        };
        append(requested);
        ...
        const std::uint32_t requested_known = requested_style_flags & font::known_font_style_flags;
        if (requested_known == 0U)
            return base;
        ...
        for (std::size_t anchor_index = 0U; anchor_index < base.size(); ++anchor_index) {
            if (emitted[anchor_index])
                continue;
            const font::FontMetadata& anchor = base[anchor_index]->second.metadata();
            std::vector<std::size_t> family;
            for (std::size_t candidate_index = anchor_index; candidate_index < base.size();
                 ++candidate_index) {
                if (emitted[candidate_index])
                    continue;
                const font::FontMetadata& candidate = base[candidate_index]->second.metadata();
                const bool same_family =
                    !anchor.family.empty() && candidate.family == anchor.family;
            ...
            std::ranges::stable_sort(
                family, [&base, requested_known](const std::size_t left, const std::size_t right) {
    ...
    const FontOrder default_font_order =
        fallback_order(key.font_id, key.fallback_fonts, key.font_style_flags);
    ...
    for (const StyleRun& style : key.style_runs) {
        style_font_orders.push_back(
            fallback_order(style.font_id, style.fallback_fonts, style.font_style_flags));
    }
    ...
    while (byte_offset < key.text.size()) {
        ...
        if (style != nullptr)
            unit.authored_span_index = style->authored_span_index;
        ...
        unit.line_height_multiplier =
            style != nullptr ? style->line_height_multiplier : key.line_height_multiplier;
        unit.letter_spacing = style != nullptr ? style->letter_spacing : key.letter_spacing;
        unit.font_style_flags = style != nullptr ? style->font_style_flags : key.font_style_flags;
        unit.font_rasterization =
            style != nullptr ? style->font_rasterization : key.font_rasterization;
        unit.color_rgba = style != nullptr ? style->color_rgba : std::nullopt;
        unit.interactive = style != nullptr && style->interactive;
        const FontOrder& ordered_fonts =
            style != nullptr ? style_font_orders[style_run_index] : default_font_order;
        unit.fallback_fonts = style != nullptr ? &style->fallback_fonts : &key.fallback_fonts;
    ...
    operation_counters_.font_resolution_nanos +=
        static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    for (std::size_t begin = 0U; begin < units.size();) {
        ...
        const std::vector<font::PositionedRunGlyph> shaped = face.shape_glyph_run(
            std::span<const font::GlyphRunInput>(glyphs), units[begin].font_style_flags);
        double y_pen = 0.0;
        ...
        for (std::size_t shaped_index = 0U; shaped_index < shaped.size(); ++shaped_index) {
            ...
            if (glyph.ligature_component_clusters.size() > 1U)
                unit.whitespace = false;
    ...
    operation_counters_.opentype_nanos +=
        static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    const auto layout_segment = [&](const std::vector<std::size_t>& segment,
                                    const std::size_t segment_start, const std::size_t segment_end,
        ...
        if (!wraps) {
            drafts.push_back(DraftLine{segment,
                                       {},
                                       segment.front() < units.size() ? units[segment.front()].start
                                                                      : segment_start,
                                       segment_end,
                                       hard_break_unit});
            return;
        ...
        if (character_wrap) {
            ...
            for (const std::size_t index : segment) {
                if (tokens.empty() ||
                    units[tokens.back().front()].whitespace != units[index].whitespace) {
                    tokens.push_back({});
                }
        ...
        const auto flush = [&](const bool trim_break_whitespace) {
            ...
            if (trim_break_whitespace &&
                ...
                const auto trailing = std::ranges::find_if(current.units | std::views::reverse,
                                                           [&units](const std::size_t index) {
                                                               return !units[index].whitespace;
                                                           })
                                          .base();
            ...
            const std::vector<std::size_t>& source =
                !current.units.empty() ? current.units : current.soft_wrap_gap_units;
        ...
        for (const std::vector<std::size_t>& token : tokens) {
            ...
            if (standalone_width > wrap_width && token.size() > 1U) {
                ...
                const double accepted_width =
                    current.units.empty() ? standalone_width : appended_width;
    ...
    if (drafts.empty())
        drafts.push_back(DraftLine{{}, {}, 0U, 0U, std::nullopt});
    ...
    if (line_count_truncated && upper(key.overflow) == "ELLIPSIS" && key.wrap_width.has_value() &&
        !drafts.empty() && !std::ranges::contains(ellipsis_lines, drafts.size() - 1U)) {
    ...
    for (const std::size_t ellipsis_line : ellipsis_lines) {
        ...
        const Unit* marker_style = !target_line.units.empty() ? &units[target_line.units.back()]
        ...
        const std::string& marker_request =
            marker.font_id != nullptr ? *marker.font_id : key.font_id;
        ...
        for (const FontCandidate* candidate : marker_fonts) {
            ...
            const font::GlyphPositionAdjustment style =
                face.position_glyphs(marker_glyph, marker.font_style_flags).front();
            marker.advance =
                (static_cast<double>(face.horizontal_advance(glyph)) + style.x_advance) * scale;
            marker.ascent = std::max(0.0, static_cast<double>(face.ascender()) * scale);
            const double descent = std::max(0.0, -static_cast<double>(face.descender()) * scale);
            const double gap = std::max(0.0, static_cast<double>(face.line_gap()) * scale);
            marker.natural_height = std::max(marker.pixel_size, marker.ascent + descent + gap);
            break;
        }
    ...
    for (std::size_t line_index = 0U; line_index < drafts.size(); ++line_index) {
        ...
        const bool has_resolved_metrics = !draft.units.empty() || draft.hard_break_unit.has_value();
        double natural_height = has_resolved_metrics ? 0.0 : key.pixel_size;
        double line_height =
            has_resolved_metrics
                ? 0.0
        ...
        for (const std::size_t unit_index : draft.units) {
            ...
            line_height =
                std::max(line_height, unit.line_height.value_or(unit.natural_height *
                                                                unit.line_height_multiplier));
        }
        ...
        result.shaped.metrics.natural_line_height =
            std::max(result.shaped.metrics.natural_line_height, natural_height);
        const double baseline =
            line_y + std::max(0.0, line_height - natural_height) * 0.5 + line_ascent;
        double cursor = 0.0;
        for (std::size_t position = 0U; position < draft.units.size(); ++position) {
                    ...
                    if (continues_run) {
                        ...
                        result.resolved_runs.push_back(TextResolvedRun{
                            ...
                            *unit.font_id,
                            unit.pixel_size,
                            unit.letter_spacing,
                            unit.color_rgba.has_value() ? std::optional<runtime::ColorValue>(
        ...
        const std::optional<std::size_t> gap_start =
            draft.soft_wrap_gap_units.empty()
                ? std::nullopt
                : std::optional<std::size_t>(units[draft.soft_wrap_gap_units.front()].start);
        const std::optional<std::size_t> gap_end =
            draft.soft_wrap_gap_units.empty()
                ? std::nullopt
                : std::optional<std::size_t>(units[draft.soft_wrap_gap_units.back()].end);
    ...
    operation_counters_.line_assembly_nanos +=
        static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
...
void TextEngine::record_missing_font(const std::string_view font_id_value) const {
    if (!missing_fonts_.insert(std::string(font_id_value)).second)
        return;
    diagnostics_.push_back(runtime::RuntimeDiagnostic{
...
void TextEngine::record_missing_glyphs(const std::string_view font_id_value,
                                       const font::ShapedText& shaped) const {
    ...
    for (const std::uint32_t code_point : shaped.missing_code_points) {
        ...
        if (!missing_glyph_groups_.insert(key).second)
            continue;
        diagnostics_.push_back(runtime::RuntimeDiagnostic{
