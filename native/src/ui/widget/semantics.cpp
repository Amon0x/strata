...
[[nodiscard]] std::vector<std::string> actions_for(const std::string_view role) {
    if (role == "button" || role == "link") return {"activate", "focus"};
    if (role == "switch" || role == "checkbox") return {"focus", "toggle"};
    if (role == "text_field" || role == "spin_button") return {"activate", "focus"};
    if (role == "slider") return {"decrement", "focus", "increment"};
    if (role == "tab" || role == "tab_list") return {"activate", "focus"};
    if (role == "dialog") return {"focus"};
    if (role == "combo_box") return {"expand", "focus"};
    if (role == "tree" || role == "tree_item") return {"focus"};
    if (role == "radio_group") return {"activate", "focus"};
    if (role == "table" || role == "grid" || role == "grid_cell") return {"focus"};
...
WidgetSemanticsScope::WidgetSemanticsScope(
    ...
    commands_(commands),
    input_(input),
    explicit_semantics_(property("semantics")),
    ...
    if (!disabled_ && commands_ != nullptr) {
        ...
        disabled_ = binding.has_value() && binding->command != nullptr &&
                    !binding->command->enabled;
    }
    ...
    if (!checked_.has_value() && (role_ == "switch" || role_ == "checkbox")) {
        ...
        if (!checked_.has_value()) checked_ = boolean(retained("$checked"));
        if (!checked_.has_value()) checked_ = boolean(property("defaultChecked"));
    }
    ...
    if (role_ == "text_field" && !read_only_.has_value()) read_only_ = false;
    if (!selected_.has_value() && (role_ == "row" || role_ == "grid_cell")) selected_ = false;
    ...
    if (current != nullptr && current->number() != nullptr) {
        ...
        value_range_ = object({
            ...
            {"maximum", maximum != nullptr && maximum->number() != nullptr
                            ? JsonValue(*maximum->number()) : JsonValue{}},
            {"minimum", minimum != nullptr && minimum->number() != nullptr
                            ? JsonValue(*minimum->number()) : JsonValue{}},
...
const RetainedNode& WidgetSemanticsScope::node() const noexcept { return node_; }
const CommandIndex* WidgetSemanticsScope::command_index() const noexcept { return commands_; }

const runtime::ExpressionValue* WidgetSemanticsScope::expression_property(
...
const runtime::Value* WidgetSemanticsScope::explicit_field(const std::string_view name) const noexcept {
...
bool WidgetSemanticsScope::effective_boolean(
    ...
    if (!value.has_value()) value = boolean(retained(retained_name));
    if (!value.has_value()) value = boolean(property(initial));
    return value.value_or(fallback);
}

std::optional<std::string> WidgetSemanticsScope::effective_text(
    const std::string_view controlled,
    const std::string_view retained_name,
    const std::string_view initial
) const {
    std::optional<std::string> value = text(property(controlled));
    if (!value.has_value()) value = text(retained(retained_name));
    if (!value.has_value()) value = text(property(initial));
    return value;
}

bool WidgetSemanticsScope::has_ancestor(const std::string_view type) const noexcept {
    for (const RetainedNode* current = node_.parent(); current != nullptr; current = current->parent()) {
        if (current->description().type == type) return true;
    }
    return false;
}

std::string_view WidgetSemanticsScope::role() const noexcept { return role_; }
...
void WidgetSemanticsScope::name(std::string value) {
    if (!explicit_name_) name_ = std::move(value);
}
void WidgetSemanticsScope::default_name(std::string value) {
    if (!explicit_name_ && name_.empty()) name_ = std::move(value);
}
void WidgetSemanticsScope::actions(std::vector<std::string> values) { actions_ = std::move(values); }
void WidgetSemanticsScope::live_region(std::string value) { live_region_ = lower_ascii(std::move(value)); }
void WidgetSemanticsScope::checked(const std::optional<bool> value) noexcept { checked_ = value; }
void WidgetSemanticsScope::expanded(const std::optional<bool> value) noexcept { expanded_ = value; }
void WidgetSemanticsScope::selected(const std::optional<bool> value) noexcept { selected_ = value; }
void WidgetSemanticsScope::value_text(std::optional<std::string> value) { value_text_ = std::move(value); }
void WidgetSemanticsScope::default_value_text(std::optional<std::string> value) {
    if (!value_text_.has_value()) value_text_ = std::move(value);
}
void WidgetSemanticsScope::value_range(JsonValue value) { value_range_ = std::move(value); }
void WidgetSemanticsScope::read_only(const std::optional<bool> value) noexcept { read_only_ = value; }
void WidgetSemanticsScope::dirty(const std::optional<bool> value) noexcept { dirty_ = value; }
void WidgetSemanticsScope::invalid(const std::optional<bool> value) noexcept { invalid_ = value; }
void WidgetSemanticsScope::required(const std::optional<bool> value) noexcept { required_ = value; }
void WidgetSemanticsScope::touched(const std::optional<bool> value) noexcept { touched_ = value; }
...
void WidgetSemanticsScope::virtual_before(JsonValue child) { before_.push_back(std::move(child)); }
void WidgetSemanticsScope::virtual_after(JsonValue child) { after_.push_back(std::move(child)); }
...
JsonValue WidgetSemanticsScope::virtual_item(
    ...
    for (std::string& action : action_names) actions.emplace_back(std::move(action));
    return object({
        {"actions", array(std::move(actions))},
        {"children", array(std::move(children))},
        {"description", JsonValue{}},
        {"key", node_.description().key.has_value() ? JsonValue(*node_.description().key) : JsonValue{}},
        ...
        {"structuralPath", JsonValue(
            structural_parent.value_or(std::string(node_.structural_path())) + "/" +
...
JsonValue WidgetSemanticsScope::virtual_command(
    ...
    return object({
        ...
        {"structuralPath", JsonValue(
            std::string(node_.structural_path()) + "/" + std::to_string(2'000'000U + index)
        )},
...
JsonValue WidgetSemanticsScope::virtual_tab(
    ...
    std::vector<JsonValue> actions;
    for (std::string action : actions_for("tab")) actions.emplace_back(std::move(action));
    return object({
        {"actions", array(std::move(actions))},
        {"children", array()},
        {"description", JsonValue{}},
...
JsonValue WidgetSemanticsScope::build(JsonValue::Array retained_children) const {
    ...
    for (const std::string& action : actions_) actions.emplace_back(action);
    return object({
        {"actions", array(std::move(actions))},
        {"children", array(std::move(children))},
...
bool widget_semantic_hidden(const RetainedNode& node) {
    ...
    const runtime::Value* semantics = found != node.description().properties.end()
                                          ? found->second.value()
                                          : nullptr;
    const runtime::Value* decorative = semantics != nullptr ? semantics->field("decorative") : nullptr;
