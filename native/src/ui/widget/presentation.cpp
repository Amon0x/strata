...
[[nodiscard]] bool participates(const WidgetLifecycle& lifecycle, const RetainedNode& node) {
    return !lifecycle.participates || lifecycle.participates(node);
}

[[nodiscard]] double value_number(const runtime::Value* value, const double fallback) noexcept {
    return value != nullptr && value->number() != nullptr && std::isfinite(*value->number())
               ? *value->number()
               : fallback;
}

[[nodiscard]] bool value_boolean(const runtime::Value* value, const bool fallback) noexcept {
    return value != nullptr && value->boolean() != nullptr ? *value->boolean() : fallback;
}

[[nodiscard]] std::optional<RenderColor> optional_color(const runtime::Value* value) noexcept {
...
[[nodiscard]] std::optional<RenderBorder>
resolved_border(const runtime::Value* value, const std::optional<RenderBorder>& fallback) noexcept {
    if (value == nullptr)
        return fallback;
    if (value->kind() == runtime::ValueKind::null_value)
        return std::nullopt;
...
[[nodiscard]] WidgetVisualStyle resolve_visual(const WidgetRenderScope& scope,
    ...
    result.foreground = widget_color(scope.style("foreground"),
    ...
    result.disabled_opacity =
        std::clamp(value_number(scope.style("disabledOpacity"), result.disabled_opacity), 0.0, 1.0);
    ...
    if (const MotionComputedValues* motion = scope.motion_values(); motion != nullptr) {
        const auto animated_color =
            [motion](const MotionProperty property) -> std::optional<RenderColor> {
            const runtime::ColorValue* value = motion->color(property);
            return value != nullptr ? std::optional<RenderColor>(RenderColor{
                                          value->red,
                                          value->green,
                                          value->blue,
                                          value->alpha,
        ...
        if (const auto value = animated_color(MotionProperty::track); value.has_value())
            result.track = Paint(*value);
        if (const auto value = animated_color(MotionProperty::fill); value.has_value())
            result.fill = Paint(*value);
        if (const auto value = animated_color(MotionProperty::thumb); value.has_value())
            result.thumb = *value;
...
TextureRegion widget_texture_region(const runtime::Value* value,
                                    const TextureRegion fallback) noexcept {
...
RenderColor widget_color(const runtime::Value* value, const RenderColor fallback) noexcept {
...
RenderColor widget_opacity(RenderColor color, const double multiplier) noexcept {
    color.alpha = static_cast<std::uint8_t>(std::clamp(
        static_cast<int>(static_cast<double>(color.alpha) * std::clamp(multiplier, 0.0, 1.0)), 0,
        255));
    return color;
}
...
void command_tooltip_overlay(WidgetRenderScope& scope) {
    ...
    const std::string text =
        shortcut.empty() ? binding->command->label : binding->command->label + "  " + shortcut;
    ...
    const double x = std::clamp(anchor.x + (anchor.width - width) * 0.5, viewport.x,
...
WidgetRenderScope::WidgetRenderScope(const RetainedNode& node, const LayoutRecord& layout,
    ...
    : node_(node), layout_(layout), layout_result_(layout_result), input_(input),
      commands_(commands), text_(text), svg_images_(svg_images), motion_(motion),
      inherited_opacity_(inherited_opacity),
      apply_presentation_opacity_(apply_presentation_opacity), output_(output),
      enabled_([&node, &commands]() {
          ...
          const bool behavior_enabled = std::ranges::none_of(
              node.description().behaviors, [](const DescriptionBehavior& behavior) {
                  return behavior.id == "strata.disabled" && behavior.enabled;
              });
          if (!behavior_enabled)
              return false;
          const std::optional<CommandActivationBinding> binding = commands.activation_binding(node);
          return !binding.has_value() || binding->command == nullptr || binding->command->enabled;
      }()) {
    visual_ = resolve_visual(*this, visual_profile);
}

const RetainedNode& WidgetRenderScope::node() const noexcept {
    return node_;
}
const LayoutRecord& WidgetRenderScope::layout() const noexcept {
    return layout_;
}
const LayoutResult& WidgetRenderScope::layout_result() const noexcept {
    return layout_result_;
}
const InputRouter& WidgetRenderScope::input() const noexcept {
    return input_;
}
const CommandIndex& WidgetRenderScope::command_index() const noexcept {
    return commands_;
}
const TextEngine* WidgetRenderScope::text_engine() const noexcept {
    return text_;
}
const WidgetVisualStyle& WidgetRenderScope::visual() const noexcept {
    return visual_;
}
bool WidgetRenderScope::enabled() const noexcept {
    return enabled_;
}
bool WidgetRenderScope::focused() const noexcept {
    return input_.focused(node_.identity());
}
bool WidgetRenderScope::focus_visible() const noexcept {
    return input_.focus_visible(node_.identity());
}
bool WidgetRenderScope::hovered() const noexcept {
    return input_.hovered(node_.identity());
}
bool WidgetRenderScope::active() const noexcept {
    return input_.active(node_.identity());
}
...
double WidgetRenderScope::motion_progress(const std::string_view id,
...
double WidgetRenderScope::alpha() const noexcept {
    ...
    const double presentation_opacity =
        computed != nullptr ? computed->number(MotionProperty::opacity).value_or(visual_.opacity)
                            : visual_.opacity;
    return inherited_opacity_ * presentation_opacity * (enabled_ ? 1.0 : visual_.disabled_opacity);
}
...
const runtime::Value* WidgetRenderScope::style(const std::string_view name) const noexcept {
    ...
    if (layout != nullptr && layout->object() != nullptr) {
        if (const runtime::Value* field = layout->field(name); field != nullptr)
            return field;
    }
    return nullptr;
}
...
double WidgetRenderScope::number(const std::string_view name,
                                 const double fallback) const noexcept {
...
std::string WidgetRenderScope::string(const std::string_view name, std::string fallback) const {
...
double WidgetRenderScope::effective_number(const std::string_view controlled,
...
bool WidgetRenderScope::effective_boolean(const std::string_view controlled,
                                          const std::string_view retained_name,
                                          const std::string_view initial,
...
void WidgetRenderScope::border(const Rect bounds, RenderBorder border_value,
                               const std::optional<double> radius) {
    border_value.color = widget_opacity(border_value.color, alpha());
...
void WidgetRenderScope::shape(const Rect bounds, PathShape value) {
    if (value.fill.has_value())
        value.fill->multiply_alpha(alpha());
    if (value.stroke.has_value())
        value.stroke->multiply_alpha(alpha());
    output_.emplace_back(PathRenderCommand{bounds, std::move(value)});
}

void WidgetRenderScope::image(const Rect bounds, std::string image, RenderColor tint,
...
void WidgetRenderScope::nine_patch(const Rect bounds, std::string texture,
                                   const Edges source_insets, const Edges destination_insets,
                                   const TextureRegion source, RenderColor tint) {
...
void WidgetRenderScope::custom_mesh(const Rect bounds, std::string mesh, MeshGeometry geometry,
...
void WidgetRenderScope::blur_region(const Rect bounds, const double radius,
                                    const std::size_t downsample) {
    output_.emplace_back(BlurRegionRenderCommand{bounds, radius, downsample});
}

void WidgetRenderScope::shadow(const Rect bounds, const CornerRadii radii, RenderColor color,
...
void WidgetRenderScope::pop_clip() {
    output_.emplace_back(ClipPopRenderCommand{});
}
...
void WidgetRenderScope::text(const std::string_view value, const Point origin, RenderColor color,
                             const double alignment_width,
                             const WidgetTextAlignment horizontal_alignment,
                             const double alignment_height,
                             const WidgetTextAlignment vertical_alignment) {
    if (text_ == nullptr || value.empty())
        return;
    const TextLayout layout_value = text_->layout(node_, value);
    const font::ShapedText& shaped = layout_value.shaped;
    double x_offset = 0.0;
    if (horizontal_alignment == WidgetTextAlignment::center) {
        x_offset = std::max(0.0, (alignment_width - shaped.metrics.width) * 0.5);
    } else if (horizontal_alignment == WidgetTextAlignment::end) {
        x_offset = std::max(0.0, alignment_width - shaped.metrics.width);
    }
    double y_offset = 0.0;
    if (vertical_alignment == WidgetTextAlignment::center) {
        y_offset = std::max(0.0, (alignment_height - shaped.metrics.height) * 0.5);
    ...
    for (const TextResolvedRun& resolved_run : layout_value.resolved_runs) {
        ...
        for (std::size_t glyph_index = resolved_run.glyph_start_index;
             glyph_index < resolved_run.glyph_end_index; ++glyph_index) {
...
void WidgetRenderScope::focus(const Rect bounds) {
    if (!focus_visible() || !visual_.focus_ring.has_value() || visual_.focus_ring->width <= 0.0) {
        return;
    }
...
build_widget_fragment(const WidgetRegistry& registry, const RetainedNode& node,
