...
class WidgetSemanticsScope final {
  public:
    WidgetSemanticsScope(const RetainedNode& node, const CommandIndex* commands,
    ...
    [[nodiscard]] const runtime::ExpressionValue*
    expression_property(std::string_view name) const noexcept;
    ...
    [[nodiscard]] std::optional<std::string> effective_text(std::string_view controlled,
                                                            std::string_view retained_name,
                                                            std::string_view initial) const;
    ...
    [[nodiscard]] data::JsonValue
    virtual_item(std::size_t path_index, std::size_t path_base, std::string role, std::string name,
                 std::vector<std::string> actions = {}, std::optional<bool> checked = std::nullopt,
                 std::optional<bool> selected = std::nullopt, bool disabled = false,
                 ...
                 std::optional<bool> expanded = std::nullopt, std::string live_region = "off",
                 std::optional<std::uint64_t> notification_id = std::nullopt,
                 data::JsonValue value_range = {},
                 std::optional<std::string> value_text = std::nullopt) const;
    [[nodiscard]] data::JsonValue virtual_command(const CommandSnapshot& command, std::size_t index,
                                                  std::string_view role = "menu_item") const;
    [[nodiscard]] data::JsonValue
    virtual_tab(const runtime::Value& entry, std::string_view selected_id, std::size_t index) const;
