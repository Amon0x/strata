...
void static_text_selection(WidgetRenderScope& scope) {
    ...
    const std::size_t start = utf16_offset_for_utf8_byte(
        selection->text, selection->selection_start
    );
    const std::size_t end = utf16_offset_for_utf8_byte(
        selection->text, selection->selection_end
    );
...
void text_content(WidgetRenderScope& scope) {
    ...
    scope.node_text(
        Point{scope.layout().content_bounds.x, scope.layout().content_bounds.y},
        scope.visual().foreground
    );
}
...
void button_content(WidgetRenderScope& scope) {
    ...
    scope.focus(scope.layout().bounds);
    if (context_menu || scope.text_engine() == nullptr) return;
    ...
    scope.text(
        *value,
        ...
        scope.visual().foreground,
        scope.layout().content_bounds.width,
...
void draw_content(WidgetRenderScope& scope) {
    ...
    for (const runtime::Value& value : shapes->values) {
        ...
        if (shape.has_value()) scope.shape(scope.layout().bounds, *shape);
    }
}
...
void menu_overlay(WidgetRenderScope& scope) {
    ...
    for (const MenuRowModel& row : projection.rows()) {
        ...
        const bool selected = projection.active_path.size() >= row.path.size() &&
...
void status_content(WidgetRenderScope& scope) {
    ...
    if (!scope.boolean("showCommandFeedback", true) || scope.text_engine() == nullptr) return;
    ...
    scope.text(
        *feedback,
        ...
        std::max(0.0, scope.layout().bounds.width - 20.0),
        WidgetTextAlignment::end
    );
}
