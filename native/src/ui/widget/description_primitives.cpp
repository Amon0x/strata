...
void button_expand(WidgetDescriptionScope& scope) {
    static_cast<void>(scope.install_presentation(
        "presentationTemplate", WidgetTemplateArguments{
...
[[nodiscard]] bool retained_boolean(WidgetDescriptionScope& scope, const std::string_view property,
...
[[nodiscard]] std::vector<std::size_t> retained_menu_path(WidgetDescriptionScope& scope) {
    ...
    for (const runtime::Value& entry : value->list()->values) {
        if (entry.number() == nullptr || !std::isfinite(*entry.number()) || *entry.number() < 0.0) {
            break;
        }
...
append_children(const std::shared_ptr<const DescriptionNode>& source,
    ...
    merged.insert(merged.end(), std::make_move_iterator(children.begin()),
...
[[nodiscard]] std::shared_ptr<const DescriptionNode>
native_menu_row_footprint(WidgetDescriptionScope& scope, std::string key, const double width,
                          const double height) {
    DescriptionNode::Properties properties = widget_transparent_properties();
    properties.emplace("$inputTransparent", runtime::ExpressionValue(runtime::Value(true)));
    properties.emplace("$semanticTransparent", runtime::ExpressionValue(runtime::Value(true)));
    properties.emplace("$layout", runtime::ExpressionValue(widget_object({
...
[[nodiscard]] std::shared_ptr<const DescriptionNode>
native_menu_popup(WidgetDescriptionScope& scope, std::string key, const double width) {
    DescriptionNode::Properties properties = widget_transparent_properties();
    properties.emplace("$layout", runtime::ExpressionValue(widget_object({
...
popup_portal_layout(const DescriptionNode& popup, const std::string& anchor, const bool submenu,
    ...
    const runtime::Value* layout =
        authored != popup.properties.end() ? authored->second.data_value() : nullptr;
    for (const std::string_view name : {
        ...
        if (value == nullptr)
            continue;
        const auto found =
            std::ranges::find(fields, name, &std::pair<std::string, runtime::Value>::first);
        if (found != fields.end())
            found->second = *value;
        else
...
void menu_expand(WidgetDescriptionScope& scope) {
    ...
    const std::string* popup_component = widget_description_string(scope.property("popupTemplate"));
    const std::string* item_component = widget_description_string(scope.property("itemTemplate"));
    ...
    if (context) {
        ...
        if (x != nullptr && x->number() != nullptr && y != nullptr && y->number() != nullptr) {
    ...
    for (std::size_t level = 0U; !level_items->empty(); ++level) {
        ...
        for (std::size_t index = 0U; index < level_items->size(); ++index) {
            ...
            std::shared_ptr<const DescriptionNode> row =
                item_component != nullptr
                    ? scope.instantiate_component(
                          *item_component, row_key,
                          WidgetTemplateArguments{
                    ...
                    : native_menu_row_footprint(scope, row_key, menu_width, row_height);
            if (item_component == nullptr)
                scope.synthesized();
            if (row != nullptr)
                rows.push_back(std::move(row));
        }
        const std::string popup_key = key + ".popup.surface." + std::to_string(level);
        std::shared_ptr<const DescriptionNode> popup =
            popup_component != nullptr
                ? scope.instantiate_component(
                      *popup_component, popup_key,
                      WidgetTemplateArguments{
        ...
        portal_properties.insert_or_assign(
            ...
            runtime::ExpressionValue(popup_portal_layout(
                *popup, anchor, level != 0U, level == 0U ? pointer_anchor : std::nullopt)));
        description.children.push_back(scope.node("Panel", key + ".popup." + std::to_string(level),
        ...
        if (selected >= level_items->size() || (*level_items)[selected].children.empty()) {
            break;
        }
...
void section_expand(WidgetDescriptionScope& scope) {
    ...
    const runtime::Value content_gap =
        authored_gap != nullptr ? *authored_gap : runtime::Value(8.0);

    DescriptionNode::Properties header = widget_transparent_properties();
    header.emplace("$inputTransparent", runtime::ExpressionValue(runtime::Value(true)));
    header.emplace("semantics",
    ...
    content.emplace("$semanticTransparent", runtime::ExpressionValue(runtime::Value(true)));
    ...
    description.children.push_back(
        scope.node("Panel", std::nullopt, std::move(content), std::move(authored_children)));
    scope.synthesized(2U);

    description.properties.insert_or_assign("$disclosureDefaults",
    ...
    if (!description.properties.contains("animateContentSize")) {
        description.properties.emplace("$contentSizeMotionDefaults",
                                       runtime::ExpressionValue(widget_object({
...
[[nodiscard]] bool popup_participates(const RetainedNode& node) noexcept {
...
void popup_defaults(WidgetLayoutDefaultsScope& scope) {
    scope.set("detachFromParentClip", runtime::Value(true));
    scope.set("height", runtime::Value("content"));
    scope.set("kind", runtime::Value("PORTAL"));
    scope.set("portalTarget", runtime::Value("root"));
    scope.set("width", runtime::Value("content"));
    scope.set("zIndex", runtime::Value(20'000.0));
}

void popup_expand(WidgetDescriptionScope& scope) {
    // Popup placement is a framework invariant. Authored chrome may use any ordinary layout kind,
    // but it cannot turn the popup surface back into an in-flow child.
    scope.set_layout("kind", runtime::Value("PORTAL"));
    scope.set_layout("portalTarget", runtime::Value("root"));
    scope.set_layout("detachFromParentClip", runtime::Value(true));
    const runtime::Value* open = scope.property("open");
    if (open == nullptr || open->boolean() == nullptr) {
        open = scope.retained("$expanded");
    }
    if (open == nullptr || open->boolean() == nullptr) {
        open = scope.property("defaultOpen");
    }
    const bool expanded = open != nullptr && open->boolean() != nullptr && *open->boolean();
    scope.description().properties.insert_or_assign(
        "$layoutParticipates", runtime::ExpressionValue(runtime::Value(expanded)));
    if (!expanded)
...
void rich_text_expand(WidgetDescriptionScope& scope) {
    ...
    for (const runtime::Value* span : scope.list("spans")) {
        if (const std::string* value = widget_description_string(span->field("text"));
            value != nullptr) {
            text += *value;
        }
    }
...
void add(WidgetRegistry& registry, std::string type,
         const WidgetLayoutDefaultsHook defaults = nullptr,
         const WidgetDescriptionHook expand = nullptr, std::string canonical_type = {},
         std::string default_action = {}, std::string authored_presentation_property = {}) {
    WidgetDescribePhase phase{
        .layout_defaults = defaults,
        .expand = expand,
        .canonical_type = std::move(canonical_type),
        .default_action = std::move(default_action),
        .authored_presentation_property = std::move(authored_presentation_property),
    };
...
void register_primitive_widget_descriptions(WidgetRegistry& registry) {
    add(registry, "Button", &button_defaults, &button_expand, {}, {}, "presentationTemplate");
    ...
    add(registry, "Popup", &popup_defaults, &popup_expand);
    registry.register_participation("Popup", &popup_participates);
