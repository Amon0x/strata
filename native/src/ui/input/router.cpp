...
[[nodiscard]] bool retained_boolean(const RetainedNode& node, const std::string_view name,
...
[[nodiscard]] std::optional<bool> retained_optional_boolean(const RetainedNode& node,
...
[[nodiscard]] std::optional<std::int64_t> retained_deadline(const RetainedNode& node) noexcept {
...
InputRouter::InputRouter(
    std::string public_surface_id, std::string host_service_owner,
    ...
    NotificationService& notifications, SurfaceFrameworkExecutor surface_framework_executor,
    DescriptionInvalidator description_invalidator, HitBoundsResolver hit_bounds_resolver,
    TextOffsetResolver text_offset_resolver, TextWidthResolver text_width_resolver,
    ImeCursorRectResolver ime_cursor_rect_resolver, DragPreviewResolver drag_preview_resolver,
    InputProcessingConfig input_config, runtime::HostServices* const host_services,
    TextLayoutResolver text_layout_resolver, ScrollMutationObserver scroll_mutation_observer,
    FrameInvalidator frame_invalidator)
    : public_surface_id_(std::move(public_surface_id)),
      host_service_owner_(host_service_owner.empty() ? public_surface_id_
                                                     : std::move(host_service_owner)),
      application_(application), widgets_(widgets), behaviors_(behaviors),
      status_feedback_(status_feedback), notifications_(notifications),
...
bool InputRouter::node_input_enabled(const RetainedNode& node) const noexcept {
    ...
    if (commands_ != nullptr) {
        const std::optional<CommandActivationBinding> binding = commands_->activation_binding(node);
...
bool InputRouter::command_tooltip_candidate(const std::uint64_t identity) const noexcept {
    if (tree_ == nullptr || commands_ == nullptr)
        return false;
    ...
    for (const RetainedNode* ancestor = node->parent(); ancestor != nullptr;
        ...
        if (ancestor->description().type == "Tooltip")
            return false;
    }
    const std::optional<CommandActivationBinding> binding = commands_->activation_binding(*node);
    return binding.has_value() && binding->command != nullptr && !binding->command->label.empty();
}
...
bool InputRouter::tooltip_disclosures_need_frame() const noexcept {
    ...
    return std::ranges::any_of(*tooltips, [this](const RetainedNode* node) {
        if (node == nullptr || tooltip_controlled_visible(*node).has_value())
            return false;
        ...
        return shown != desired ||
               retained_optional_boolean(*node, tooltip_pending_state).has_value() ||
    ...
    for (RetainedNode* node : *tooltips) {
        ...
        if (pending != desired || !deadline.has_value()) {
            const std::int64_t delay =
                desired ? tooltip_show_delay_nanos(*node) : tooltip_hide_delay_nanos(*node);
...
bool InputRouter::hover_ready(const std::uint64_t identity,
                              const std::int64_t delay_nanos) const noexcept {
...
bool InputRouter::focusable(const RetainedNode& node) const noexcept {
    ...
    if (focusable && !lifecycle->input.focusable_when.empty()) {
        const runtime::Value* condition = scalar_property(node, lifecycle->input.focusable_when);
        focusable =
            condition != nullptr && condition->boolean() != nullptr && *condition->boolean();
    }
    for (const DescriptionBehavior& attachment : node.description().behaviors) {
        if (!attachment.enabled)
            continue;
...
bool InputRouter::passive_pointer_path(const RetainedNode& owner,
                                       const RetainedNode* pointer_target) const noexcept {
    for (const RetainedNode* current = pointer_target; current != nullptr && current != &owner;
...
RetainedNode* InputRouter::pointer_focusable_ancestor(RetainedNode* node) const noexcept {
    ...
    for (RetainedNode* current = node; current != nullptr; current = current->parent()) {
        ...
        for (const DescriptionBehavior& attachment : current->description().behaviors) {
            const BehaviorLifecycle* behavior =
                attachment.enabled ? behaviors_.find(attachment.id) : nullptr;
            owns_interaction = owns_interaction ||
                               (behavior != nullptr && !behavior->input.disabled &&
                                (behavior->input.accepts_pointer || behavior->input.focusable));
        }
        if (owns_interaction)
            return current;
        if (static_text_fallback == nullptr)
            static_text_fallback = current;
    }
    return static_text_fallback;
}

void InputRouter::apply_pointer_focus_default(const PointerInputEvent& event,
...
InputOperationResult InputRouter::prepare(RetainedTree& tree,
    ...
    const auto retain = [&tree](std::optional<std::uint64_t>& identity) {
        if (identity.has_value() && tree.find_identity(*identity) == nullptr)
            identity.reset();
    };
    retain(active_);
    ...
    if (active_subtarget_.has_value() && tree.find_identity(active_subtarget_->first) == nullptr) {
        active_subtarget_.reset();
    }
    if (scrollbar_drag_.has_value() && tree.find_identity(scrollbar_drag_->identity) == nullptr) {
    ...
    std::erase_if(drag_sessions_, [&tree](auto& entry) {
        ...
        if (tree.find_identity(session.source_identity) == nullptr)
            return true;
    ...
    for (auto& [identity, range] : static_text_ranges_) {
        ...
        const std::optional<std::string_view> text =
            node != nullptr ? static_text_value(*node) : std::nullopt;
        if (!text.has_value())
            continue;
        ...
        const StaticTextRange clamped{
            clamp_boundary(range.anchor),
            clamp_boundary(range.focus),
        };
    ...
    for (const std::string& type : widgets_.text_editable_types()) {
        ...
        for (RetainedNode* node : *editable_nodes) {
            ...
            if (!inserted)
                mutation = editor->second.reconcile_controlled(next);
...
InputOperationResult InputRouter::after_layout() {
    ...
    const auto reveal_virtual_item = [this, &result](const PendingReveal& reveal) {
        for (const auto& [identity, record] : layout_->records) {
            ...
            if (owner == nullptr ||
                (reveal.scroll_key.has_value() && owner->description().key != reveal.scroll_key)) {
                continue;
            }
            std::optional<std::size_t> item_index;
            if (record.virtual_items != nullptr) {
                item_index = record.virtual_items->index_of_key(reveal.key);
            } else if (const auto direct = std::ranges::find(record.virtual_item_keys, reveal.key);
                       direct != record.virtual_item_keys.end()) {
                item_index = record.virtual_item_key_start +
                             static_cast<std::size_t>(direct - record.virtual_item_keys.begin());
            }
            if (!item_index.has_value() && record.virtual_item_members != nullptr) {
                for (std::size_t index = 0U; index < record.virtual_item_members->size(); ++index) {
                    if (std::ranges::contains((*record.virtual_item_members)[index], reveal.key)) {
                        item_index = index;
                        break;
                    }
                }
            }
            if (!item_index.has_value() || *item_index >= record.virtual_item_extents->size()) {
                continue;
            }

            const bool vertical = *record.virtual_axis == LayoutAxis::vertical;
            const double viewport_extent =
                vertical ? record.viewport->height : record.viewport->width;
            const double leading_content_inset =
                vertical ? std::max(0.0, record.content_bounds.y - record.viewport->y)
                         : std::max(0.0, record.content_bounds.x - record.viewport->x);
            const double start =
                leading_content_inset + record.virtual_item_extents->start(*item_index);
            const double end = start + record.virtual_item_extents->extent(*item_index);
            const double padding = std::clamp(reveal.padding, 0.0, viewport_extent * 0.5);
    ...
    if (focused_.has_value() && ime_cursor_rect_resolver_) {
        ...
        const LayoutRecord* const record =
            node != nullptr ? layout_->find(node->identity()) : nullptr;
        const auto editor = node != nullptr ? editors_.find(node->identity()) : editors_.end();
        const WidgetLifecycle* const lifecycle =
            node != nullptr ? widgets_.find(node->description().type) : nullptr;
        const runtime::Value* const read_only =
            node != nullptr ? scalar_property(*node, "readOnly") : nullptr;
        const bool editable =
            lifecycle != nullptr && lifecycle->input.text_edit_mode != WidgetTextEditMode::none &&
            lifecycle->input.text_edit_mode != WidgetTextEditMode::static_text &&
            !(read_only != nullptr && read_only->boolean() != nullptr && *read_only->boolean());
...
void InputRouter::publish_motion(const MotionRuntime& motion) noexcept {
    motion_ = &motion;
}

void InputRouter::publish_commands(CommandIndex& commands) noexcept {
    commands_ = &commands;
}

void InputRouter::publish_reduced_motion(const bool reduced_motion) noexcept {
    reduced_motion_ = reduced_motion;
}

bool InputRouter::request_widget_frame(const RetainedNode& node, const WidgetFrameCost cost) {
    const WidgetLifecycle* const lifecycle = widgets_.find(node.description().type);
    if (node.lifecycle() != RetainedLifecycle::attached || !node_participates(node) ||
        lifecycle == nullptr || lifecycle->frame.advance == nullptr) {
        return false;
    }
    auto [request, inserted] =
        requested_widget_frames_.try_emplace(node.identity(), RequestedWidgetFrame{cost, true});
    if (!inserted) {
        request->second.cost = cost;
        request->second.pending = true;
    }
    widget_frame_times_.try_emplace(node.identity(), frame_time_nanos_);
    if (frame_invalidator_)
        frame_invalidator_();
    return true;
}

void InputRouter::cancel_widget_frame(const std::uint64_t identity) noexcept {
    if (advancing_widget_frame_identity_ == identity) {
        if (const auto request = requested_widget_frames_.find(identity);
            request != requested_widget_frames_.end()) {
            request->second.pending = false;
        }
    } else {
        requested_widget_frames_.erase(identity);
    }
    widget_frame_times_.erase(identity);
}
...
void InputRouter::publish_frame_time(const std::int64_t frame_time_nanos) {
    ...
    for (const auto& [identity, started] : hover_started_nanos_) {
        if (matured_command_tooltips_.contains(identity) || frame_time_nanos_ < started ||
...
void InputRouter::set_focus_visibility(const bool visible) {
    if (focus_highlight_visible_ == visible)
        return;
...
void InputRouter::focus(const RetainedNode& node, const std::string_view reason,
                        InputOperationResult& result) {
    if (inside_native_presentation(node))
        return;
    if (!within_focus_containment(node))
        return;
    if (focused_ == node.identity())
        return;
    if (focused_.has_value()) {
        if (RetainedNode* previous = tree_->find_identity(*focused_); previous != nullptr) {
            ...
            static_cast<void>(route_event(previous, nullptr, InputEventKind::blur, nullptr, nullptr,
    ...
    static_cast<void>(route_event(const_cast<RetainedNode*>(&node), nullptr, InputEventKind::focus,
                                  nullptr, nullptr, nullptr, nullptr, nullptr, result));
    if (reason != "pointer")
        reveal_focus(node, result);
    JsonValue event = object({
        {"action", JsonValue{}},
...
void InputRouter::clear_focus(const std::string_view reason, InputOperationResult& result,
...
void InputRouter::hover_route(const RetainedNode* target) {
    ...
    if (unchanged && route_size == hovered_.size())
        return;
...
InputRouter::activation_action(const RetainedNode& node, const std::string_view property) const {
    ...
    if (found != node.description().properties.end() && found->second.action() != nullptr) {
        ...
        if (value != nullptr && value->action != nullptr)
            return value;
        return nullptr;
    }
    ...
    if (lifecycle != nullptr && lifecycle->describe.default_action_factory != nullptr) {
        const std::shared_ptr<const runtime::ActionValue> resolved =
            lifecycle->describe.default_action_factory(node,
                                                       application_.bundle()->action_registry());
        if (resolved != nullptr && resolved->action != nullptr)
            return resolved;
    }
    std::string_view fallback_id =
        default_id != nullptr ? std::string_view(*default_id) : std::string_view{};
    if (fallback_id.empty()) {
        if (lifecycle != nullptr)
            fallback_id = lifecycle->input.fallback_action;
    }
    const auto resolved_contract =
        fallback_id.empty() ? nullptr
                            : application_.bundle()->action_registry().contract(fallback_id);
    if (resolved_contract == nullptr)
        return nullptr;
...
std::optional<std::uint64_t> InputRouter::focused_identity() const noexcept {
    return focused_;
}
...
bool InputRouter::within_focus_containment(const RetainedNode& node) const noexcept {
    if (!focus_containment_key_.has_value())
        return true;
    const RetainedNode* boundary = focus_boundary();
    return boundary != nullptr && descendant_of(node, *boundary);
}

bool InputRouter::set_focus_containment(const std::optional<std::string_view> key,
                                        InputOperationResult& result) {
    if (!key.has_value()) {
        if (!focus_containment_key_.has_value())
            return false;
    ...
    if (key->empty() || tree_ == nullptr)
        return false;
    RetainedNode* boundary = tree_->find_key(*key);
    if (boundary == nullptr)
        return false;
    if (!focus_containment_key_.has_value())
        focus_before_containment_ = focused_;
...
bool InputRouter::focused(const std::uint64_t identity) const noexcept {
    return focused_ == identity;
}
...
bool InputRouter::active(const std::uint64_t identity) const noexcept {
    return active_ == identity;
}
...
std::optional<TextEditorSnapshot>
InputRouter::editor_snapshot(const std::uint64_t identity) const noexcept {
    const auto found = editors_.find(identity);
...
std::optional<StaticTextSelectionSnapshot>
InputRouter::static_text_selection_snapshot(const std::uint64_t identity) const noexcept {
    ...
    const std::optional<std::string_view> text =
        node != nullptr ? static_text_value(*node) : std::nullopt;
    if (range == static_text_ranges_.end() || !text.has_value())
        return std::nullopt;
...
std::vector<std::string> InputRouter::pending_navigation_targets() const {
    ...
    for (const PendingReveal& reveal : pending_reveals_)
        result.push_back(reveal.key);
    return result;
}
...
NotificationService& InputRouter::notifications() noexcept {
