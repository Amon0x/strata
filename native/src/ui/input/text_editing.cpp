...
[[nodiscard]] TextInputFilter text_filter(const RetainedNode& node, const WidgetTextEditMode mode) {
    ...
    if (name == "integer")
        return TextInputFilter::integer;
    if (name == "decimal")
        return TextInputFilter::decimal;
    if (name == "letters")
        return TextInputFilter::letters;
    if (name == "alphanumeric")
        return TextInputFilter::alphanumeric;
    return mode == WidgetTextEditMode::numeric ? TextInputFilter::decimal : TextInputFilter::any;
}
...
[[nodiscard]] TextCommitFormat text_commit_format(const RetainedNode& node) {
    ...
    const std::string name = value != nullptr ? lower_ascii(*value) : std::string{};
    if (name == "trim")
        return TextCommitFormat::trim;
    if (name == "uppercase")
        return TextCommitFormat::uppercase;
    if (name == "lowercase")
        return TextCommitFormat::lowercase;
    return TextCommitFormat::none;
}

[[nodiscard]] TextEditorConfig editor_config(const RetainedNode& node,
    ...
    if (const runtime::Value* maximum = scalar_property(node, "maxLength");
        maximum != nullptr && maximum->number() != nullptr && std::isfinite(*maximum->number()) &&
        *maximum->number() >= 0.0 &&
...
[[nodiscard]] std::optional<double> number_field_value(const RetainedNode& node,
    ...
    else if (normalized.size() >= 2U && (normalized.front() == '+' || normalized.front() == '-') &&
             normalized[1] == '.') {
...
[[nodiscard]] const runtime::Value*
effective_number_field_value(const RetainedNode& node) noexcept {
    const runtime::Value* retained = node.retained_value("$value");
    if (retained != nullptr && retained->number() != nullptr)
        return retained;
...
[[nodiscard]] bool editor_reserves_key(const std::string_view key,
                                       const KeyModifiers modifiers) noexcept {
    if (command_key(modifiers)) {
        return key == "a" || key == "c" || key == "x" || key == "v" || key == "z" || key == "y";
    }
    if (modifiers.control || modifiers.super_key || modifiers.alt)
        return false;
    return key == "left" || key == "right" || key == "up" || key == "down" || key == "home" ||
           key == "end" || key == "backspace" || key == "delete" || key == "enter" ||
           key == "space" || key.size() == 1U;
}

} // namespace

void InputRouter::record_editor_mutation(RetainedNode& node, const TextEditorMutation& mutation,
    ...
    if (lifecycle != nullptr && !lifecycle->input.editor_emits_change)
        return;
    ...
    if (property_action != node.description().properties.end() &&
        property_action->second.action() != nullptr &&
        *property_action->second.action() != nullptr) {
        action = *property_action->second.action();
    }
    const bool numeric =
        lifecycle != nullptr && lifecycle->input.text_edit_mode == WidgetTextEditMode::numeric;
    if (action == nullptr) {
...
bool InputRouter::insert_editor_text(RetainedNode& node, const std::string_view value,
                                     InputOperationResult& result) {
    const auto editor = editors_.find(node.identity());
    if (editor == editors_.end())
        return false;
...
bool InputRouter::clear_editor_text(RetainedNode& node, InputOperationResult& result) {
    const auto editor = editors_.find(node.identity());
    if (editor == editors_.end())
        return false;
...
void InputRouter::synchronize_editor_text(RetainedNode& node, const std::string_view value,
    ...
    if (move_caret_to_end)
        mutation = mutation + editor->second.move_end(false);
    if (!mutation.changed())
        return;
    text_navigation_.erase(node.identity());
    if (tree_ != nullptr)
...
void InputRouter::commit_editor(RetainedNode& node, InputOperationResult& result) {
    ...
    if (text_read_only(node)) {
        ...
        mutation =
            editor->second.restore_controlled(runtime::display_string(runtime::Value(committed)));
    } else {
...
std::optional<std::size_t> InputRouter::visual_text_navigation_offset(const RetainedNode& node,
    ...
    const std::optional<std::size_t> preferred_line =
        retained != text_navigation_.end() && retained->second.caret == caret
            ...
            : std::nullopt;
    const std::size_t line = text_layout_caret_line(layout, utf16_caret, preferred_line);
    ...
    if (key == "home" || key == "end") {
        ...
        x_goal = retained != text_navigation_.end() && retained->second.caret == caret &&
        ...
        next_line = key == "up" ? (line == 0U ? 0U : line - 1U)
    ...
    text_navigation_.insert_or_assign(node.identity(),
                                      TextNavigationState{next, next_line, x_goal});
    return next;
}

InputOperationResult InputRouter::key(const std::string_view key_value,
...
InputOperationResult InputRouter::key(KeyInputEvent event, const bool navigation_traversal_repeat) {
    InputOperationResult result;
    ...
    // OS window-switching and modifier-only events are not keyboard navigation.
    if (event.type != KeyEventType::release && !modifiers.alt && !modifiers.super_key &&
        normalized_key != "alt" && normalized_key != "super" && normalized_key != "control" &&
        normalized_key != "shift") {
        set_focus_visibility(true);
    }
    RetainedNode* node =
    ...
    if (event.type == KeyEventType::release)
        return result;
    if (normalized_key == "escape" && dismiss_topmost(result))
        return result;
    if (normalized_key == "escape" && cancel_active_drag(result))
        return result;
    if ((event.type == KeyEventType::press ||
         (event.type == KeyEventType::repeat && navigation_traversal_repeat)) &&
        normalized_key == "tab" && !modifiers.alt && !modifiers.control && !modifiers.super_key) {
        static_cast<void>(traverse_focus(modifiers.shift, result));
        return result;
    }
    const bool editing_owns_key =
        focused_.has_value() && tree_ != nullptr &&
        ((editors_.contains(*focused_) && editor_reserves_key(normalized_key, modifiers)) ||
         (node != nullptr && static_text_selectable(*node) &&
          (normalized_key == "left" || normalized_key == "right" || normalized_key == "up" ||
           normalized_key == "down" || normalized_key == "home" || normalized_key == "end" ||
           (command_key(modifiers) &&
            (normalized_key == "a" || normalized_key == "c" || normalized_key == "x" ||
             normalized_key == "v" || normalized_key == "z" || normalized_key == "y")))));
    if (route_command_shortcut(normalized_key, modifiers, event.type, editing_owns_key, result)) {
        return result;
    }
    if (!editing_owns_key && command_key(modifiers) &&
        ...
        if (changed && label.has_value()) {
            status_feedback_.publish(std::string(redo ? "Redo " : "Undo ") + *label);
        }
        return result;
    }
    if (node == nullptr)
    ...
    if (editor != editors_.end()) {
        for (RetainedNode* current = node; current != nullptr; current = current->parent()) {
            ...
            if (current_lifecycle->input.editor_key(scope))
                return result;
        }
        ...
        if (command && key == "a") {
            ...
            if (!read_only)
                mutation = modifiers.shift ? editor->second.redo() : editor->second.undo();
            editor_handled = true;
        } else if (command && key == "y") {
            if (!read_only)
                mutation = editor->second.redo();
            editor_handled = true;
        } else if (key == "left") {
            text_navigation_.erase(node->identity());
            ...
            const std::optional<std::size_t> visual =
                config.multiline && !command
                    ? visual_text_navigation_offset(*node, editor->second.text(),
                                                    editor->second.snapshot().caret, key)
                    : std::nullopt;
            mutation = visual.has_value() ? editor->second.place_caret(*visual, modifiers.shift)
            ...
            if (!read_only)
                mutation = editor->second.backspace(command, frame_time_nanos_);
            editor_handled = true;
        } else if (key == "delete") {
            if (!read_only)
                mutation = editor->second.delete_forward(command, frame_time_nanos_);
            editor_handled = true;
        } else if (key == "enter") {
    ...
    if (!editor_handled) {
        editor_handled = move_static_text_selection(*node, normalized_key, modifiers, result);
    }
    ...
    if (!editor_handled) {
        ...
        for (RetainedNode* current = node; current != nullptr && !widget_handled;
            ...
            const WidgetLifecycle* current_lifecycle = widgets_.find(current->description().type);
            if (current_lifecycle == nullptr || current_lifecycle->input.key == nullptr)
                continue;
            InputDispatchContext context(*this, *current, node, nullptr,
                                         current == node ? InputEventPhase::target
                                                         : InputEventPhase::bubble,
                                         InputEventKind::key, dispatch, nullptr, nullptr, &event);
            WidgetInputScope scope(*this, *current, result, normalized_key, modifiers, nullptr,
                                   nullptr, 0U, std::nullopt, {}, &context);
        ...
        } else if (!widget_handled && (normalized_key == "left" || normalized_key == "right" ||
...
InputOperationResult InputRouter::text(TextInputEvent event) {
    ...
    if (node == nullptr)
        return result;
    const InputDispatchState dispatch = route_event(node, nullptr, InputEventKind::text, nullptr,
                                                    nullptr, nullptr, &event, nullptr, result);
    if (dispatch.consumed)
        return result;
    for (RetainedNode* current = node; current != nullptr; current = current->parent()) {
        const WidgetLifecycle* lifecycle = widgets_.find(current->description().type);
        if (lifecycle == nullptr || lifecycle->input.text == nullptr)
            continue;
        WidgetInputScope scope(*this, *current, result, {}, {}, nullptr, nullptr, 0U, std::nullopt,
                               event.text);
        if (lifecycle->input.text(scope))
            return result;
    }
...
InputOperationResult InputRouter::ime_preedit(std::string value, const std::size_t selection_start,
...
InputOperationResult InputRouter::ime_preedit(ImePreeditInputEvent event) {
    ...
    if (node == nullptr)
        return result;
    const InputDispatchState dispatch =
        route_event(node, nullptr, InputEventKind::ime_preedit, nullptr, nullptr, nullptr, nullptr,
                    &event, result);
    if (dispatch.consumed)
        return result;
    const auto editor = editors_.find(node->identity());
    ...
    const TextEditorMutation mutation =
        text_read_only(*node)
            ? editor->second.cancel_preedit()
            : editor->second.set_preedit(std::move(event.text), event.selection_start,
                                         event.selection_end, config.multiline);
    record_editor_mutation(*node, mutation, result);
    return result;
}
...
bool InputRouter::copy_static_text_selection(RetainedNode& owner) {
    ...
    const std::vector<RetainedNode*> nodes =
        container.has_value() ? static_text_nodes(*container) : std::vector<RetainedNode*>{&owner};
...
bool InputRouter::select_all_static_text(RetainedNode& owner, InputOperationResult& result) {
    if (!static_text_selectable(owner))
        return false;
    const std::optional<std::string_view> container = static_text_container(owner);
    ...
    if (!other_selections.empty() && frame_invalidator_)
        frame_invalidator_();
