...
[[nodiscard]] double edge_velocity(const double position, const double start, const double end,
                                   const InputProcessingConfig& config) noexcept {
    double strength = 0.0;
    if (position < start + config.drag_auto_scroll_edge) {
        strength = -std::clamp((start + config.drag_auto_scroll_edge - position) /
                                   config.drag_auto_scroll_edge,
                               0.0, 1.0);
    } else if (position > end - config.drag_auto_scroll_edge) {
        strength = std::clamp((position - (end - config.drag_auto_scroll_edge)) /
                                  config.drag_auto_scroll_edge,
                              0.0, 1.0);
    }
...
std::vector<std::string> InputRouter::drag_operations(const runtime::Value* value,
    ...
    for (const runtime::Value& entry : value->list()->values) {
        ...
        if (operation != "move" && operation != "copy" && operation != "link")
            continue;
        if (!std::ranges::contains(parsed, operation))
            parsed.push_back(std::move(operation));
    }
    return parsed.empty() ? std::move(fallback) : std::move(parsed);
}

InputRouter::DragTarget InputRouter::resolve_drag_target(const DragSession& session,
    ...
    while (node != nullptr) {
        ...
        if (record != nullptr && contains(record->bounds, position) &&
            motion_input_eligible(*node) && (modal == nullptr || descendant_of(*node, *modal))) {
            if (const DescriptionBehavior* attachment = behavior(*node, "strata.drop-target");
                ...
                const bool accepts_type =
                    accepted != nullptr && accepted->list() != nullptr &&
                    std::ranges::any_of(accepted->list()->values, [&session](
                                                                      const runtime::Value& value) {
                        return value.string() != nullptr && *value.string() == session.payload_type;
                    });
                if (accepts_type) {
                    const std::vector<std::string> target_operations = drag_operations(
                        attachment->options.field("allowedOperations"), {"move", "copy", "link"});
                    const auto selected =
                        std::ranges::find_first_of(session.allowed_operations, target_operations);
                    if (selected != session.allowed_operations.end()) {
                        std::string placement = "on";
                        const runtime::Value* placement_mode =
                            attachment->options.field("placementMode");
                        ...
                        if (placement == "on" && axis != nullptr && axis->string() != nullptr) {
                            ...
                            if (normalized == "horizontal") {
                                placement =
                                    position.x < record->bounds.x + record->bounds.width * 0.5
                                        ? "before"
                                        : "after";
                            } else if (normalized == "vertical") {
                                placement =
                                    position.y < record->bounds.y + record->bounds.height * 0.5
                                        ? "before"
                                        : "after";
                            }
                        }
...
InputRouter::DragTarget InputRouter::retained_drag_target(const DragSession& session) const {
    ...
    if (attachment == nullptr)
        attachment = behavior(*target, "strata.reorder-target");
    return attachment != nullptr
               ? DragTarget{target, attachment, *session.operation, session.placement}
               : DragTarget{};
}

void InputRouter::emit_drag_lifecycle(DragSession& session, const std::string_view phase,
                                      const DragTarget& target, const Point position,
                                      InputOperationResult& result) {
    if (tree_ == nullptr)
        return;
    RetainedNode* source_node = tree_->find_identity(session.source_identity);
    const DescriptionBehavior* source_behavior =
        source_node != nullptr ? behavior(*source_node, "strata.drag-source") : nullptr;
    ...
    if (reorder && layout_ != nullptr) {
        ...
        const bool horizontal =
            axis != nullptr && axis->string() != nullptr &&
            (*axis->string() == "HORIZONTAL" || *axis->string() == "horizontal");
        insertion = resolve_reorder_insertion(*target.node, *layout_, position, horizontal);
    }
    const auto event = [this, &allowed, &drag_source, &payload, phase, position, &target,
                        &insertion, has_target](const RetainedNode& recipient,
    ...
    emit(event(*source_node, *source_behavior, "source"), source_behavior->action, *source_node,
         event_value, result);
    if (has_target) {
        emit(event(*target.node, *target.behavior, "target"), target.behavior->action, *target.node,
             event_value, result);
    }
}

void InputRouter::synchronize_drag_target(DragSession& session, const Point position,
                                          const bool emit_move, InputOperationResult& result) {
    if (!session.active || tree_ == nullptr)
        return;
    const DragTarget next = resolve_drag_target(session, position);
    const DragTarget previous = retained_drag_target(session);
    const bool identity_changed =
        next.node != previous.node || next.operation != previous.operation;
    ...
    session.target_identity =
        next.node != nullptr ? std::optional(next.node->identity()) : std::nullopt;
    session.operation = next.node != nullptr ? std::optional(next.operation) : std::nullopt;
...
bool InputRouter::auto_scroll_drag(DragSession& session, InputOperationResult& result) {
    if (!session.active || tree_ == nullptr || layout_ == nullptr)
        return false;
    ...
    if (!previous.has_value())
        return false;
    const std::int64_t elapsed_nanos = std::clamp(frame_time_nanos_ - *previous, std::int64_t{0},
    ...
    for (RetainedNode* current = widget_native_input_owner(hit_test(session.position));
        ...
        if (style.scroll_horizontal) {
            next.x += edge_velocity(session.position.x, record->viewport->x,
                                    record->viewport->right(), input_config_) *
                      elapsed_seconds;
        }
        if (style.scroll_vertical) {
            next.y += edge_velocity(session.position.y, record->viewport->y,
                                    record->viewport->bottom(), input_config_) *
                      elapsed_seconds;
        }
        next.x = std::clamp(next.x, 0.0, limits->x);
        next.y = std::clamp(next.y, 0.0, limits->y);
        if (next.x == record->scroll_offset.x && next.y == record->scroll_offset.y)
            continue;
        if (!set_scroll_offset(*current, next, result))
            continue;
...
void InputRouter::advance_widget_frames(InputOperationResult& result) {
    for (auto iterator = requested_widget_frames_.begin();
         iterator != requested_widget_frames_.end();) {
        const auto current = iterator++;
        const std::uint64_t identity = current->first;
        RetainedNode* const node = tree_ != nullptr ? tree_->find_identity(identity) : nullptr;
        ...
        if (node == nullptr || node->lifecycle() != RetainedLifecycle::attached ||
            layout_ == nullptr || layout_->find(identity) == nullptr || !node_participates(*node) ||
            lifecycle == nullptr || lifecycle->frame.advance == nullptr) {
            widget_frame_times_.erase(identity);
            requested_widget_frames_.erase(current);
            continue;
        }
        const auto requested_at = widget_frame_times_.find(identity);
        if (requested_at != widget_frame_times_.end() &&
            requested_at->second == frame_time_nanos_) {
            continue;
        }
        const std::int64_t delta =
            requested_at != widget_frame_times_.end() && frame_time_nanos_ >= requested_at->second
                ? frame_time_nanos_ - requested_at->second
                : 0;
        widget_frame_times_.insert_or_assign(identity, frame_time_nanos_);
        const WidgetFrameCost cost = current->second.cost;
        current->second.pending = false;
        WidgetInputScope scope(*this, *node, result, {}, {});
        advancing_widget_frame_identity_ = identity;
        try {
            lifecycle->frame.advance(scope,
                                     WidgetFrameInfo{frame_time_nanos_, delta, reduced_motion_});
        } catch (...) {
            advancing_widget_frame_identity_.reset();
            widget_frame_times_.erase(identity);
            requested_widget_frames_.erase(current);
            throw;
        }
        advancing_widget_frame_identity_.reset();
        static_cast<void>(tree_->mark(
            identity, cost == WidgetFrameCost::layout ? DirtyReason::layout : DirtyReason::paint));
        if (reduced_motion_)
            current->second.pending = false;
        if (!current->second.pending) {
            widget_frame_times_.erase(identity);
            requested_widget_frames_.erase(current);
        }
    }
