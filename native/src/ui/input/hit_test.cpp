...
[[nodiscard]] std::size_t text_boundary(
...
[[nodiscard]] std::size_t previous_text_boundary(
    const std::string_view text,
...
[[nodiscard]] std::size_t next_text_boundary(
    const std::string_view text,
    const std::size_t offset
) noexcept {
    std::size_t result = text_boundary(text, offset);
    if (result >= text.size()) return text.size();
    ++result;
...
[[nodiscard]] int text_word_class(
    const std::string_view text,
    const std::size_t offset
) noexcept {
    if (offset >= text.size()) return -1;
...
[[nodiscard]] std::pair<std::size_t, std::size_t> word_range(
    const std::string_view text,
    const std::size_t offset
) noexcept {
    if (text.empty()) return {0U, 0U};
    const std::size_t anchor = text_boundary(text, offset);
    const std::size_t probe = anchor < text.size()
        ? anchor
        : previous_text_boundary(text, anchor);
    ...
    while (start > 0U) {
        ...
        if (text_word_class(text, previous) != kind) break;
        start = previous;
    }
...
[[nodiscard]] std::size_t text_line_start(
    const std::string_view text,
    const std::size_t offset
) noexcept {
    const std::size_t clamped = text_boundary(text, offset);
    const std::size_t newline = clamped == 0U
        ? std::string_view::npos
        : text.rfind('\n', clamped - 1U);
    return newline == std::string_view::npos ? 0U : newline + 1U;
}

[[nodiscard]] std::size_t text_line_end(
    const std::string_view text,
    const std::size_t offset
) noexcept {
...
InputDispatchContext::InputDispatchContext(
    InputRouter& router,
    RetainedNode& node,
    RetainedNode* const target,
    RetainedNode* const pointer_target,
    const InputEventPhase phase,
    const InputEventKind kind,
    InputDispatchState& state,
    const PointerInputEvent* const pointer,
    const ScrollInputEvent* const scroll,
    const KeyInputEvent* const key,
    const TextInputEvent* const text,
    const ImePreeditInputEvent* const ime_preedit
...
RetainedNode& InputDispatchContext::node() noexcept { return node_; }
const RetainedNode& InputDispatchContext::node() const noexcept { return node_; }
RetainedNode* InputDispatchContext::target() const noexcept { return target_; }
RetainedNode* InputDispatchContext::pointer_target() const noexcept { return pointer_target_; }
InputEventPhase InputDispatchContext::phase() const noexcept { return phase_; }
InputEventKind InputDispatchContext::kind() const noexcept { return kind_; }
const PointerInputEvent* InputDispatchContext::pointer() const noexcept { return pointer_; }
const ScrollInputEvent* InputDispatchContext::scroll() const noexcept { return scroll_; }
const KeyInputEvent* InputDispatchContext::key() const noexcept { return key_; }
const TextInputEvent* InputDispatchContext::text() const noexcept { return text_; }
const ImePreeditInputEvent* InputDispatchContext::ime_preedit() const noexcept {
    return ime_preedit_;
}
bool InputDispatchContext::consumed() const noexcept { return state_.consumed; }
...
std::optional<Point> InputDispatchContext::press_origin() const noexcept {
    if (pointer_ == nullptr) return std::nullopt;
    ...
    return found != router_.pressed_pointer_targets_.end()
        ? std::optional(found->second.position)
        : std::nullopt;
}
...
GestureClaimState InputDispatchContext::gesture_claim_state() const noexcept {
    if (pointer_ == nullptr) return GestureClaimState::unclaimed;
    const auto found = router_.pressed_pointer_targets_.find(pointer_->pointer_id);
    return found != router_.pressed_pointer_targets_.end()
        ? found->second.gesture
...
std::optional<std::uint64_t> InputDispatchContext::gesture_claim_owner() const noexcept {
    ...
    const auto found = router_.pressed_pointer_targets_.find(pointer_->pointer_id);
    return found != router_.pressed_pointer_targets_.end()
        ? found->second.gesture_owner
        : std::nullopt;
}

void InputDispatchContext::consume() noexcept { state_.consumed = true; }
...
void InputRouter::place_pointer_caret(
    RetainedNode& node,
    ...
    if (static_text_selectable(node)) {
        ...
        const std::optional<std::size_t> offset = resolve_text_offset(
            node, *text, event.position
        );
        ...
        if (clicks >= 3U) {
            set_static_text_selection(
                node, text_line_start(*text, focus), text_line_end(*text, focus), result
            );
        } else if (clicks == 2U) {
    ...
    const std::optional<std::size_t> offset = resolve_text_offset(
        node, editor->second.text(), event.position
    );
    if (!offset.has_value()) return;
    ...
    const TextEditorMutation mutation = clicks >= 3U
                                            ? editor->second.select_line_at(*offset)
                                        : clicks == 2U
    ...
    seed_pointer_text_navigation(
        node, editor->second.text(), editor->second.snapshot().caret, event.position
    );
}

std::optional<std::size_t> InputRouter::resolve_text_offset(
    ...
    if (static_text_node(node) && text_layout_resolver_) {
        const TextLayout visual = text_layout_resolver_(
            node, text, TextLayoutOptions{}
        );
    ...
    if (!text_offset_resolver_) return std::nullopt;

    return text_offset_resolver_(node, *layout, text, position);
}

Point InputRouter::logical_pointer_position(
    const RetainedNode& node,
    Point position
) const {
    if (motion_ != nullptr) {
...
void InputRouter::seed_pointer_text_navigation(
    const RetainedNode& node,
    const std::string_view text,
    ...
    if (record == nullptr || !text_layout_resolver_) return;
    const TextLayout visual = text_layout_resolver_(node, text, TextLayoutOptions{});
    if (visual.lines.empty()) return;
...
bool InputRouter::static_text_selectable(const RetainedNode& node) const noexcept {
    if (!static_text_node(node)) return false;
...
RetainedNode* InputRouter::selectable_static_text_owner(
    RetainedNode* hit
) const noexcept {
...
std::optional<std::string_view> InputRouter::static_text_value(
    const RetainedNode& node
) const noexcept {
    const runtime::Value* value = scalar_property(node, "text");
    if (value == nullptr || value->string() == nullptr) value = node.retained_value("$text");
    return value != nullptr && value->string() != nullptr
        ? std::optional<std::string_view>(*value->string())
        : std::nullopt;
}

std::optional<std::string_view> InputRouter::static_text_container(
    const RetainedNode& node
) const noexcept {
    ...
    return value != nullptr && value->string() != nullptr && !value->string()->empty()
        ? std::optional<std::string_view>(*value->string()) : std::nullopt;
}

std::vector<RetainedNode*> InputRouter::static_text_nodes(
    const std::string_view container
) const {
    std::vector<RetainedNode*> result;
    if (tree_ == nullptr || tree_->root() == nullptr || container.empty()) return result;
...
void InputRouter::begin_static_text_selection(
    RetainedNode& node,
    InputOperationResult& result
) {
    ...
    if (selected == static_text_ranges_.end()) return;
    ...
    for (const auto& [identity, range] : static_text_ranges_) {
        ...
        if (identity == node.identity()) continue;
...
bool InputRouter::static_text_selection_owner_matches(
    const RetainedNode& node
) const noexcept {
    if (!static_text_selection_.has_value()) return false;
    const std::optional<std::string_view> container = static_text_container(node);
...
void InputRouter::transition_static_text_selection_owner(
    RetainedNode* const owner,
    InputOperationResult& result
) {
    static_cast<void>(result);
    const bool same_owner = owner != nullptr && static_text_selection_owner_matches(*owner);
    const std::optional<std::string_view> owner_container = owner != nullptr
        ? static_text_container(*owner)
        : std::nullopt;
    bool changed = false;
    for (auto range = static_text_ranges_.begin(); range != static_text_ranges_.end();) {
        ...
        const bool belongs = owner != nullptr && node != nullptr &&
            (owner_container.has_value()
                 ? static_text_container(*node) == owner_container
                 : node->identity() == owner->identity());
        if (belongs) {
            ++range;
            continue;
        }
        if (node != nullptr) static_cast<void>(tree_->mark(node->identity(), DirtyReason::input));
        text_navigation_.erase(range->first);
        range = static_text_ranges_.erase(range);
        changed = true;
    }
    if (!same_owner) static_text_selection_.reset();
    if (changed && frame_invalidator_) frame_invalidator_();
}

void InputRouter::extend_static_text_selection(
    ...
    if (!focus_text.has_value()) return;
    const std::optional<std::size_t> focus_offset = resolve_text_offset(
        candidate, *focus_text, position
    );
    if (!focus_offset.has_value()) return;
    const std::vector<RetainedNode*> nodes = static_text_selection_->container.has_value()
        ? static_text_nodes(*static_text_selection_->container)
        : std::vector<RetainedNode*>{
              tree_ != nullptr
                  ? tree_->find_identity(static_text_selection_->anchor_identity)
                  : nullptr,
          };
    if (nodes.empty() || nodes.front() == nullptr) return;
    const auto anchor = std::ranges::find(
        nodes, static_text_selection_->anchor_identity, &RetainedNode::identity
    );
    const auto focus = std::ranges::find(nodes, candidate.identity(), &RetainedNode::identity);
    if (anchor == nodes.end() || focus == nodes.end()) return;
    ...
    for (std::size_t index = 0U; index < nodes.size(); ++index) {
        ...
        if (start == end && index != anchor_index && index != focus_index) {
            ...
            const StaticTextRange next{
                text_boundary(*text, start), text_boundary(*text, end),
            };
            const auto current = static_text_ranges_.find(identity);
            if (current == static_text_ranges_.end() ||
                current->second.anchor != next.anchor ||
...
void InputRouter::set_static_text_selection(
    RetainedNode& node,
    ...
    static_cast<void>(result);
    const std::optional<std::string_view> text = static_text_value(node);
    if (!text.has_value() || !static_text_selectable(node)) return;
    ...
    if (tree_ != nullptr) static_cast<void>(tree_->mark(node.identity(), DirtyReason::input));
    if (frame_invalidator_) frame_invalidator_();
}

bool InputRouter::move_static_text_selection(
    RetainedNode& node,
    const std::string_view key,
    const KeyModifiers modifiers,
    InputOperationResult& result
) {
    if (!static_text_selectable(node)) return false;
    ...
    const bool navigation = key == "left" || key == "right" || key == "up" ||
        key == "down" || key == "home" || key == "end";
    if (!navigation || modifiers.alt) return false;
    ...
    const auto current = static_text_ranges_.find(node.identity());
    const std::size_t old_anchor = current != static_text_ranges_.end()
        ? current->second.anchor
        : 0U;
    const std::size_t old_focus = current != static_text_ranges_.end()
    ...
    if (key == "left") {
        ...
        const std::optional<std::size_t> visual = visual_text_navigation_offset(
            node, *text, old_focus, key
        );
        if (!visual.has_value()) return true;
        next = *visual;
    }
    if (!modifiers.shift) {
        bool cleared = false;
        ...
        if (cleared && frame_invalidator_) frame_invalidator_();
        const std::optional<std::string_view> container = static_text_container(node);
        static_text_selection_ = StaticTextSelectionSession{
            node.identity(),
            next,
...
void InputRouter::prepare_pointer_geometry() const {
    ...
    const auto visit = [this](
        ...
        std::ranges::stable_sort(
            children,
            [this](const RetainedNode* left, const RetainedNode* right) {
...
const std::vector<WidgetSubtarget>& InputRouter::projected_subtargets(
    const RetainedNode& node
) const {
    ...
    return projected_subtargets_.emplace(
        ...
        widget_subtargets(
            ...
            *layout_,
            commands_,
            text_width_resolver_,
            text_layout_resolver_,
            &notifications_,
...
void InputRouter::prepare_detached_subtargets() const {
    prepare_pointer_geometry();
    if (!pointer_geometry_ready_ || detached_subtargets_ready_) return;
    const std::vector<DetachedOverlayRoot> overlays = detached_overlay_roots(
        *tree_,
        *layout_,
        [this](const RetainedNode& node) {
            if (!widget_projects_detached_subtargets(node.description().type)) return false;
            return std::ranges::any_of(
                projected_subtargets(node), &WidgetSubtarget::detached
            );
        }
    );
    for (auto root = overlays.rbegin(); root != overlays.rend(); ++root) {
        ...
        for (const WidgetSubtarget& target : projected) {
            if (target.detached) detached.push_back(&target);
        }
        std::ranges::stable_sort(
            detached,
            {},
...
RetainedNode* InputRouter::hit_test(const Point position) const noexcept {
    ...
    for (const PointerHitEntry& entry : pointer_hit_entries_) {
        if ((!entry.traversal_clip.has_value() ||
             contains(*entry.traversal_clip, position)) &&
...
bool InputRouter::subtarget_hovered(
...
bool InputRouter::subtarget_active(
    const std::uint64_t identity,
    const std::string_view id
) const noexcept {
...
std::optional<WidgetSubtarget> InputRouter::hit_subtarget(
    const Point position,
    const RetainedNode* const ordinary_target
) const {
    ...
    for (const RetainedNode* current = ordinary_target; current != nullptr;
        ...
        for (auto candidate = projected.rbegin(); candidate != projected.rend(); ++candidate) {
            if (candidate->detached || !contains(candidate->bounds, position)) continue;
...
void InputRouter::set_hovered_subtarget(const std::optional<WidgetSubtarget>& target) {
    const std::optional<std::pair<std::uint64_t, std::string>> next = target.has_value()
        ? std::optional(std::pair(target->owner_identity, target->id))
        : std::nullopt;
    const std::optional<std::uint64_t> next_notification = target.has_value()
        ? target->notification_id
        : std::nullopt;
    if (next == hovered_subtarget_ && next_notification == hovered_notification_id_) return;
    ...
    if (tree_ != nullptr && hovered_subtarget_.has_value()) {
        static_cast<void>(tree_->mark(hovered_subtarget_->first, DirtyReason::input));
    }
...
RetainedNode* InputRouter::interactive_ancestor(RetainedNode* node) const noexcept {
    for (RetainedNode* current = node; current != nullptr; current = current->parent()) {
        ...
        for (const DescriptionBehavior& attachment : current->description().behaviors) {
            const BehaviorLifecycle* behavior_lifecycle = attachment.enabled
...
InputDispatchState InputRouter::route_event(
    ...
    InputDispatchState state;
    if (target == nullptr) return state;
    std::vector<RetainedNode*> route;
    for (RetainedNode* current = target; current != nullptr; current = current->parent()) {
        route.push_back(current);
    }
    const auto dispatch = [this, &result, target, pointer_target, kind, pointer, scroll,
                           key, text, ime_preedit, &state](
                              RetainedNode& node,
                              const InputEventPhase phase
                          ) {
        ...
        InputDispatchContext context(
            *this, node, target, pointer_target, phase, kind, state,
            pointer, scroll, key, text, ime_preedit
        );
        for (const DescriptionBehavior& attachment : node.description().behaviors) {
            const BehaviorLifecycle* lifecycle = attachment.enabled
                ? behaviors_.find(attachment.id)
                : nullptr;
            if (lifecycle == nullptr || lifecycle->input.disabled) continue;
            const bool has_typed_hook = pointer != nullptr
                ? lifecycle->input.pointer != nullptr
                : key != nullptr && key->type != KeyEventType::release &&
                    lifecycle->input.key != nullptr;
            if (lifecycle->input.event == nullptr && !has_typed_hook) continue;
            BehaviorInputScope scope(context, attachment, result);
            if (lifecycle->input.event != nullptr) {
                if (lifecycle->input.event(scope)) context.stop_propagation();
            }
            if (context.propagation_stopped()) return;
            if (pointer != nullptr && lifecycle->input.pointer != nullptr) {
                if (lifecycle->input.pointer(scope)) context.stop_propagation();
            } else if (key != nullptr && key->type != KeyEventType::release &&
                       lifecycle->input.key != nullptr) {
                if (lifecycle->input.key(scope)) context.stop_propagation();
            }
            if (context.propagation_stopped()) return;
        }
        const WidgetLifecycle* widget = widgets_.find(node.description().type);
        if (widget == nullptr || widget->input.event == nullptr) return;
        WidgetInputScope scope(
            *this,
            node,
            result,
            key != nullptr ? std::string_view(key->key) : std::string_view{},
            ...
            0U,
            routed_subtarget_,
            text != nullptr ? std::string_view(text->text) : std::string_view{},
            &context
        );
        if (widget->input.event(scope)) context.stop_propagation();
    };
    for (auto current = route.rbegin(); current != route.rend(); ++current) {
        if (*current == target) break;
        dispatch(**current, InputEventPhase::capture);
        if (state.propagation_stopped) return state;
    }
    dispatch(*target, InputEventPhase::target);
    if (state.propagation_stopped) return state;
    for (RetainedNode* current = target->parent(); current != nullptr; current = current->parent()) {
...
InputDispatchState InputRouter::route_pointer_event(
    RetainedNode* const target,
    RetainedNode* const pointer_target,
    const PointerInputEvent& event,
    InputOperationResult& result
    ...
    switch (event.type) {
    case PointerEventType::press: kind = InputEventKind::pointer_press; break;
    case PointerEventType::release: kind = InputEventKind::pointer_release; break;
    case PointerEventType::cancel: kind = InputEventKind::pointer_cancel; break;
    ...
    return route_event(
        target, pointer_target, kind, &event, nullptr, nullptr, nullptr, nullptr, result
    );
}

InputDispatchState InputRouter::route_key_event(
    RetainedNode* const target,
    const KeyInputEvent& event,
    InputOperationResult& result
) {
    return route_event(
        target, nullptr, InputEventKind::key, nullptr, nullptr, &event, nullptr, nullptr, result
    );
}

bool InputRouter::route_widget_pointer(
    RetainedNode* target,
    RetainedNode* const pointer_target,
    const PointerInputEvent& event,
    ...
    for (RetainedNode* current = target; current != nullptr; current = current->parent()) {
        const WidgetLifecycle* lifecycle = widgets_.find(current->description().type);
        if (lifecycle == nullptr || lifecycle->input.pointer == nullptr) continue;
        InputDispatchContext context(
            *this,
            *current,
            target,
        ...
        if (context.propagation_stopped()) return true;
    }
    return false;
}

void InputRouter::route_active_lifecycle_hooks(
    const bool after_layout,
    InputOperationResult& result
) {
    if (tree_ == nullptr) return;
    std::set<std::uint64_t> visited;
    const auto route = [this, after_layout, &result, &visited](const std::uint64_t identity) {
        for (RetainedNode* current = tree_->find_identity(identity);
             current != nullptr;
             current = current->parent()) {
            if (!visited.insert(current->identity()).second) continue;
            const WidgetLifecycle* lifecycle = widgets_.find(current->description().type);
            const WidgetInputHook* hook = lifecycle == nullptr
                ? nullptr
                : after_layout ? &lifecycle->input.after_layout : &lifecycle->input.advance;
            ...
            for (const DescriptionBehavior& attachment : current->description().behaviors) {
                const BehaviorLifecycle* behavior = attachment.enabled
                    ? behaviors_.find(attachment.id)
                    : nullptr;
                const BehaviorInputHook* behavior_hook = behavior == nullptr
                    ? nullptr
                    : after_layout
                        ? &behavior->input.after_layout
                        : &behavior->input.advance;
                if (behavior_hook == nullptr || *behavior_hook == nullptr) continue;
                BehaviorInputScope scope(
                    *this,
                    *current,
                    attachment,
                    after_layout
                        ? BehaviorInputEventPhase::after_layout
...
InputOperationResult InputRouter::pointer(const PointerInputEvent event) {
    ...
    if (routed_subtarget_.has_value()) {
        ...
        if (routed_subtarget_->kind == WidgetSubtargetKind::scrim && owner != nullptr &&
            hover_target != nullptr && hover_target != owner && descendant_of(*hover_target, *owner)) {
    ...
    if (RetainedNode* modal = active_modal(); modal != nullptr) {
        const bool routed_above_modal = routed_subtarget_.has_value() &&
            routed_subtarget_->detached &&
            routed_subtarget_->owner_identity != modal->identity() &&
            routed_subtarget_->z_index > detached_overlay_z_index(*modal);
        const RetainedNode* focused = focused_.has_value()
    ...
    const auto finish_capture = [this, &event] {
        ...
        if (active_.has_value()) static_cast<void>(tree_->mark(*active_, DirtyReason::input));
        active_.reset();
    ...
    if (event.type == PointerEventType::press) {
        ...
        pressed_pointer_targets_.insert_or_assign(
            ...
            PointerPress{
                ...
                .button = event.button,
                .timestamp_nanos = event.timestamp_nanos > 0
                    ? event.timestamp_nanos
                    : frame_time_nanos_,
                .last_position = event.position,
                .subtarget_id = routed_subtarget_.has_value()
                    ? routed_subtarget_->id
                    : std::string{},
            }
        );
        active_ = target->identity();
        active_subtarget_ = routed_subtarget_.has_value()
        ...
        static_cast<void>(route_widget_pointer(
            target, hover_target, event, dispatch, result
        ));
        return result;
    }
    ...
    InputDispatchState dispatch = route_pointer_event(
        dispatch_target, hover_target, event, result
    );
    pressed = pressed_pointer_targets_.find(event.pointer_id);
    ...
    if (dispatch_target != nullptr && route_widget_pointer(
            dispatch_target, hover_target, event, dispatch, result
        )) {
        if (event.type == PointerEventType::release || event.type == PointerEventType::cancel) {
            finish_capture();
        }
    ...
    pressed = pressed_pointer_targets_.find(event.pointer_id);
    const std::optional<std::uint64_t> pressed_identity = pressed != pressed_pointer_targets_.end()
        ? std::optional<std::uint64_t>(pressed->second.identity)
        : std::nullopt;
    const bool click_suppressed = pressed != pressed_pointer_targets_.end() &&
                                  (pressed->second.moved_beyond_slop ||
                                   pressed->second.long_press_emitted ||
                                   pressed->second.gesture != GestureClaimState::unclaimed);
    const bool subtarget_matches = pressed == pressed_pointer_targets_.end() ||
    ...
    if (!click_suppressed && subtarget_matches && target != nullptr &&
        ...
        const std::uint64_t click_identity = hover_target != nullptr
            ? hover_target->identity()
            : target->identity();
        const std::string current_subtarget = routed_subtarget_.has_value()
            ? routed_subtarget_->id
            : std::string{};
        const bool same_click = last_widget_click_ == click_identity &&
            ...
            frame_time_nanos_ - last_widget_click_nanos_ <= input_config_.multi_click_interval_nanos &&
            std::abs(event.position.x - last_widget_click_position_.x) <= input_config_.multi_click_slop &&
            std::abs(event.position.y - last_widget_click_position_.y) <= input_config_.multi_click_slop;
        ...
        activate(
            *target, result, &event, hover_target, widget_click_count_, routed_subtarget_
        );
    }
    return result;
}
