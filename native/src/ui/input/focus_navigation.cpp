...
void collect_focus_entries(RetainedNode& node, const auto& accepts, std::vector<FocusEntry>& result,
                           std::size_t& order) {
...
std::vector<RetainedNode*> InputRouter::focusable_nodes() const {
    ...
    std::ranges::stable_sort(entries, [](const FocusEntry& left, const FocusEntry& right) {
        ...
        const double right_index =
            right.tab_index.value_or(std::numeric_limits<double>::infinity());
        return left_index != right_index ? left_index < right_index : left.order < right.order;
    });
    result.reserve(entries.size());
    for (const FocusEntry& entry : entries)
        result.push_back(entry.node);
    return result;
}

bool InputRouter::traverse_focus(const bool backwards, InputOperationResult& result) {
    ...
    if (current == nodes.end()) {
        ...
        index = backwards ? (current_index == 0U ? nodes.size() - 1U : current_index - 1U)
...
bool InputRouter::move_focus_spatial(const std::string_view direction,
                                     InputOperationResult& result) {
    const std::vector<RetainedNode*> nodes = focusable_nodes();
    if (nodes.empty() || layout_ == nullptr)
        return false;
    RetainedNode* current =
    ...
    const auto presented = [this](const RetainedNode& node) -> std::optional<Rect> {
        ...
        for (auto item = route.rbegin(); item != route.rend(); ++item) {
            ...
            transform = concatenate_presentation_transform(
                transform, local_presentation_transform(**item, *motion_, item_layout->bounds));
        }
    ...
    for (std::size_t order = 0U; order < nodes.size(); ++order) {
        ...
        if (!valid)
            continue;
        const double primary_gap =
            std::max(0.0, horizontal ? (direction == "right" ? bounds->x - source->right()
        ...
        const double cross_gap =
            target_cross_end < source_cross_start   ? source_cross_start - target_cross_end
            : target_cross_start > source_cross_end ? target_cross_start - source_cross_end
                                                    : 0.0;
    ...
    const auto selected = std::ranges::min_element(
        candidates, [has_beam](const Candidate& left, const Candidate& right) {
            if (has_beam && left.in_beam != right.in_beam)
                return left.in_beam;
            return left.score != right.score ? left.score < right.score : left.order < right.order;
        });
    if (selected == candidates.end() || (has_beam && !selected->in_beam))
        return false;
    focus(*selected->node, "traversal", result);
    return true;
}

void InputRouter::restore_popup_focus(const RetainedNode& popup, InputOperationResult& result) {
    if (tree_ == nullptr || popup.description().type != "Popup")
        return;
    const std::string& anchor_key = layout_style(popup.description()).anchor_target;
    RetainedNode* const anchor = anchor_key.empty() ? nullptr : tree_->find_key(anchor_key);
    if (anchor == nullptr || !focusable(*anchor))
        return;
    const bool was_dismissing = dismissing_transient_popups_;
    dismissing_transient_popups_ = true;
    focus(*anchor, "popup_dismissed", result);
    dismissing_transient_popups_ = was_dismissing;
}
...
bool InputRouter::dismiss_topmost(InputOperationResult& result) {
    ...
    if (!input.popup_retained.empty()) {
        ...
        if (controlled == nullptr || controlled->boolean() == nullptr) {
            changed = tree_->set_retained_value(node->identity(), input.popup_retained,
    ...
    if (node->description().type == "Select") {
        if (const std::optional<EffectiveChoice> selected = effective_choice(*node);
            selected.has_value()) {
            changed =
                tree_->set_retained_value(node->identity(), "$choiceIndex",
    ...
    if (node->description().type == "CommandPalette") {
        static_cast<void>(tree_->set_retained_value(node->identity(), "$paletteActive",
                                                    runtime::Value(0.0), DirtyReason::input));
        static_cast<void>(clear_editor_text(*node, result));
    } else if (node->description().type == "Menu") {
        static_cast<void>(tree_->set_retained_value(node->identity(), "$menuPath", runtime::Value{},
                                                    DirtyReason::input));
    }
    restore_popup_focus(*node, result);
    const std::shared_ptr<const runtime::ActionValue> action =
        activation_action(*node, !input.popup_dismiss_action_property.empty()
...
void InputRouter::sync_modal_focus(InputOperationResult& result) {
    ...
    const std::optional<std::uint64_t> next_identity =
        next_modal != nullptr ? std::optional(next_modal->identity()) : std::nullopt;
    if (next_identity == synchronized_modal_)
        return;

    if (!next_identity.has_value()) {
        if (!synchronized_modal_.has_value())
            return;
    ...
    if (!synchronized_modal_.has_value())
        focus_before_modal_ = focused_;
    synchronized_modal_ = next_identity;
    RetainedNode* current =
        focused_.has_value() && tree_ != nullptr ? tree_->find_identity(*focused_) : nullptr;
    if (current != nullptr && descendant_of(*current, *next_modal))
        return;
...
void InputRouter::sanitize_focus(InputOperationResult& result) {
    ...
    if (node == nullptr || !focusable(*node) ||
        (modal != nullptr && !descendant_of(*node, *modal)) || !within_focus_containment(*node)) {
