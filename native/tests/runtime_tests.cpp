...
void test_immutable_values_and_runtime_schemas() {
    ...
    check(record.field("title") != nullptr && *record.field("title")->string() == "Strata",
          "object field lookup failed");
    ...
    check(list_schema->accepts(Value(std::vector<Value>{Value("a"), Value("b")})),
          "valid list schema value rejected");
    check(!list_schema->accepts(Value(std::vector<Value>{Value("a"), Value("b"), Value("c")})),
          "list maximum was ignored");
    ...
    check(object_schema->accepts(
              Value(std::vector<std::pair<std::string, Value>>{{"name", Value("ok")}})),
          "valid object schema value rejected");
    check(!object_schema->accepts(
              Value(std::vector<std::pair<std::string, Value>>{{"enabled", Value(true)}})),
          "required object field was ignored");
}
...
void test_host_snapshots_are_lazy_and_generation_invalidated() {
    ...
    check(title.has_value() && title->string() != nullptr && *title->string() == "Initial",
          "host title did not resolve");
    ...
    check(resolved_indexed_label.has_value() && resolved_indexed_label->string() != nullptr &&
    ...
    check(indexed->resolve(explicit_null).has_value() &&
    ...
    HostStore store(
        [&invalidated](const std::uint64_t generation) { invalidated.push_back(generation); });
    ...
    const auto second = HostSnapshot::from_json(
        "updated", 11U, strata::data::parse_json(R"({"app":{"title":"Updated"}})"));
    check(store.adopt(second), "new host snapshot was not adopted");
    check(invalidated == std::vector<std::uint64_t>{10U, 11U},
          "host invalidation generations changed");
    check(second->evaluated_scalar_count() == 0U, "new host snapshot evaluated before access");
    check(*store.resolve("app.title")->string() == "Updated",
          "host store did not expose adopted generation");
    const auto data = HostSnapshot::from_json(
        "data", 12U, strata::data::parse_json(R"({"data":{"count":5000}})"));
    check(store.adopt(data) && *store.resolve("app.title")->string() == "Updated" &&
...
void test_state_store_and_typed_mutations() {
    ...
    check(adjust.apply(state).status == StateMutationStatus::changed,
          "numeric state adjustment failed");
    check(*state.find(count_address)->number() == 7.0, "numeric state adjustment value changed");

    const ValueSchemaPtr string_list =
        ValueSchema::list(ValueSchema::scalar(ValueSchemaKind::string), false, 3U);
    ...
    const StateMutation wrong_element{
        items_address,           initial_items, string_list, StateMutationKind::list_append,
        {{"value", Value(3.0)}},
    };
    check(wrong_element.apply(state).status == StateMutationStatus::rejected,
          "wrong list element type was accepted");
    ...
    const Value initial_record(
        std::vector<std::pair<std::string, Value>>{{"status", Value("initial")}});
    ...
    check(record_set.apply(state).status == StateMutationStatus::changed,
          "record field mutation failed");
    check(*state.find(record_address)->field("status")->string() == "updated",
          "record mutation value changed");
...
std::shared_ptr<const strata::runtime::ActionContract>
contract(std::string id, const strata::runtime::ActionDispatchPolicy policy) {
...
void test_action_contracts_dispatch_and_handler_lifetime() {
    ...
    check(dispatcher.dispatch(event, Action(required, Value("draft"))).status ==
              ActionDispatchStatus::unhandled,
          "required action without a handler was not unhandled");
    check(dispatcher.dispatch(event, Action(optional, Value("notice"))).status ==
    ...
    auto required_registration = dispatcher.register_handler(
        required, "settings", [&observed](const ActionContext& context) {
            observed.push_back(*context.action.payload.string());
            return ActionHandlerResult::handled;
        });
    check(dispatcher.dispatch(event, Action(required, Value("saved"))).status ==
    ...
    check_throws<std::invalid_argument>(
        [&dispatcher, &required] {
            static_cast<void>(
                dispatcher.register_handler(required, "duplicate", [](const ActionContext&) {
                    return ActionHandlerResult::handled;
                }));
        },
    ...
    auto first_broadcast = dispatcher.register_handler(
        broadcast, "first", [](const ActionContext&) { return ActionHandlerResult::ignored; });
    auto second_broadcast = dispatcher.register_handler(
        broadcast, "second", [](const ActionContext&) { return ActionHandlerResult::handled; });
    const auto broadcast_outcome = dispatcher.dispatch(event, Action(broadcast, Value("value")));
    check(broadcast_outcome.status == ActionDispatchStatus::handled &&
    ...
    auto forwarding_registration = dispatcher.register_handler(
        forwarded, "bridge", [](const ActionContext&) { return ActionHandlerResult::handled; });
    check(dispatcher.dispatch(event, Action(forwarded, Value("value"))).status ==
    ...
    forwarding_registration = dispatcher.register_handler(
        forwarded, "bridge", [](const ActionContext&) { return ActionHandlerResult::forwarded; });
    check(dispatcher.dispatch(event, Action(forwarded, Value("value"))).status ==
              ActionDispatchStatus::forwarded,
          "forwarded action did not report its boundary crossing");

    required_registration.close();
    check(dispatcher.handler_owners("app.save").empty(),
          "closed action registration retained its handler");
    check(dispatcher.missing_required_handlers({required, forwarded}).size() == 1U,
          "required-handler audit changed");
}

void test_builtin_catalog_projects_runtime_action_contracts() {
    const strata::compiler::SchemaRegistry schema = strata::compiler::SchemaRegistry::builtins();
    ...
    check(button != nullptr &&
              button->dispatch_policy == strata::runtime::ActionDispatchPolicy::optional &&
    ...
    const strata::runtime::Value valid_payload(
        std::vector<std::pair<std::string, strata::runtime::Value>>{
            {"name", strata::runtime::Value("count")},
            {"value", strata::runtime::Value(3.0)},
        });
    check(state_set->payload_schema->accepts(valid_payload),
          "framework action payload schema rejected valid arguments");
    check(!state_set->payload_schema->accepts(
...
animation FrozenEnum {
  ...
  fillMode: "forwards";
    ...
    const compiler::ParseResult parsed =
        compiler::parse_source("animation-invalid.strata", invalid_source);
    ...
    check(std::ranges::count(invalid.diagnostics,
                             std::string_view("STRATA.DSL.SEMANTIC_DUPLICATE_ANIMATION_FRAME"),
                             &compiler::Diagnostic::code) == 2U &&
              ...
              has_code("STRATA.DSL.SEMANTIC_ANIMATION_SCHEMA_VALUE") && invalid.animations.empty(),
          "animation semantic validation did not reject every malformed lowering contract");
    check(std::ranges::all_of(invalid.diagnostics,
                              [&invalid_source](const compiler::Diagnostic& value) {
...
animation Validated {
  ...
  trigger: "ENTER";
    ...
    const compiler::ParseResult valid_parsed =
        compiler::parse_source("animation-valid.strata", valid_source);
    ...
    check(valid.diagnostics.empty() && valid.lowering_diagnostics.empty() &&
              valid.animations.size() == 1U,
          "valid animation did not produce exactly one typed lowering payload");
    const compiler::PortableIrResult lowered =
        compiler::lower_portable_ir(valid_parsed.file, schema, valid.animations);
    const data::JsonValue* declarations =
        lowered.unit.has_value() ? lowered.unit->find("animations") : nullptr;
    const data::JsonValue* declaration = declarations != nullptr &&
                                             ...
                                             : nullptr;
    const data::JsonValue* animation =
        declaration != nullptr ? declaration->find("animation") : nullptr;
    ...
    check(lowered.diagnostics.empty() && animation != nullptr && timing != nullptr &&
              *timing->find("durationNanos")->integer() == 125'000'000 &&
              *timing->find("delayNanos")->integer() == 0 &&
              *timing->find("fillMode")->string() == "backwards" && repeat != nullptr &&
              *repeat->find("kind")->string() == "count" &&
              *repeat->find("count")->integer() == 2 && declaration->find("trigger") != nullptr &&
    ...
    check(translated != nullptr && translated->find("keyframes") != nullptr &&
              translated->find("keyframes")->array() != nullptr &&
              *translated->find("keyframes")
                      ->array()
                      ->front()
                      .find("value")
                      ->find("value")
...
void test_application_bundle_and_runtime_isolation() {
    ...
    const strata::runtime::StateSlot slot =
        strata::runtime::StateSlot::from_initial("count", strata::runtime::Value(0.0));
    static_cast<void>(left.state().read(address, slot));
    static_cast<void>(right.state().read(address, slot));
    check(left.state().write(address, slot, strata::runtime::Value(1.0)),
          "left application state update failed");
    check(*left.state().find(address)->number() == 1.0 &&
    ...
    const auto left_host = strata::runtime::HostSnapshot::from_json(
        "left-host", 1U, strata::data::parse_json(R"({"app":{"title":"Left"}})"));
    static_cast<void>(left.host().adopt(left_host));
    check(left.host().resolve("app.title").has_value() &&
              !right.host().resolve("app.title").has_value(),
          "application contexts shared host snapshots");
    left.layers().push_screen("left-screen");
    check(left.layers().active_screen() == "left-screen" &&
              !right.layers().active_screen().has_value(),
          "application contexts shared layer stacks");
    check(left.dirty_generation() == 3U && right.dirty_generation() == 0U,
...
void test_canonical_runtime_diagnostic_store() {
    ...
    RuntimeServices services(
        [&](const RuntimeDiagnosticRecord& record, const std::uint64_t dropped_count) {
    ...
    services.end_frame();
    check(published.size() == 2U && published[0].sequence == published[1].sequence &&
    ...
    services.report(RuntimeDiagnostic{
        "STRATA.TEST.PENDING",
        "Must be cleared before publication.",
        {},
        std::nullopt,
    ...
    check(snapshot.records.empty() && snapshot.dropped_count == 0U && published.size() == 2U,
    ...
    check(inactive_publication_rejected,
    ...
    services.publish_current_frame(diagnostic);
    services.publish_current_frame(diagnostic);
    check(!services.has_pending_frame_work() && published.size() == 4U &&
              published[2U].sequence == published[3U].sequence && published[3U].frame_index == 4U &&
...
void test_transactional_activation_and_last_good_reload(const std::filesystem::path& fixture_root) {
    using namespace strata::runtime;
    ...
    const auto load = [](const std::filesystem::path& root, const std::string& name) {
        return strata::resource::load_utf8_resource(root,
    ...
    const auto loader = [](const std::string_view,
                           const std::string_view path) -> strata::compiler::ModuleSource {
    ...
    const ActivationResult inferred_activation = inferred_state.compile_and_activate(
        ...
        loader, 0U);
    check(inferred_activation.activated() &&
    ...
    check(identity_contract
              .compile_and_activate(
                  strata::compiler::ModuleSource{
                      "app/identity.strata",
                      "overlay Identity { root Panel() { for item in [1] { Text(text: "
                      "format(\"{0}\", item)) } } }",
                  },
                  loader, 0U)
              .activated(),
          "portable identity contract fixture did not compile");
    const std::string ordinary_ir =
        strata::data::encode_canonical_json(identity_contract.active_unit()->portable_ir());
    check(ordinary_ir.find("\"identity\":") == std::string::npos,
    ...
    check(
        ordinary_round_trip
            .activate(
                strata::data::materialize_json(identity_contract.active_unit()->portable_ir()), 0U)
            .activated(),
        "portable IR activation required identity on an ordinary for");

    std::string ordinary_ir_with_extra_identity = ordinary_ir;
    const std::size_t ordinary_index_name = ordinary_ir_with_extra_identity.find("\"indexName\":");
    check(ordinary_index_name != std::string::npos,
          "ordinary for IR did not contain its indexName field");
    ordinary_ir_with_extra_identity.insert(ordinary_index_name, "\"identity\":false,");
    ...
    const ActivationResult extra_identity_result = ordinary_extra_identity.activate(
        strata::data::parse_json(ordinary_ir_with_extra_identity), 0U);
    check(extra_identity_result.activated(),
    ...
    check(repeater_identity
              .compile_and_activate(
                  strata::compiler::ModuleSource{
                      "app/repeater-identity.strata",
                      "overlay RepeaterIdentity { root Repeater(estimatedItemExtent: 24) { for "
                      "item in [{ key: \"one\" }] { Panel(key: item.key) } } }",
                  },
                  loader, 0U)
              .activated(),
          "portable Repeater identity fixture did not compile");
    std::string null_repeater_identity =
        strata::data::encode_canonical_json(repeater_identity.active_unit()->portable_ir());
    ...
    check(repeater_identity_position != std::string::npos,
          "compiled Repeater IR did not contain an identity plan");
    const std::size_t identity_start = repeater_identity_position + identity_object.size() - 1U;
    ...
    check(identity_end < missing_repeater_identity.size() &&
              missing_repeater_identity[identity_end] == ',',
          "compiled Repeater identity plan was not followed by another loop field");
    missing_repeater_identity.erase(repeater_identity_position,
                                    identity_end - repeater_identity_position + 1U);
    ApplicationContext missing_repeater_identity_context("portable-repeater-identity-missing",
                                                         bundle);
    const ActivationResult missing_repeater_identity_result =
        missing_repeater_identity_context.activate(
            strata::data::parse_json(missing_repeater_identity), 0U);
    check(missing_repeater_identity_result.status == ActivationStatus::rejected_unit &&
    ...
    null_repeater_identity.replace(identity_start, identity_end - identity_start, "null");
    ApplicationContext malformed_repeater_identity("portable-repeater-identity-null", bundle);
    const ActivationResult null_identity_result =
        malformed_repeater_identity.activate(strata::data::parse_json(null_repeater_identity), 0U);
    check(null_identity_result.status == ActivationStatus::rejected_unit &&
    ...
    ActivationResult initial = application.compile_and_activate(
        ...
        loader, 0U);
    check(initial.activated() && application.active_generation() == 0U,
          "initial runtime unit activation failed (status " +
              std::to_string(static_cast<int>(initial.status)) + "): " +
              (initial.diagnostics.empty() ? std::string("no diagnostic")
                                           : initial.diagnostics.front().message));
    check(static_cast<bool>(application.active_unit()->overlay("Main")),
          "active overlay was not indexed");

    const strata::data::JsonValue source_portable_ir =
        strata::data::materialize_json(application.active_unit()->portable_ir());
    strata::compiler::CompiledModuleArtifact frozen_artifact =
        strata::compiler::decode_compiled_module_artifact(
            strata::compiler::encode_compiled_module_artifact(
                source_portable_ir, application.active_unit()->source_map()));
    ...
    const ActivationResult frozen_activation = frozen_application.activate(
        std::move(frozen_artifact.unit), 0U, std::move(frozen_artifact.source_map));
    check(
    ...
    check(application.state().write(count_address, count_slot, Value(4.0)),
          "pre-reload state write failed");
    ...
    ActivationResult good = application.compile_and_activate(
        ...
        loader, 1U);
    check(good.activated() && good.state_migrated,
          "compatible reload did not activate and migrate");
    check(application.state().find(count_address) != nullptr &&
    ...
    const Action adjust(adjust_contract, Value(std::vector<std::pair<std::string, Value>>{
                                             {"amount", Value(1.0)},
                                             {"name", Value("count")},
                                         }));
    application.bind_state_scope("overlay Main", "count", "overlay Main", count_address.scope);
    const ActionDispatchOutcome adjusted = application.dispatch(
        ActionEvent{"activate", std::string("dynamic.increment"), Value{}}, adjust, "overlay Main");
    check(adjusted.status == ActionDispatchStatus::handled &&
    ...
    ActivationResult bad = application.compile_and_activate(
        ...
        loader, 2U);
    check(bad.status == ActivationStatus::rejected_compile &&
              application.active_unit() == last_good && application.active_generation() == 1U,
    ...
    ActivationResult incompatible = application.compile_and_activate(
        ...
        loader, 3U);
    check(incompatible.status == ActivationStatus::rejected_capability &&
    ...
    ActivationResult stale =
        application.activate(strata::data::materialize_json(last_good->portable_ir()), 2U);
    check(stale.status == ActivationStatus::rejected_generation &&
              application.last_attempted_generation() == 3U &&
              application.active_unit() == last_good,
...
void test_portable_ir_expression_runtime() {
    ...
    check(mapped_items != nullptr && mapped_items->values.size() == 2U &&
    ...
    check(stable_metadata_one.collection() != nullptr &&
              ...
              *stable_metadata_one.collection() == *stable_metadata_hit.collection() &&
              (*stable_metadata_hit.collection())->cache_hits.load(std::memory_order_relaxed) == 1U,
    ...
    const auto stable_host = HostSnapshot::from_json(
        "expression-host-stable-leaf", 3U,
        strata::data::parse_json(
    ...
    check(explicit_null_view.collection() != nullptr && missing_view.collection() != nullptr &&
          ...
          "collection cache collapsed an explicit-null host leaf into absence or evaluated a "
          "sibling");
    ...
    check(runtime.diagnostics().empty(),
          "valid portable IR expressions produced runtime diagnostics");
}
...
void test_target10_compile_diagnostics() {
    ...
    const compiler::ParseResult invalid_parsed =
        compiler::parse_source("target10-invalid.strata", invalid_source);
    check(invalid_parsed.diagnostics.empty(), "Target 10 invalid semantic fixture did not parse");
    const compiler::SemanticResult invalid =
        compiler::validate_semantics(invalid_parsed.file, schema);
    const auto has = [&invalid](const std::string_view code) {
        return std::ranges::any_of(invalid.diagnostics, [code](const compiler::Diagnostic& value) {
            return value.code == code;
        });
    };
    check(has("STRATA.DSL.SEMANTIC_ASYNC_WHEN_NOT_EXHAUSTIVE") &&
    ...
    const compiler::ParseResult flow_invalid_parsed =
        compiler::parse_source("target10-flow-invalid.strata", flow_invalid_source);
    check(flow_invalid_parsed.diagnostics.empty(), "async flow invalid fixture did not parse");
    const compiler::SemanticResult flow_invalid =
        compiler::validate_semantics(flow_invalid_parsed.file, schema);
    check(std::ranges::any_of(flow_invalid.diagnostics,
                              [](const compiler::Diagnostic& value) {
                                  return value.code ==
                                         "STRATA.DSL.SEMANTIC_ASYNC_WHEN_NOT_EXHAUSTIVE";
                              }) &&
              std::ranges::any_of(flow_invalid.diagnostics,
                                  [](const compiler::Diagnostic& value) {
    ...
    const compiler::ParseResult valid_parsed =
        compiler::parse_source("target10-valid.strata", valid_source);
    ...
    check(application.durability().application_value("target.list") == nullptr,
...
void test_authored_effect_schema_and_semantics() {
    ...
    const compiler::SemanticResult unbounded = validate(R"(
      overlay Main {
        root Panel(effect: effect(
          "test:glass", refreshRate: "UNBOUNDED", radius: 12, tint: #7DB8FF44
        ))
      }
    )");
    check(unbounded.diagnostics.empty(),
          "unbounded authored effect refresh policy produced diagnostics");
    const compiler::SemanticResult invalid_refresh = validate(R"(
      overlay Main {
        root Panel(effect: effect("test:glass", refreshRate: 0, radius: 12, tint: #7DB8FF44))
      }
    )");
    check(std::ranges::any_of(invalid_refresh.diagnostics,
                              [](const compiler::Diagnostic& value) {
                                  return value.code == "STRATA.DSL.SEMANTIC_TYPE_MISMATCH" &&
                                         value.component_path.has_value() &&
                                         value.component_path->ends_with(".refreshRate");
                              }),
          "non-positive authored effect refresh rate bypassed semantic validation");
    ...
    check(has("STRATA.DSL.SEMANTIC_MISSING_ARGUMENT") &&
...
void test_durability_round_trip_migration_and_corruption() {
    ...
    first.set_widget_value("view.tree", "strata.tree.expanded",
                           Value(std::vector<Value>{Value("root")}));
    first.set_shell_value("window",
                          Value(std::vector<std::pair<std::string, Value>>{{"x", Value(20.0)}}));
    ...
    check(restored.configure("test.application", adapter).empty(),
          "durable round trip was rejected");
    check(restored.application_value("settings.name") != nullptr &&
              *restored.application_value("settings.name")->string() == "Strata",
          "application durable value did not round trip");
    check(restored.widget_value("view.tree", "strata.tree.expanded") != nullptr &&
              restored.widget_value("view.tree", "strata.tree.expanded")->list() != nullptr,
          "widget durable value did not round trip");
    check(restored.command_recency("surface") == std::vector<std::string>{"save", "open"},
    ...
    check(migrated_json.find("version") != nullptr &&
    ...
    const std::vector<DurableLoadIssue> corrupt_issues =
        corrupt.configure("test.application", adapter);
    check(corrupt_issues.size() == 1U &&
    ...
    DurableState future;
    const std::vector<DurableLoadIssue> future_issues =
        future.configure("test.application", adapter);
    check(future_issues.size() == 1U &&
    ...
    bytes =
        R"({"format":"strata.durable","version":1,"application":{"valid":true},"widgets":[],"commands":{},"shell":{}})";
    DurableState malformed;
    const std::vector<DurableLoadIssue> malformed_issues =
        malformed.configure("test.application", adapter);
    check(malformed_issues.size() == 1U &&
...
void test_declared_widget_persistence_is_exact() {
    ...
    const runtime::Value table_widths(std::vector<runtime::Value>{
        runtime::Value(std::vector<std::pair<std::string, runtime::Value>>{
            {"id", runtime::Value("name")},
            {"width", runtime::Value(240.0)},
        })});
    check(table != nullptr && table->persistence.accepts != nullptr &&
    ...
    durable.set_widget_value("tree.view", "strata.tree.expanded",
                             runtime::Value(std::vector<runtime::Value>{runtime::Value("root")}));
    ...
    tree.configure_persistence(
        [](const std::string_view type) {
            return type == "TreeView" ? std::vector<std::string>{"strata.tree.expanded"}
                                      : std::vector<std::string>{};
        },
        [&durable](const std::string_view, const std::string_view key,
        ...
        [&durable](const std::string_view key, const std::string_view field,
    ...
    static_cast<void>(tree.reconcile(
        ui::DescriptionNode::create("TreeView", "tree", "test", "scope", std::move(properties))));
    check(tree.root()->retained_value("strata.tree.expanded") != nullptr &&
    ...
    check(durable.widget_value("tree.view", "strata.tree.expanded") != nullptr &&
              *durable.widget_value("tree.view", "strata.tree.expanded")
                      ->list()
                      ->values.front()
                      .string() == "next",
...
void test_async_debounce_latest_wins_and_owner_cancellation() {
    ...
    const std::optional<Value> normalized_tree = tree_items->normalize(
        Value(std::vector<Value>{Value(std::vector<std::pair<std::string, Value>>{
            {"key", Value("folder.child")},
            {"parentKey", Value("folder")},
        })}));
    check(normalized_tree.has_value() && normalized_tree->list() != nullptr &&
    ...
    AsyncDataService service(
        ...
        [](const std::string_view, const Value& value) { return std::optional<Value>(value); },
    ...
    check(cancelled == std::vector<std::uint64_t>{second},
          "re-query did not cancel begun host work");
    ...
    check(service.succeed(third, Value(std::vector<Value>{Value("ready")})),
          "async success was rejected");
    const Value ready = service.state("search");
    check(ready.field("status") != nullptr && *ready.field("status")->string() == "READY" &&
    ...
    check(service.cancel_owner("surface:owner") == 1U,
          "async owner cancellation did not cancel work");
    ...
    static_cast<void>(service.query("search", Value("children"), "surface:item:folder", 250, 0));
    check(service.retain_owners("surface:", {"surface:node:tree"}) == 1U,
          "detached async item owner did not cancel its host work");

    const std::uint64_t worker_request =
        *service.query("search", Value("worker"), "surface:node:query", 300, 0);
    std::jthread completion([&service, worker_request] {
        check(service.post_progress(worker_request, AsyncProgress{1.0, 2.0, "worker"}),
    ...
    completion.join();
    service.advance(301);
    check(service.state("search").field("status") != nullptr &&
...
void test_application_undo_grouping_coalescing_and_invalidation() {
    ...
    undo.record("surface", before, state.snapshot(),
                UndoRecordOptions{"Adjust count", "slider", 10});
    before = state.snapshot();
    static_cast<void>(state.write(address, slot, Value(2.0)));
    undo.record("surface", before, state.snapshot(),
                UndoRecordOptions{"Adjust count", "slider", 20});
    ...
    check(!undo.status("noop").can_undo,
    ...
    before = state.snapshot();
    static_cast<void>(state.write(address, slot, Value(2.0)));
    undo.record("noop-group", before, state.snapshot(),
                UndoRecordOptions{"down", std::nullopt, 51});
    undo.end_group("noop-group");
    check(!undo.status("noop-group").can_undo,
...
void test_layer_stack_and_declarative_registry() {
    ...
    check(registry.execute(stack, DeclarativeLayerOperation::push, "Main").status ==
    ...
    check(registry.execute(stack, DeclarativeLayerOperation::show, "Inspector").status ==
              LayerOperationStatus::handled,
          "declarative overlay show failed");
    check(registry.execute(stack, DeclarativeLayerOperation::show, "Inspector").status ==
              LayerOperationStatus::handled,
          "registered overlay re-show was not handled");
    check(registry.execute(stack, DeclarativeLayerOperation::replace, "Settings").status ==
                  LayerOperationStatus::handled &&
              stack.active_screen() == "screen:settings",
          "declarative screen replacement failed");
    check(stack.snapshot() ==
              std::vector<LayerSnapshot>{
                  {"screen:settings", LayerRole::screen},
                  {"overlay:inspector", LayerRole::overlay},
              },
          "layer snapshot ordering changed");
    check(registry.execute(stack, DeclarativeLayerOperation::hide, "Inspector").status ==
              LayerOperationStatus::handled,
          "declarative overlay hide failed");
    check(registry.execute(stack, DeclarativeLayerOperation::pop).status ==
                  LayerOperationStatus::handled &&
              registry.execute(stack, DeclarativeLayerOperation::pop).status ==
                  LayerOperationStatus::ignored,
