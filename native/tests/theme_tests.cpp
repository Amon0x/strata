...
[[nodiscard]] std::shared_ptr<const strata::ui::DescriptionChildren> eager(
...
[[nodiscard]] std::shared_ptr<const strata::ui::DescriptionNode> node(
    std::string type,
    std::string key,
...
[[nodiscard]] strata::ui::DescriptionNode::Properties properties(
...
[[nodiscard]] const strata::runtime::Value* field(
    const strata::ui::DescriptionNode& node,
...
void test_scopes_styles_tokens_and_lazy_ranges() {
    ...
    parent_styles.emplace(
        ui::ThemeWidgetKey{"Button", "primary"},
        ui::ThemedWidgetStyle{
            inherited_visual, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
        }
    );
    auto parent = std::make_shared<const ui::Theme>(
        "parent", ui::ThemeTokens{}, nullptr, std::nullopt, std::move(parent_styles)
    );
    ...
    auto button_properties = properties(
        std::nullopt,
        {
    ...
    check(field(button, "background") != nullptr &&
              *field(button, "background")->color() ==
                  runtime::ColorValue{80U, 81U, 82U, 255U},
    ...
    const ui::ThemeMaterializationResult entering = ui::materialize_theme_subtree(
        ...
        catalog,
        resolved.root->projected_theme,
        resolved.root->projected_theme_scope,
        [](const std::string_view type) { return type != "ThemeScope"; }
    );
    check(
        entering.root != nullptr &&
    ...
    check(
        cached_second.root == cached_first.root && cached_second.stats.resolved_nodes == 0U,
    ...
    lazy->children = std::make_shared<const ui::GeneratedDescriptionChildren>(
        100U,
        [&lazy_resolver_calls](const std::size_t index) {
            ++lazy_resolver_calls;
            return node("Text", "row-" + std::to_string(index));
        }
    );
    lazy->materialization = ui::MaterializationRange{40U, 43U};
    const ui::ThemeMaterializationResult lazy_result = ui::materialize_theme_tree(
        lazy,
        catalog,
...
void test_catalog_and_motion_policy() {
    ...
    ui::ThemeMotionPolicy policy(true, {
        {"fast", ui::MotionTiming{9'000'000, 2'000'000, "cubic-out", {}, false, ui::MotionFillMode::both}},
        {"standard", ui::MotionTiming{17'000'000, 0, "cubic-in-out", {}, false, ui::MotionFillMode::both}},
    });
    ...
    auto animated_properties = properties(
        std::nullopt,
        {
            {"animateChanges", runtime::Value(std::vector<std::pair<std::string, runtime::Value>>{
                 ...
                 {"properties", runtime::Value(std::vector<runtime::Value>{runtime::Value("radius")})},
             })},
            {"animateContentSize", runtime::Value(
    ...
    const ui::ThemeMaterializationResult resolved = ui::materialize_theme_tree(
        node("Button", "animated", std::move(animated_properties)),
        catalog,
        [](const std::string_view) { return true; },
    ...
    const std::optional<ui::ContentSizeMotionSpec> content = ui::content_size_motion(*resolved.root);
...
void test_exact_resolution_validation_and_local_scope() {
    ...
    check(child.style("Button", "danger").visual->radius == 1.0,
          "local exact variant did not win");
    ...
    check(rejects([] {
        static_cast<void>(runtime::Value(
            std::vector<std::pair<std::string, runtime::Value>>{{"  ", runtime::Value(1.0)}}
        ));
    }), "whitespace-only style patch key was accepted");
    ui::ThemedWidgetStyle{}.validate();
    check(rejects([] {
        static_cast<void>(ui::ThemeMotionPolicy(false, {
            {"standard", ui::MotionTiming{10, 0, " \t", {}, false, ui::MotionFillMode::both}},
        }));
    ...
    const ui::ThemeMaterializationResult local = ui::materialize_theme_tree(
        node("Panel", "local.scope", {}, {node("Text", "local.text", properties(std::nullopt, {}, "primary"))}),
        catalog,
        [](const std::string_view type) { return type != "Panel"; }
    );
...
public:
    ...
    [[nodiscard]] std::uint64_t generation() const noexcept override { return 1U; }
...
void test_full_style_round_trip_and_nullability() {
    ...
    layout.width = ui::LayoutSize::clamp(
        ...
        ui::LayoutSize::clamp(
            std::nullopt,
            ui::LayoutSize(ui::LayoutSize::Kind::content),
    ...
    virtual_list.items = std::make_shared<const TestKeySequence>(
    ...
    motion.attachments.push_back(ui::ThemeMotionAttachment{
        ui::MotionTrigger::hover, "fade", true, ui::MotionDirection::forward, std::nullopt,
    });
    motion.value_channels.push_back(ui::ThemeMotionValueChannel{
        "indicator-target", ui::MotionProperty::indicator_position, 0.5, "standard",
    });
    motion.resolved_properties = ui::ThemeResolvedPropertyMotion{{ui::MotionProperty::radius}, "fast"};
    ...
    const ui::ThemeMaterializationResult materialized = ui::materialize_theme_tree(
        node("Panel", "roundtrip.node", properties("roundtrip", {}, "ExactCase")),
        catalog,
        [](const std::string_view) { return true; }
    );
    ...
    check(!decoded.participates && decoded.kind == layout.kind && decoded.width == layout.width &&
              ...
              decoded.intrinsic_size == layout.intrinsic_size && decoded.padding == layout.padding &&
              decoded.margin == layout.margin && decoded.gap == layout.gap &&
              decoded.align_items == layout.align_items &&
              decoded.justify_content == layout.justify_content &&
              decoded.align_content == layout.align_content &&
              decoded.align_self == layout.align_self && decoded.justify_self == layout.justify_self &&
              decoded.wrap == layout.wrap && decoded.clip == layout.clip &&
              decoded.z_index == layout.z_index && decoded.grid_column == layout.grid_column &&
              decoded.grid_row == layout.grid_row && decoded.column_span == layout.column_span &&
              decoded.row_span == layout.row_span &&
              decoded.grid_columns == layout.grid_columns && decoded.grid_rows == layout.grid_rows &&
              decoded.scroll_viewport_insets == layout.scroll_viewport_insets &&
              decoded.scroll_content_padding == layout.scroll_content_padding &&
              decoded.scrollbar_gutter == layout.scrollbar_gutter &&
              decoded.scroll_horizontal && decoded.scroll_vertical &&
              decoded.scroll_offset == layout.scroll_offset && decoded.pin_horizontal &&
              decoded.pin_vertical && decoded.portal_target == layout.portal_target &&
              !decoded.detach_from_parent_clip,
    ...
    check(field(*materialized.root, "hover") != nullptr &&
              ...
              field(*materialized.root, "hover")->field("animation")->string()->starts_with("$theme|"),
    ...
    disabled.validate();
    ui::Theme disabled_theme(
        "disabled", ui::ThemeTokens{}, nullptr, std::nullopt,
    ...
    const ui::ThemeMaterializationResult empty_materialized = ui::materialize_theme_tree(
        node("Panel", "empty", properties("disabled", {
            {"$contentSizeMotionDefaults", runtime::Value(std::vector<std::pair<std::string, runtime::Value>>{
                {"height", runtime::Value(true)},
            })},
            {"$disclosureDefaults", runtime::Value(std::vector<std::pair<std::string, runtime::Value>>{
    ...
    const ui::ThemeMaterializationResult authored_materialized = ui::materialize_theme_tree(
        node("Panel", "authored", std::move(authored_motion)), disabled_catalog,
        [](const std::string_view) { return true; }
    );
...
void test_unicode_recursive_layout_and_typed_motion_contracts() {
    ...
    check(rejects([] {
              static_cast<void>(ui::Theme("\xE2\x80\x83", ui::ThemeTokens{}));
          }) &&
              rejects([] {
                  static_cast<void>(runtime::Value(runtime::ThemeTokenValue{"\xC2\xA0"}));
              }) &&
              rejects([] {
                  ui::ThemeWidgetKey{"Panel", "\xE2\x80\x83"}.validate();
              }) &&
              rejects([] {
                  static_cast<void>(ui::ThemeMotionPolicy(false, {
              ...
              rejects([] {
                  static_cast<void>(ui::Theme("\xF0\x28\x8C\x28", ui::ThemeTokens{}));
              }),
    ...
    ui::CompiledMotion inline_motion{
        ...
        ui::MotionTiming{
            70, 11, ui::MotionEasing::cubic_bezier(0.2, -0.3, 0.8, 1.3),
            {ui::MotionRepeatKind::count, 2U}, true, ui::MotionFillMode::forwards,
        },
        {ui::MotionTrack{ui::MotionProperty::opacity, {
    ...
    anonymous.attachments.push_back(ui::ThemeMotionAttachment{
        ui::MotionTrigger::exit, ui::ThemeAnimationSpec(inline_motion), true,
        ui::MotionDirection::forward, ui::MotionTrigger::enter,
    });
    anonymous.validate();
    ...
    const auto retains_exit = [](ui::ThemeAnimationSet set, std::string name) {
        ...
        ui::Theme theme(
            name, ui::ThemeTokens{}, nullptr, std::nullopt,
            {{{"Panel", "default"}, ui::ThemedWidgetStyle{
                std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::move(motion),
            }}}
        );
        ui::ThemeCatalog catalog;
        check(catalog.register_theme(theme), "inline exit theme did not register");
        const ui::ThemeMaterializationResult materialized = ui::materialize_theme_tree(
            node("Panel", "exit.node", properties(name)), catalog,
            [](const std::string_view) { return true; }
        );
    ...
    check(rejects([] {
              static_cast<void>(ui::MotionEasing("\xF0\x28\x8C\x28"));
          }),
    ...
    repeating_channel.channels.push_back(ui::ThemeMotionChannel{
        "timeline", ui::ThemeAnimationSpec(inline_motion), ui::MotionInteraction::hover,
        std::nullopt,
    });
    ...
    conflict.channels.push_back(ui::ThemeMotionChannel{
        "timeline", ui::ThemeAnimationSpec(opacity_motion), ui::MotionInteraction::hover,
        std::nullopt,
    });
    conflict.value_channels.push_back(ui::ThemeMotionValueChannel{
        "target", ui::MotionProperty::opacity, 0.5, "standard",
    });
    check(rejects([&] { conflict.validate(); }),
          "competing typed motion channels were accepted");
    ui::ThemeAnimationSet wrong_kind;
    wrong_kind.value_channels.push_back(ui::ThemeMotionValueChannel{
        "wrong", ui::MotionProperty::opacity, true, "standard",
    });
    ...
    const auto themed_animation = [](std::string theme_name, std::string component) {
        ui::CompiledMotion declaration{
            "e", ui::MotionTrigger::animate, ui::MotionTiming{},
            {ui::MotionTrack{ui::MotionProperty::opacity, {
        ...
        styles.emplace(
            ui::ThemeWidgetKey{std::move(component), "d"},
            ui::ThemedWidgetStyle{std::nullopt, std::nullopt, std::nullopt,
                                  std::nullopt, std::move(motion)}
        );
        return ui::Theme(std::move(theme_name), ui::ThemeTokens{}, nullptr,
                         std::nullopt, std::move(styles));
    };
    ...
    ui::Theme empty_style_theme(
        "empty-style", ui::ThemeTokens{}, nullptr, std::nullopt,
        {{{"Panel", "default"}, ui::ThemedWidgetStyle{}}}
    );
    const ui::ResolvedThemeWidgetStyle empty_style =
        empty_style_theme.resolved_style("Panel");
    const ui::ResolvedThemeWidgetStyle missing_style =
        empty_style_theme.resolved_style("Button");
    check(empty_style.explicit_style && empty_style.style == ui::ThemedWidgetStyle{} &&
              empty_style_theme.widget_styles().contains(
                  ui::ThemeWidgetKey{"Panel", "default"}
    ...
    const ui::ThemeMaterializationResult empty_style_materialized =
        ui::materialize_theme_tree(
            node("Panel", "empty-style.node", properties("empty-style")),
            empty_style_catalog,
            [](const std::string_view) { return true; }
        );
    ...
    ui::Theme inside_theme(
        "inside", ui::ThemeTokens{}, nullptr, std::nullopt,
        {{{"Panel", "default"}, ui::ThemedWidgetStyle{
            inside_visual, std::nullopt, std::nullopt, inside_layout, std::nullopt,
        }}}
    );
    ui::ThemeCatalog inside_catalog;
    check(inside_catalog.register_theme(inside_theme), "inside-border theme did not register");
    const ui::ThemeMaterializationResult inside = ui::materialize_theme_tree(
        node("Panel", "inside.node", properties("inside")), inside_catalog,
        [](const std::string_view) { return true; }
    );
    ...
    const ui::ThemeMaterializationResult authored_inside = ui::materialize_theme_tree(
        node("Panel", "inside.authored", properties("inside", {
...
[[nodiscard]] strata::compiler::ModuleLoader no_imports() {
    return [](const std::string_view, const std::string_view path) -> strata::compiler::ModuleSource {
        throw strata::compiler::ModuleLoadError("unexpected theme fixture import '" + std::string(path) + "'");
    };
}
...
overlay Main {
  root Scroll(key: "scroll", vertical: true, horizontal: false, layout: { width: 100, height: 100 }) {
    ...
    ui::ThemeMotionPolicy timings(false, {
        {"fast", ui::MotionTiming{200'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
        {"standard", ui::MotionTiming{200'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
    });
    ui::Surface surface(
        "theme-scroll", application, runtime::LayerRole::overlay, "Main", environment,
        {}, ui::WidgetRegistry{}, ui::BehaviorRegistry{}, nullptr,
    ...
    try {
        static_cast<void>(surface.animate_scroll_to(ui::ScrollAnimationRequest{
            "scroll", std::nullopt, 1.0, "\xE2\x80\x83", std::nullopt,
        }));
    } catch (const std::invalid_argument&) {
        unicode_timing_rejected = true;
    }
    check(unicode_timing_rejected,
          "animated scrolling accepted a Unicode-whitespace timing name");
    ui::ThemeMotionPolicy scoped_timings(false, {
        {"fast", ui::MotionTiming{100'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
        {"standard", ui::MotionTiming{200'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
    });
    check(surface.set_scoped_theme(
              "scroll",
              ui::Theme("scroll-local", ui::ThemeTokens{}, nullptr, scoped_timings)
          ),
    ...
    check(surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 300.0, "fast", std::nullopt,
          }),
          "scroll animation did not start");
    static_cast<void>(surface.frame(11'000'000));
    static_cast<void>(surface.frame(61'000'000));
    ...
    check(surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 250.0, "fast", std::nullopt,
          }),
    ...
    ui::ThemeMotionPolicy scoped_reduced(true, {
        {"fast", ui::MotionTiming{100'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
        {"standard", ui::MotionTiming{200'000'000, 0, "linear", {}, false, ui::MotionFillMode::both}},
    });
    check(surface.set_scoped_theme(
              "scroll",
              ui::Theme("scroll-local-reduced", ui::ThemeTokens{}, nullptr, scoped_reduced)
          ),
    ...
    check(surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 0.0, "missing", 50'000'000,
          }),
          "unknown-policy scroll animation did not use fallback timing");
    static_cast<void>(surface.frame(141'000'000));
    check(std::ranges::any_of(surface.last_frame().diagnostics.records, [](const auto& diagnostic) {
              return diagnostic.diagnostic.code == "STRATA.ANIMATION.MOTION_POLICY_UNKNOWN";
          }),
    ...
    check(surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 100.0, "fast", std::nullopt,
          }) && surface.active_scroll_animation_count() == 0U,
    ...
    check(surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 0.0, "fast", 10'000'000,
          }),
    ...
    check(!surface.animate_scroll_to(ui::ScrollAnimationRequest{
              "scroll", std::nullopt, 0.0, "fast", std::nullopt,
          }) && surface.active_scroll_animation_count() == 0U,
    ...
    constexpr std::string_view detached_source = "overlay Main { root Panel(key: \"replacement\") }";
    check(application.compile_and_activate(
              compiler::ModuleSource{"theme-scroll-detached.strata", std::string(detached_source)},
              no_imports(), 1U
          ).activated(),
