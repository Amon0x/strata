...
std::shared_ptr<const strata::ui::DescriptionChildren> children(
    std::vector<std::shared_ptr<const strata::ui::DescriptionNode>> values
) {
    return std::make_shared<const strata::ui::EagerDescriptionChildren>(std::move(values));
}

std::shared_ptr<const strata::ui::DescriptionNode> node(
    std::string type,
...
[[nodiscard]] bool rendered_text(
    const strata::ui::RenderCommandBuffer& commands,
    ...
    return std::ranges::any_of(commands.commands(), [expected](const auto& command) {
        ...
        if (text == nullptr || text->glyphs.size() != expected.size()) return false;
...
[[nodiscard]] const strata::data::JsonValue* semantic_child_named(
    const strata::data::JsonValue* node,
...
void test_collection_models() {
    ...
    const SelectionTransition toggled = select(
        ordered,
        current,
        std::string("zeta"),
        "alpha",
    ...
    check(
        toggled.selected.values() == std::vector<std::string>({"zeta", "beta", "alpha"}),
        "collection toggle lost stable insertion order"
    );
    const SelectionTransition ranged = select(
        ordered,
        toggled.selected,
        std::string("zeta"),
        "gamma",
    ...
    check(
        ranged.selected.values() == std::vector<std::string>({"zeta", "alpha", "gamma"}),
        "collection range selection did not follow source order"
    );
    check(
        navigate(ordered, std::string("alpha"), Navigation::next, 10U, 2U) == "beta" &&
    ...
    const std::vector<Label> labels{
        {"zeta", "Zulu"}, {"alpha", "Alpha"}, {"gamma", "Gamma"}, {"beta", "Beta"},
    };
    check(
        typeahead(labels, std::string("gamma"), "a") == "alpha",
...
void test_bundled_font_metrics(const std::filesystem::path& resource_root) {
    ...
    check(shaped.glyphs.front().code_point == static_cast<std::uint32_t>('E'), "missing glyph emitted .notdef");
    check(shaped.glyphs.front().text_start_offset == 3U, "missing glyph lost canonical UTF-16 offsets");
    ...
    check(latin_a_outline != nullptr && !latin_a_outline->empty(), "native glyf decoder lost Latin A");
    check(latin_a_outline->bounds.has_value(), "native glyf decoder lost Latin A bounds");
    check(
        latin_a_outline == font.glyph_outline(latin_a),
    ...
    check(
        std::ranges::any_of(latin_a_raster->bytes, [](const std::uint8_t value) { return value != 0U; }),
    ...
    const std::optional<strata::font::GlyphRasterBitmap> unhinted_h =
        strata::font::rasterize_coverage(
            font, latin_h, 11.3, 1.0, strata::font::SubpixelPhase{1U, 4U}, unhinted_coverage
        );
    const std::optional<strata::font::GlyphRasterBitmap> hinted_h =
        strata::font::rasterize_coverage(
            font, latin_h, 11.3, 1.0, strata::font::SubpixelPhase{1U, 4U}, hinted_coverage
        );
    const std::optional<strata::font::GlyphRasterBitmap> other_phase_h =
        strata::font::rasterize_coverage(
            font, latin_h, 11.3, 1.0, strata::font::SubpixelPhase{3U, 4U}, hinted_coverage
        );
    ...
    const std::optional<strata::font::GlyphAtlasEntry> first_entry = atlas.request_coverage(
        "strata:fonts/default-medium", font, latin_a, 12.0, {}
    );
    ...
    const auto default_large_entry = explicit_mode_atlas.request(
        "strata:fonts/default-medium", font, latin_a, 72.0, {}
    );
    const auto explicit_msdf_entry = explicit_mode_atlas.request(
        "strata:fonts/default-medium", font, latin_a, 72.0, {}, 0U,
    ...
    const std::optional<strata::font::GlyphAtlasEntry> cached_entry = atlas.request_coverage(
        "strata:fonts/default-medium", font, latin_a, 12.0, {}
    );
    check(
        cached_entry.has_value() &&
        cached_entry->texture == first_entry->texture &&
    ...
    check(
        atlas_operations[0].kind == strata::font::AtlasOperationKind::create &&
    ...
    check(atlas.take_operations().empty(), "native glyph atlas replayed consumed upload operations");
    atlas.adopt_display_scale(2.0);
    const std::vector<strata::font::AtlasOperation> scale_operations = atlas.take_operations();
    check(
        scale_operations.size() == 1U &&
    ...
    const std::array<strata::font::GlyphAtlasWarmupRequest, 2U> batched_requests{
        strata::font::GlyphAtlasWarmupRequest{
            "strata:fonts/default-medium", &font, latin_a, 12.0, {}, 0U,
            strata::font::GlyphRasterMode::coverage,
        },
        strata::font::GlyphAtlasWarmupRequest{
            "strata:fonts/default-medium", &font, latin_b, 12.0, {}, 0U,
            strata::font::GlyphRasterMode::coverage,
        },
    };
    ...
    for (std::uint32_t code_point = 'D'; code_point <= 'T'; ++code_point) {
        ...
        parallel_requests.push_back(strata::font::GlyphAtlasWarmupRequest{
            "strata:fonts/default-medium", &font, glyph, 13.0, {}, 0U,
            strata::font::GlyphRasterMode::coverage,
        });
    }
    ...
    check(
        bounded_atlas.request_coverage(
            "strata:fonts/default-medium", font, latin_a, 12.0, {}
        ).has_value(),
    ...
    try {
        static_cast<void>(bounded_atlas.request_coverage(
            "strata:fonts/default-medium", font, latin_b, 12.0, {}
        ));
        check(false, "bounded atlas recycled while published geometry could reference its pages");
    } catch (const strata::font::FontError&) {
    }
    check(
        bounded_atlas.reclamation_pending() &&
        bounded_atlas.generation() == bounded_generation,
        "bounded atlas capacity pressure mutated a generation outside safe preparation"
    );
    check(
        bounded_atlas.begin_frame_preparation() &&
    ...
    check(
        !bounded_atlas.frame_preparation_active() &&
    ...
    check(
        one_recycle_atlas.request_coverage(
            "strata:fonts/default-medium", font, latin_a, 12.0, {}
        ).has_value() &&
        one_recycle_atlas.request_coverage(
            "strata:fonts/default-medium", font, latin_b, 12.0, {}
    ...
    try {
        static_cast<void>(one_recycle_atlas.request_coverage(
            "strata:fonts/default-medium", font, latin_c, 12.0, {}
        ));
        check(false, "atlas exceeded its bounded per-preparation recycle budget");
    } catch (const strata::font::FontError&) {
    }
    check(
        one_recycle_atlas.reclamation_pending() &&
...
void test_bundled_texture_descriptor(const std::filesystem::path& resource_root) {
    ...
    const strata::resource::ResourceBytes png = strata::resource::load_binary_resource(
        ...
        strata::resource::ResourceId::parse(
            "assets/strata/textures/ui/icons/chevron-down.png"
        )
    );
    const strata::resource::ImageDimensions dimensions = strata::resource::inspect_png(png);
    check(
        dimensions.width == 32U && dimensions.height == 32U,
...
void test_render_packet_batches_every_portable_command() {
    ...
    commands.append(CustomMeshRenderCommand{
        ...
        MaterialState{
            "fixture.material", "straight_alpha", 0.75,
...
void test_render_submission_cache(const std::filesystem::path& resource_root) {
    ...
    const ui::RenderSubmission* first = &cache.resolve(
        commands, atlas, *text_engine, environment
    );
    check(
        cache.hit_count() == 0U && cache.miss_count() == 1U &&
            !first->vertex_bytes.empty() && !first->indices.empty(),
        "render submission cache did not populate from native geometry"
    );
    const ui::RenderSubmission* settled = &cache.resolve(
        commands, atlas, *text_engine, environment
    );
    check(
        settled == first && cache.hit_count() == 1U && cache.miss_count() == 1U,
    ...
    check(
        cache.hit_count() == 1U && cache.miss_count() == 2U,
    ...
    check(
        cache.hit_count() == 1U && cache.miss_count() == 5U,
    ...
    check(visible_glyph != 0U && clipped_glyph != 0U,
          "submission culling fixture lost its glyphs");
    ui::RenderCommandBuffer culled_text;
    culled_text.append(ui::TextRunRenderCommand{
        {10.0, 10.0}, {}, 12.0,
        {ui::LogicalGlyph{
            "strata:fonts/default-medium", visible_glyph, 'A', 0U, 1U,
            0.0, 9.0, 7.0,
        }},
    ...
    culled_text.append(ui::TextRunRenderCommand{
        {10.0, 600.0}, {}, 12.0,
        {ui::LogicalGlyph{
            "strata:fonts/default-medium", clipped_glyph, 'B', 0U, 1U,
            0.0, 9.0, 7.0,
        }},
        ui::FontRasterization::grayscale,
    ...
    const ui::RenderSubmission& culled_submission = culled_cache.resolve(
        culled_text, culled_atlas, *text_engine, environment
    );
    check(
        culled_submission.planned_draws == 1U &&
            culled_atlas.cached_glyph_count() == 1U,
...
void test_authored_material_scope_is_fill_local() {
    ...
    const ui::RenderSubmission submission = ui::build_render_submission(
        commands, atlas, nullptr, 1.0, 640, 480, 640.0, 480.0
    );
    check(
        submission.planned_draws == 3U && submission.batches.size() == 2U &&
    ...
    check(
        submission.vertex_bytes.size() >= first_vertex_draw_mode_offset + sizeof(float),
        "authored rounded fill emitted no draw data"
    );
    float draw_mode = 0.0F;
    std::memcpy(
        &draw_mode,
        submission.vertex_bytes.data() + first_vertex_draw_mode_offset,
        sizeof(draw_mode)
    );
    check(
        draw_mode == 2.0F,
        "an authored fill replaced the rounded primitive's geometry mode"
    );
}

float submission_vertex_component(
...
void check_translation_reused_geometry(
    const strata::ui::RenderSubmission& actual,
    const strata::ui::RenderSubmission& expected
) {
    constexpr std::size_t vertex_bytes = 88U;
    check(
        actual.vertex_bytes.size() == expected.vertex_bytes.size() &&
            actual.vertex_bytes.size() % vertex_bytes == 0U &&
            actual.indices == expected.indices && actual.batches.size() == expected.batches.size(),
    ...
    for (std::size_t vertex = 0U; vertex < vertex_count; ++vertex) {
        ...
        check(
            std::abs(
                submission_vertex_component(actual.vertex_bytes, vertex, 0U) -
        ...
        check(
            std::abs(
                submission_vertex_component(actual.vertex_bytes, vertex, 1U) -
        ...
        check(
            std::equal(
                actual.vertex_bytes.begin() + static_cast<std::ptrdiff_t>(vertex * vertex_bytes + 8U),
                actual.vertex_bytes.begin() + static_cast<std::ptrdiff_t>((vertex + 1U) * vertex_bytes),
                expected.vertex_bytes.begin() + static_cast<std::ptrdiff_t>(vertex * vertex_bytes + 8U)
            ),
...
void test_render_submission_translation_reuse(const std::filesystem::path& resource_root) {
    ...
    const ui::RenderSubmission& reused = retained.resolve(
        moved, atlas, *text_engine, environment
    );
...
void test_native_nine_patch_geometry(const std::filesystem::path& resource_root) {
    ...
    check(
        submission.vertex_bytes.size() == 36U * 88U && submission.indices.size() == 54U &&
    ...
    check(
        cache.hit_count() == 1U && cache.miss_count() == 1U,
    ...
    static_cast<void>(packet_cache.encode(
        commands,
        1U,
        textures,
        atlas,
    ...
    const std::vector<std::uint8_t>& descriptor_only_packet = packet_cache.encode(
        commands,
        2U,
        {},
        atlas,
    ...
    check(
        atlas.request_coverage(
            "strata:fonts/control", text_engine->control_font(), teardown_glyph, 12.0, {}
        ).has_value(),
    ...
    static_cast<void>(packet_cache.encode(
        commands, 3U, {}, atlas, *text_engine, 1.0, 640, 480, 640.0, 480.0
    ));
    ...
    check(
        release_packet.size() >= release_offset &&
            std::string_view(
                reinterpret_cast<const char*>(release_packet.data()), release_offset
            ) == "STRATARP",
    ...
    check(
        release_offset == release_packet.size() &&
            std::ranges::find(released_textures, textures.front().host_id) !=
                released_textures.end() &&
            std::ranges::any_of(released_textures, [](const std::string& texture) {
...
void test_native_custom_mesh_geometry(const std::filesystem::path& resource_root) {
    ...
    check(
        submission.vertex_bytes.size() == 3U * 88U && submission.indices.size() == 3U &&
...
void test_gradient_paint_authoring_and_tessellation() {
    ...
    check(paint.has_value() && paint->is_gradient(), "authored linear gradient was not read as a paint");
    const ui::Gradient& gradient = *paint->gradient();
    check(
        gradient.sample(0.5) == runtime::ColorValue{128U, 128U, 128U, 255U},
        "gradient sampling is not a linear ramp between its stops"
    );
    check(
        gradient.sample(-1.0) == gradient.stops.front().color &&
    ...
    check(
        near(start.x, 0.0) && near(start.y, 0.5) && near(end.x, 1.0) && near(end.y, 0.5),
    ...
    check(
        mesh.vertices.size() == 4U && mesh.indices.size() == 6U,
    ...
    check(
        mesh.vertices.front().color == gradient.stops.front().color &&
    ...
    check(
        !rings.indices.empty() && rings.indices.size() % 3U == 0U,
    ...
    ui::RenderCommandBuffer commands;
    commands.append(ui::RoundedRectRenderCommand{
        ui::Rect{0.0, 0.0, 120.0, 60.0}, ui::CornerRadii::all(8.0), *paint, std::nullopt, 1.0,
    });
    const ui::RenderCommand faded = ui::render_command_with_opacity(commands.commands().front(), 0.5);
    const ui::Paint& faded_fill = std::get<ui::RoundedRectRenderCommand>(faded).fill;
    check(
        faded_fill.is_gradient() && faded_fill.gradient()->stops.back().color.alpha == 128U,
...
void test_vector_shape_tessellation() {
    ...
    check(
        parsed.segments().size() == 4U &&
            parsed.segments()[2].to == ui::Point{1.0, 1.0} &&
    ...
    check(
        fill_mesh.indices.size() >= 3U && fill_mesh.indices.size() % 3U == 0U,
        "a filled triangle must tessellate into triangles"
    );
    const bool feathered = std::ranges::any_of(
        fill_mesh.vertices,
        [](const ui::PaintVertex& vertex) { return vertex.color.alpha == 0U; }
    );
    ...
    stroked.stroke_style = ui::StrokeStyle{4.0, ui::PathCap::round, ui::PathJoin::round, 4.0, {}, 0.0};
    const ui::PaintMesh stroke_mesh = ui::tessellate_shape(stroked, ui::Size{120.0, 120.0}, 2.0);
    check(
        !stroke_mesh.indices.empty() && stroke_mesh.indices.size() % 3U == 0U,
        "a stroked arc must tessellate into triangles"
    );
    const bool indexed = std::ranges::all_of(
        stroke_mesh.indices,
        [&stroke_mesh](const std::uint32_t index) { return index < stroke_mesh.vertices.size(); }
    );
    ...
    const auto painted_area = [](const ui::PaintMesh& mesh) {
        ...
        for (std::size_t index = 0U; index + 2U < mesh.indices.size(); index += 3U) {
            ...
            if (a.color.alpha == 0U || b.color.alpha == 0U || c.color.alpha == 0U) continue;
            total += std::abs(
                (b.normalized.x - a.normalized.x) * (c.normalized.y - a.normalized.y) -
                (c.normalized.x - a.normalized.x) * (b.normalized.y - a.normalized.y)
            ) * 0.5;
        }
        return total;
    };
    const double solid_area = painted_area(ui::tessellate_shape(solid, ui::Size{120.0, 120.0}, 2.0));
    const double dashed_area = painted_area(
        ui::tessellate_shape(dashed, ui::Size{120.0, 120.0}, 2.0)
    );
    check(
        dashed_area > 0.0 && dashed_area < solid_area * 0.75,
...
void test_keyed_reconciliation_and_detach_cleanup() {
    ...
    const auto first = node("Panel", "root", {
        node("Text", "a", {}, {{"text", strata::runtime::ExpressionValue(strata::runtime::Value("A"))}}),
        node("Text", "b", {}, {{"text", strata::runtime::ExpressionValue(strata::runtime::Value("B"))}}),
    });
    const ReconcileStats initial = tree.reconcile(first);
    check(initial.created == 3U && initial.materialized == 2U, "initial retained materialization changed");
    ...
    check(tree.find_source("/fixture") != nullptr && tree.find_source("/fixture")->size() == 3U, "source index changed");
    tree.clear_dirty();

    std::uint64_t detached = 0U;
    tree.find_key("a")->add_cleanup([&detached] { ++detached; });
    check(tree.find_key("b")->set_retained_value("selection", strata::runtime::Value(2.0)), "retained value write failed");
    const auto reordered = node("Panel", "root", {
        node("Text", "b", {}, {{"text", strata::runtime::ExpressionValue(strata::runtime::Value("B2"))}}),
        node("Text", "a", {}, {{"text", strata::runtime::ExpressionValue(strata::runtime::Value("A"))}}),
    });
    const ReconcileStats reorder = tree.reconcile(reordered);
    check(
        reorder.created == 0U && reorder.reused == 3U && tree.find_key("a")->identity() == a_identity &&
    ...
    check(tree.find_key("b")->structural_path() == "/0", "reordered structural path was not refreshed");
    check(tree.find_key("b")->dirty().contains(DirtyReason::text), "text property change missed its dirty reason");

    const auto removed = node("Panel", "root", {node("Text", "b")});
    const ReconcileStats removal = tree.reconcile(removed);
    check(removal.detached == 1U && detached == 1U && tree.find_key("a") == nullptr, "detached cleanup was not exact");
    tree.clear();
...
void test_exit_retention_and_prune_ownership() {
    ...
    const auto owns_scope = [](
        ...
        result->materialization_result = std::make_shared<const DescriptionMaterialization>(
            ...
            std::vector<strata::runtime::RuntimeDiagnostic>{},
            0U,
            0U
        );
        return std::shared_ptr<const DescriptionNode>(std::move(result));
    };
    const std::shared_ptr<const DescriptionNode> exiting_child = owns_scope(
        node("Text", "exiting.child"),
        "scope.exiting.child"
    );
    const std::shared_ptr<const DescriptionNode> exiting = owns_scope(
        node("Panel", "exiting", {exiting_child}),
        "scope.exiting"
    );
    const std::shared_ptr<const DescriptionNode> stable = owns_scope(
        node("Text", "stable"),
        "scope.stable"
    );
    RetainedTree tree;
    ...
    check(
        attached_description_state_scopes(tree) == strata::runtime::StateScopeSet{
            "scope.exiting", "scope.exiting.child", "scope.stable",
        },
    ...
    static_cast<void>(tree.reconcile(
        node("Panel", "root", {stable}),
        [](const RetainedNode& candidate) {
            return candidate.description().key == "exiting";
        }
    ));
    check(tree.root()->children().size() == 2U, "exit boundary was not retained after attached children");
    check(
        tree.root()->children()[0] != nullptr && tree.root()->children()[1] != nullptr,
        "exit reconciliation left a null retained owner"
    );
    check(
        tree.find_key("exiting")->lifecycle() == RetainedLifecycle::exiting &&
            tree.find_key("stable")->lifecycle() == RetainedLifecycle::attached,
        "exit reconciliation corrupted retained lifecycle state"
    );
    check(
        attached_description_state_scopes(tree) ==
            strata::runtime::StateScopeSet{"scope.stable"},
        "an EXITING lifecycle boundary retained its own or descendant state scopes"
    );

    check(
        tree.prune_exiting([](const RetainedNode&) { return false; }) == 0U &&
            tree.root()->children().size() == 2U &&
            tree.root()->children()[0] != nullptr && tree.root()->children()[1] != nullptr,
        "a no-op exit prune moved ownership out of the retained tree"
    );
    check(
        tree.prune_exiting([](const RetainedNode&) { return true; }) == 2U &&
            tree.root()->children().size() == 1U &&
            tree.find_key("exiting") == nullptr && tree.find_key("stable") != nullptr &&
            cleanup_count == 1U,
...
void test_lazy_materialization_and_noop_reconcile() {
    ...
    auto generated_children = std::make_shared<const GeneratedDescriptionChildren>(
        1'000U,
        [&generated](const std::size_t index) {
            ++generated;
    ...
    check(initial.created == 11U && generated == 10U, "lazy description materialized outside its range");
    ...
    check(
        !settled.changed() && settled.reused == 11U && generated == 10U &&
...
void test_retained_virtual_realization_window() {
    ...
    const auto provider = std::make_shared<const GeneratedDescriptionChildren>(
        ...
        [&generated](const std::size_t index) {
            ++generated;
            auto row = std::make_shared<DescriptionNode>(*DescriptionNode::create(
                "Row", "row." + std::to_string(index)
            ));
            row->materialization_result =
                std::make_shared<const DescriptionMaterialization>(
    ...
    const auto lazy_root = [&provider](const runtime::Value& revision) {
        auto root = std::make_shared<DescriptionNode>(*DescriptionNode::create(
            "VirtualList",
            "virtual",
            {},
            {},
            {{"revision", runtime::ExpressionValue(revision)}},
            provider
        ));
    ...
    check(
        tree.root() != nullptr && tree.root()->children().empty() &&
    ...
    const ReconcileStats first = tree.realize_children(
        ...
        rows(provider, MaterializationRange{10U, 13U})
    );
    check(
        first.created == 3U && generated == 3U &&
    ...
    check(
        owner_update.updated == 1U && tree.root()->children().size() == 3U &&
    ...
    for (std::size_t index = 12U; index < 15U; ++index) {
        std::shared_ptr<const DescriptionNode> description =
            tree.root()->realized_child_description(
                index, provider, projected_theme, std::nullopt, 7U
            );
        if (description == nullptr) description = provider->at(index);
        forward.push_back(RealizedDescriptionChild{index, std::move(description)});
    }
    static_cast<void>(tree.realize_children(
        tree.root()->identity(),
        provider,
        projected_theme,
        std::nullopt,
        7U,
        MaterializationRange{12U, 15U},
        std::move(forward)
    ));
    check(
        generated == 5U && tree.find_key("row.12")->identity() == overlap_identity &&
    ...
    const auto replacement = std::make_shared<const GeneratedDescriptionChildren>(
        100U,
        [&replacement_generated](const std::size_t index) {
    ...
    check(
        replacement_generated == 3U &&
            tree.root()->realization_current(
                replacement,
                projected_theme,
                std::nullopt,
                8U,
    ...
    static_cast<void>(tree.reconcile(DescriptionNode::create(
        "VirtualList",
        "virtual",
        {},
        {},
    ...
    check(
        !tree.root()->realized_range().has_value() &&
            ...
            tree.root()->children().size() == 1U &&
            tree.find_key("row.eager") != nullptr,
...
void test_lazy_range_convergence_state_machine() {
    ...
    check(
        cycle.observe(first) == LazyConvergenceStatus::progress &&
    ...
    check(
        fixed.observe(fixed_signature) == LazyConvergenceStatus::fixed_point &&
    ...
    check(
        generic_reconcile.observe(no_lazy_producers) ==
                LazyConvergenceStatus::fixed_point &&
            generic_reconcile.observe(no_lazy_producers) ==
                LazyConvergenceStatus::cycle &&
...
void test_materialization_publication_identity_and_eviction() {
    ...
    check(
        transferred_id != 0U && move_source.transaction_id() == 0U &&
    ...
    auto generated = std::make_shared<const GeneratedDescriptionChildren>(
        1U,
        [&evaluations](const std::size_t) {
            ++evaluations;
            auto row = std::make_shared<DescriptionNode>(*DescriptionNode::create(
                "Row", "transaction.row"
            ));
    ...
    for (std::size_t iteration = 0U; iteration < 32U; ++iteration) {
        {
            ...
            const std::uint64_t next_id =
                rematerialized->materialization_result->transaction_id();
            check(
                next_id > previous_id &&
                    ledger.claim(rematerialized->materialization_result) ==
                        MaterializationPublicationClaim::publish &&
                    ledger.tracked_count() == 1U,
                "evicted row rematerialization did not publish a fresh stable transaction identity"
            );
            previous_id = next_id;
        }
        ledger.purge_expired();
    ...
    check(evaluations == 33U, "evicted generated rows did not rematerialize exactly once per visit");
}
...
void test_variable_virtual_extents_and_stable_anchor() {
    ...
    const auto virtual_list = [](const std::vector<std::pair<std::string, double>>& rows) {
        ...
        for (const auto& [key, height] : rows) {
            children.push_back(node("Panel", key, {}, layout_properties(object({
    ...
    check(
        inserted->virtual_items != nullptr &&
            inserted->virtual_items->count() == 4U &&
            inserted->virtual_items->index_of_key("b") ==
...
void test_virtualization_cache_queries_only_observed_keys() {
    ...
    public:
        ...
        [[nodiscard]] std::string key_at(const std::size_t index) const override {
            if (index >= count_) throw std::out_of_range("counting key index is outside the sequence");
            ++key_queries;
        ...
        [[nodiscard]] std::optional<std::size_t> index_of_key(
            ...
            if (suffix.empty() ||
                !std::ranges::all_of(suffix, [](const unsigned char value) {
    ...
    static_cast<void>(cache.resolve(
        7U,
        VirtualExtentRequest{initial, 20.0, std::nullopt, false},
        65.0,
        std::span<const VirtualMeasurement>{}
    ));
    check(
        initial->key_queries == 0U && initial->lookup_queries == 0U,
    ...
    const VirtualExtentResolution anchored = cache.resolve(
        7U,
        VirtualExtentRequest{replacement, 20.0, std::nullopt, false},
        65.0,
        std::span<const VirtualMeasurement>{}
    );
    check(
        initial->key_queries == 1U && replacement->key_queries == 0U &&
    ...
    const VirtualExtentResolution reset = cache.resolve(
        7U,
        VirtualExtentRequest{sorted, 20.0, std::nullopt, false, true},
        65.0,
        std::span<const VirtualMeasurement>{}
    );
    check(
        reset.anchor_changed && reset.offset == 0.0 && replacement->key_queries == 0U &&
...
void test_retained_layout_cache_scale_and_invalidation() {
    ...
    check(fixed_record != nullptr && fill_record != nullptr, "layout records lost keyed retained identities");
    ...
    check_near(
        fixed_record->snapped_bounds.right() * environment.scale,
...
void test_retained_layout_cache_translates_unchanged_subtrees() {
    ...
    const auto description = [](const double leading_height) {
        return node("Panel", "translation.root", {
            node("Panel", "translation.leading", {}, layout_properties(object({
                {"height", runtime::Value(leading_height)},
                {"width", runtime::Value(20.0)},
            }))),
            node("Panel", "translation.trailing", {
                node("Panel", "translation.inner", {}, layout_properties(object({
    ...
    check(
        shifted.operations.arranged_nodes == 2U && shifted_trailing != nullptr &&
            shifted_inner != nullptr,
        "translated arrangement cache did not skip the unchanged trailing subtree"
    );
    check_near(
        shifted_trailing->bounds.y,
        initial_trailing.bounds.y + 3.25,
        "translated arrangement cache misplaced its subtree root"
    );
    check_near(
        shifted_inner->bounds.y,
        initial_inner.bounds.y + 3.25,
        "translated arrangement cache misplaced a descendant"
    );
    check(
        initial_trailing.local_clip.has_value() && initial_inner.clip.has_value() &&
    ...
    check_near(
        shifted_trailing->snapped_bounds.y * environment.scale,
    ...
    check(
        shifted_again.operations.arranged_nodes == 2U,
        "translated arrangement cache did not advance its retained placement"
    );
    check_near(
        shifted_again.find(trailing_identity)->bounds.y,
        shifted_y + 1.25,
...
void test_explicit_container_size_constrains_descendants() {
    ...
    const auto fill_child = [](const std::string& key) {
        return node("Panel", key, {}, layout_properties(object({
            {"height", runtime::Value(10.0)},
            {"width", object({{"weight", runtime::Value(1.0)}})},
        })));
    };
    RetainedTree tree;
    static_cast<void>(tree.reconcile(node("Panel", "constraint.root", {
    ...
    const LayoutResult& result = layout.layout(tree, LayoutEnvironment{
        0U, Rect{0.0, 0.0, 300.0, 100.0}, 1.0, {},
        PointSnapPolicy::nearest, RectangleSnapPolicy::outward, false,
    });
    ...
    check(
        second->bounds.right() <= shell->bounds.right() - 10.0,
...
void test_scroll_content_box_constrains_fill_child() {
    ...
    RetainedTree tree;
    static_cast<void>(tree.reconcile(node("Scroll", "scroll.box", {
        node("Panel", "scroll.fill", {}, layout_properties(object({
    ...
    LayoutEngine layout;
    const LayoutResult& result = layout.layout(tree, LayoutEnvironment{
        0U, Rect{0.0, 0.0, 100.0, 100.0}, 1.0, {},
        PointSnapPolicy::nearest, RectangleSnapPolicy::outward, false,
    });
...
void test_exiting_child_retains_placement_without_affecting_flow() {
    ...
    const LayoutRecord* stable_record = transitioned.find(tree.find_key("exit.flow.stable")->identity());
...
void test_nested_scroll_pin_uses_nearest_scroll_offset() {
    ...
    scroll.emplace("scrollOffset", runtime::ExpressionValue(object({
        {"x", runtime::Value(0.0)}, {"y", runtime::Value(50.0)},
    })));
    RetainedTree tree;
    static_cast<void>(tree.reconcile(node("Scroll", "pin.root", {
    ...
    check_near(pinned_record->bounds.y, 0.0, "nested scroll pin did not cancel nearest scroll offset");
}
...
void test_wrapped_linear_and_intrinsic_grid_layout() {
    ...
    static_cast<void>(row_tree.reconcile(node("Panel", "wrap.row.root", {
        node("Row", "wrap.row", {
    ...
    const LayoutRecord* row_first = row_result.find(row_tree.find_key("wrap.row.first")->identity());
    const LayoutRecord* row_second = row_result.find(row_tree.find_key("wrap.row.second")->identity());
    const LayoutRecord* row_third = row_result.find(row_tree.find_key("wrap.row.third")->identity());
    ...
    static_cast<void>(column_tree.reconcile(node("Panel", "wrap.column.root", {
    ...
    const LayoutRecord* column_first = column_result.find(
        column_tree.find_key("wrap.column.first")->identity()
    );
    const LayoutRecord* column_third = column_result.find(
        column_tree.find_key("wrap.column.third")->identity()
    );
    ...
    grid_properties.emplace("columns", runtime::ExpressionValue(runtime::Value(
    ...
    grid_properties.emplace("rows", runtime::ExpressionValue(runtime::Value(
        std::vector<runtime::Value>{runtime::Value("content"), runtime::Value("content")}
    )));
    RetainedTree grid_tree;
    static_cast<void>(grid_tree.reconcile(node("Panel", "grid.root", {
    ...
    const LayoutRecord* grid_second = grid_result.find(grid_tree.find_key("grid.second")->identity());
...
void test_content_size_motion_interrupts_and_settles() {
    ...
    const auto moving_child = [](const double width) {
        ...
        properties.emplace("animateContentSize", runtime::ExpressionValue(object({
            {"clip", runtime::Value(true)},
    ...
    const auto moving_root = [&moving_child](const double width) {
        return node("Panel", "motion.root", {
            moving_child(width),
            node("Spacer", "motion.stable", {}, layout_properties(object({
                {"height", runtime::Value(10.0)},
    ...
    check(
        midpoint.operations.measured_nodes == 2U &&
    ...
    check_near(interrupted_midpoint.find(tree.find_key("motion.child")->identity())->bounds.width, 65.0,
               "interrupted content-size midpoint changed");
...
void test_scroll_virtual_range_clipping_and_safe_insets() {
    ...
    auto description = DescriptionNode::create(
        "List",
        "virtual.root",
        {},
        {},
    ...
    ranged->properties.emplace("scrollOffset", runtime::ExpressionValue(object({
        {"x", runtime::Value(0.0)}, {"y", runtime::Value(2'000.0)},
    })));
    RetainedTree tree;
...
void test_active_work_scheduler_and_detach_cleanup() {
    ...
    WorkScheduler scheduler([&failures](WorkKind, std::uint64_t, std::string_view, std::string_view) {
        ++failures;
    });
    ...
    static_cast<void>(isolated.schedule(
        WorkKind::service,
        1U,
        "isolated",
        [](const std::int64_t) { return true; }
...
void test_portable_description_and_declaration_state(
    ...
    const auto no_imports = [](const std::string_view, const std::string_view path) -> compiler::ModuleSource {
        throw compiler::ModuleLoadError("unexpected import '" + std::string(path) + "'");
    };
    check(
        application.compile_and_activate(
            compiler::ModuleSource{"app/main.strata", load(scenario_directory, "main.strata")},
            no_imports,
            0U
        ).activated(),
    ...
    check(description.root != nullptr && description.diagnostics.empty(), "portable description produced diagnostics");
    ...
    check(
        tree.find_key("dynamic.root") != nullptr && tree.find_key("dynamic.increment") != nullptr &&
    ...
    const ui::LayoutResult& laid_out = layout.layout(tree, ui::LayoutEnvironment{
        0U, ui::Rect{0.0, 0.0, 960.0, 720.0}, 1.0, {},
        ui::PointSnapPolicy::nearest, ui::RectangleSnapPolicy::outward, false,
    });
    ...
    check(
        count_text != tree.find_key("dynamic.count")->description().properties.end() &&
    ...
    const auto action_property = tree.find_key("dynamic.increment")->description().properties.find("onClick");
    check(
        action_property != tree.find_key("dynamic.increment")->description().properties.end() &&
    ...
    check(adjusted.status == runtime::ActionDispatchStatus::handled, "described framework action did not dispatch");
    ...
    check(
        update.created == 0U && tree.find_key("dynamic.count")->identity() == count_identity &&
            updated_text->second.value() != nullptr && *updated_text->second.value()->string() == "Count 1" &&
    ...
    check(
        surface_application.compile_and_activate(
            compiler::ModuleSource{"app/main.strata", load(scenario_directory, "main.strata")},
            no_imports,
            0U
        ).activated(),
    ...
    runtime::HostServices host_services(
        [] { return true; },
        [&host_clipboard] {
            return runtime::HostClipboardRead{0U, host_clipboard};
        },
    ...
    check(first_frame.operations.rebuilds == 1U, "initial surface frame did not rebuild exactly once");
    check(surface.tree().root()->structural_path() == "/", "surface retained root path is not canonical");
    check(
        surface.tree().find_key("dynamic.root")->structural_path() == "/0/0",
    ...
    check(
        inspected_root != nullptr && inspected_root->find("componentType") != nullptr &&
    ...
    check(
        inspected_editors != nullptr && inspected_editors->array() != nullptr &&
        ...
        std::ranges::any_of(
            ...
            [](const ui::RenderCommand& command) {
                ...
                return border != nullptr && border->border.width == 2.0 &&
                    border->border.color == ui::RenderColor{255U, 191U, 76U, 255U};
            }
        ),
    ...
    check(
        settled_frame.operations.rebuilds == 0U &&
    ...
    const ui::LayoutRecord* stable_hover_layout = stable_hover_node != nullptr
        ? surface.layout().find(stable_hover_node->identity())
        : nullptr;
    ...
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        stable_hover, ui::PointerEventType::move, 0, 0,
    }));
    ...
    check(
        stable_pointer_frame.operations.input_events_processed == 1U &&
    ...
    check(
        cached_pointer_frame.operations.input_events_processed == 1U &&
    ...
    check(
        blocked.events.empty() && blocked.action_outcomes.empty() &&
    ...
    check(
        close_modal.events.size() >= 2U &&
            modal_open != nullptr && modal_open->boolean() != nullptr && !*modal_open->boolean(),
    ...
    check(
        ime_active_updates == std::vector<bool>{true} &&
            ime_rect_updates.size() == 1U && ime_rect_updates.front().width > 0.0 &&
            ime_rect_updates.front().height > 0.0,
    ...
    const data::JsonValue* committed_source = !committed.events.empty()
        ? committed.events.front().find("source")
        : nullptr;
    const data::JsonValue* committed_surface_id = committed_source != nullptr
        ? committed_source->find("surfaceId")
        : nullptr;
    check(
        committed.events.size() == 1U &&
            committed_surface_id != nullptr && committed_surface_id->string() != nullptr &&
            *committed_surface_id->string() == public_surface_id &&
            *committed_surface_id->string() != private_host_owner &&
            surface.input().edited_text(surface.tree().find_key("dynamic.editor")->identity()) != nullptr &&
    ...
    check(
        host_clipboard == "clipboard value✓",
        "editor copy did not reach the installed host clipboard"
    );
    surface.cancel_interactions();
    check(
        ime_active_updates == std::vector<bool>{true, false},
        "focus cancellation did not synchronously release the runtime-owned host IME"
    );
    host_services.request_ime(
        "sibling-surface",
        runtime::HostServiceRect{12.0, 24.0, 2.0, 18.0}
    );
    check(
        ime_active_updates == std::vector<bool>{true, false, true},
    ...
    check(
        ime_active_updates == std::vector<bool>{true, false, true, false},
    ...
    host_services.request_ime(
        teardown_private_owner,
        runtime::HostServiceRect{4.0, 8.0, 2.0, 16.0}
    );
    {
        ui::Surface teardown_surface(
            std::string(teardown_public_id),
            surface_application,
            runtime::LayerRole::overlay,
            "Main",
            surface_environment,
            std::shared_ptr<const ui::TextEngine>{},
            ui::WidgetRegistry{},
            ui::BehaviorRegistry{},
            &host_services,
            ui::Theme{},
        ...
        check(
            ime_active_updates == std::vector<bool>{true, false, true, false, true},
            "teardown public id incorrectly released the private host owner"
        );
    }
    check(
        ime_active_updates == std::vector<bool>{true, false, true, false, true, false},
...
void test_repeater_description_is_data_backed() {
    ...
    const auto collection_property = [](
        ...
        auto view = std::shared_ptr<runtime::CollectionViewValue>(
            new runtime::CollectionViewValue{
    ...
    check(
        !ui::expression_value_equal(retained_property, rebuilt_property) &&
...
overlay IndexedDependencyRepeater {
  root Repeater(key: "indexed.repeater", estimatedItemExtent: 24) {
    ...
    check(
        ...
        "lazy repeater fixture did not activate: " + activation_failure
    );
    ui::DescriptionBuilder builder(application);
    ui::RetainedTree retained;
    std::weak_ptr<const runtime::IndexableSequence> retained_sequence;
    {
        ui::DescriptionBuildResult built = builder.build(
            runtime::LayerRole::overlay,
            "LazyRepeater"
        );
        ...
        const runtime::Value* repeater_layout =
            repeater_layout_property != repeater->properties.end()
                ...
                : nullptr;
        check(
            repeater->type == "VirtualList" && repeater->children->size() == 3U &&
                repeater->materialization == std::optional<ui::MaterializationRange>(
                    ui::MaterializationRange{}
                ) &&
        ...
        check(
            third->key == std::optional<std::string>("TWO") &&
                third->materialization_key == std::optional<std::string>("TWO") &&
                third->children->size() == 1U &&
                third->materialization_result != nullptr &&
        ...
        check(
            unrelated.has_value() && unrelated->declaration != nullptr &&
        ...
        ui::DescriptionBuildResult settled = builder.build(
            runtime::LayerRole::overlay,
            "LazyRepeater"
        );
        ...
        check(
            settled_repeater->virtual_sequence == repeater->virtual_sequence &&
        ...
        check(
            identity_suffix.has_value() && identity_suffix->declaration != nullptr &&
                application.state().write(
                    identity_suffix->address,
                    runtime::StateSlot{
                        "identitySuffix",
        ...
        ui::DescriptionBuildResult identity_changed = builder.build(
            runtime::LayerRole::overlay,
            "LazyRepeater"
        );
        check(
            identity_changed.root->children->at(0U)->virtual_sequence !=
                repeater->virtual_sequence &&
                identity_changed.root->children->at(0U)->virtual_sequence->key_at(1U) ==
                    "TWO!",
    ...
    try {
        ...
        duplicate_rejected = std::string_view(error.what()).find("duplicated") !=
    ...
    try {
        ...
        invalid_key_rejected = std::string_view(error.what()).find("non-empty") !=
            std::string_view::npos;
    }
...

    {
        ...
        ui::DescriptionBuildResult one = builder.build(
            runtime::LayerRole::overlay,
            "CollectionMetadata"
        );
        const std::shared_ptr<const ui::DescriptionNode> one_repeater =
            one.root->children->at(0U);
        const auto one_view = one_repeater->virtual_sequence_generation
                                  ->lexical_dependencies.find("view");
        check(
            one_repeater->virtual_sequence->key_at(0U) == "one.row" &&
                one_view != one_repeater->virtual_sequence_generation
                                ->lexical_dependencies.end() &&
                one_view->second.kind ==
        ...
        ui::DescriptionBuildResult two = builder.build(
            runtime::LayerRole::overlay,
            "CollectionMetadata"
        );
        const std::shared_ptr<const ui::DescriptionNode> two_repeater =
            two.root->children->at(0U);
        check(
            two_repeater->virtual_sequence != one_repeater->virtual_sequence &&
        ...
        ui::DescriptionBuildResult same_metadata = builder.build(
            runtime::LayerRole::overlay,
            "CollectionMetadata"
        );
        check(
            same_metadata.root->children->at(0U)->virtual_sequence ==
                two_repeater->virtual_sequence &&
                same_metadata.root->children->at(0U)->virtual_sequence->key_at(0U) ==
                    "many.row",
        ...
        ui::DescriptionBuildResult stable_cache_hit = builder.build(
            runtime::LayerRole::overlay,
            "CollectionMetadata"
        );
        check(
            stable_cache_hit.root->children->at(0U)->virtual_sequence ==
                two_repeater->virtual_sequence &&
...

    {
        ...
        ui::DescriptionBuildResult stable = builder.build(
            runtime::LayerRole::overlay,
            "DirectCollectionMetadata"
        );
        check(
            stable.root->children->at(0U)->virtual_sequence ==
                two_repeater->virtual_sequence,
...

    {
        ...
        check(
            one_repeater->virtual_sequence->count() == 1U &&
                one_repeater->virtual_sequence->key_at(0U) == "A" &&
                one_repeater->virtual_sequence_generation->source.kind ==
                    runtime::ExpressionDependencyValueKind::scalar &&
                lexical != one_repeater->virtual_sequence_generation
                               ->lexical_dependencies.end() &&
        ...
        check(
            source_items.has_value() && source_items->declaration != nullptr &&
                application.state().write(
        ...
        check(
            two.root->children->at(0U)->virtual_sequence !=
                one_repeater->virtual_sequence &&
...

    {
        ...
        ui::DescriptionBuildResult missing = builder.build(
            runtime::LayerRole::overlay,
            "IndexedDependencyRepeater"
        );
        const std::shared_ptr<const ui::DescriptionNode> missing_repeater =
            missing.root->children->at(0U);
        check(
            missing_repeater->virtual_sequence->key_at(0U) == "ZERO" &&
        ...
        ui::DescriptionBuildResult explicit_null = builder.build(
            runtime::LayerRole::overlay,
            "IndexedDependencyRepeater"
        );
        check(
            explicit_null.root->children->at(0U)->virtual_sequence !=
        ...
        ui::DescriptionBuildResult false_guard = builder.build(
            runtime::LayerRole::overlay,
            "IndexedDependencyRepeater"
        );
        ...
        ui::DescriptionBuildResult short_circuited = builder.build(
            runtime::LayerRole::overlay,
            "IndexedDependencyRepeater"
        );
        check(
            short_circuited.root->children->at(0U)->virtual_sequence == false_sequence,
        ...
        ui::DescriptionBuildResult selected_branch = builder.build(
            runtime::LayerRole::overlay,
            "IndexedDependencyRepeater"
        );
        check(
            selected_branch.root->children->at(0U)->virtual_sequence != false_sequence &&
    ...
    {
        ui::DescriptionBuildResult diagnostic_build = builder.build(
            runtime::LayerRole::overlay,
            "DiagnosticRepeater"
        );
        ...
        const std::shared_ptr<const ui::DescriptionNode> second_diagnostic_row =
            diagnostic_repeater->children->at(0U);
        check(
            first_diagnostic_row == second_diagnostic_row &&
    ...
    environment.logical_height = 80.0;
    {
        ...
        check(
            row != nullptr && first_frame.operations.evaluated_expressions != 0U &&
        ...
        check(
            row_state.has_value() && row_state->declaration != nullptr &&
                application.state().write(
                    row_state->address,
                    runtime::StateSlot{
                        "rowFlag",
        ...
        check(
            retained_row_state != nullptr && retained_row_state->boolean() != nullptr &&
    ...
    }
    {
        ...
        check(
            surface.tree().set_retained_value(
                pane->identity(),
        ...
        const runtime::Value* first_layout = pane != nullptr && !pane->children().empty()
            ? pane->children().front()->description().properties.at("$layout").value()
            : nullptr;
        const runtime::Value* first_width = first_layout != nullptr
            ? first_layout->field("width")
            : nullptr;
        check(
            first_width != nullptr && first_width->field("weight") != nullptr &&
    ...
    }
    {
        ...
        const auto diagnostic = std::ranges::find(
            diagnosed.diagnostics.records,
            std::string_view("STRATA.DSL.RUNTIME_TYPE_MISMATCH"),
            [](const runtime::RuntimeDiagnosticRecord& record) {
                return std::string_view(record.diagnostic.code);
            }
        );
        check(
            diagnostic != diagnosed.diagnostics.records.end() &&
        ...
        check(
            settled_diagnostic != settled.diagnostics.records.end() &&
    ...
    std::shared_ptr<const ui::DescriptionNode> detached_repeater;
    {
        ...
        ui::DescriptionBuildResult temporary = temporary_builder.build(
            runtime::LayerRole::overlay,
            "LazyRepeater"
        );
    ...
    check(
        detached_row->key == std::optional<std::string>("TWO"),
    ...
    ui::DescriptionBuilder snapshot_builder(application);
    {
        ...
        ui::DescriptionBuildResult temporary = snapshot_builder.build(
            runtime::LayerRole::overlay,
            "LazyRepeater"
        );
    ...
    ui::DescriptionBuildResult after_retained_destruction = snapshot_builder.build(
        runtime::LayerRole::overlay,
        "LazyRepeater"
    );
    check(
        after_retained_destruction.root->children->at(0U)->virtual_sequence->key_at(1U) ==
            "TWO",
...
overlay LazyRepeater {
  root Repeater(key: "lazy.repeater", estimatedItemExtent: 24) {
    ...
    ui::DescriptionBuildResult replacement = builder.build(
        runtime::LayerRole::overlay,
        "LazyRepeater"
    );
    check(
        replacement.root->children->at(0U)->virtual_sequence->key_at(0U) == "SEVEN",
        "active-unit replacement reused an obsolete Repeater sequence"
    );
    static_cast<void>(retained.reconcile(replacement.root));
    check(
        retained_sequence.expired(),
...
overlay Environment {
  root EnvironmentPanel()
    ...
    check(
        application.compile_and_activate(
            compiler::ModuleSource{"environment.strata", source},
            no_imports,
            0U
        ).activated(),
    ...
    const auto property = [](const ui::Surface& surface, const std::string_view key)
        -> const runtime::Value* {
        ...
        return found != retained->description().properties.end()
                   ? found->second.value()
                   : nullptr;
    };
    ...
    check(
        compact_frame.diagnostics.records.empty() && wide_frame.diagnostics.records.empty(),
    ...
    check(
        property(wide, "environment.viewport") != nullptr &&
    ...
    check(
        compact.adopt_environment_preferences(local_preferences),
    ...
    check(
        compact.adopt_environment(regular_environment),
    ...
    check(
        *property(compact, "environment.viewport")->string() == "regular" &&
...
void test_async_collection_key_isolation_and_lazy_tree_publication(
    ...
    check(
        application.host().adopt(bundle->host_snapshot(
            "async-collection-host",
            1U,
            data::parse_json(R"({"data":{"treeItems":[{"key":"data.tree.folder.0","label":"Folder 0","parentKey":null,"mayHaveChildren":true,"childrenLoaded":false}],"tableRows":[],"gridEntries":[]}})")
        )),
...
overlay AsyncCollections {
  ...
  Panel(key: "async.root", layout: { kind: "COLUMN", width: { weight: 1 }, height: { weight: 1 }, gap: 8, padding: 8 }) {
    ...
    const runtime::ActivationResult activation = application.compile_and_activate(
        compiler::ModuleSource{"async-collections.strata", source},
        no_imports,
        0U
    );
    ...
    {
        ui::DescriptionBuilder builder(application);
        const ui::DescriptionBuildResult idle = builder.build(
            runtime::LayerRole::overlay,
            "AsyncCollections"
        );
        ...
        check(
            virtual_list->children->size() == 1U && query_tree->children->size() == 1U &&
    ...
    const ui::LayoutRecord* folder_layout = folder != nullptr
        ? surface.layout().find(folder->identity()) : nullptr;
    ...
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        disclosure, ui::PointerEventType::press, 31, 0,
    }));
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        disclosure, ui::PointerEventType::release, 31, 0,
    }));
    const ui::SurfaceFrame expanded = surface.frame(1'016'666'667);
    check(
        std::ranges::any_of(expanded.lifecycle_input.events, [](const data::JsonValue& event) {
    ...
    check(
        application.host().adopt(bundle->host_snapshot(
            "async-collection-host",
            2U,
            data::parse_json(R"({"data":{"treeItems":[{"key":"data.tree.folder.0","label":"Folder 0","parentKey":null,"mayHaveChildren":true,"childrenLoaded":true},{"key":"data.tree.asset.0.0","label":"Asset 0","parentKey":"data.tree.folder.0","mayHaveChildren":false,"childrenLoaded":true}],"tableRows":[],"gridEntries":[]}})")
        )),
    ...
    check(
        application.async().post_succeed(requests.front().id, lazy_result),
    ...
    check(
        surface.tree().find_key("data.tree.asset.0.0") != nullptr &&
            lazy_state.field("status") != nullptr &&
            *lazy_state.field("status")->string() == "READY" &&
            lazy_value != nullptr && lazy_value->list() != nullptr &&
            lazy_value->list()->values.front().field("key")->key() != nullptr &&
            lazy_value->list()->values.front().field("parentKey")->key() != nullptr,
        "typed async completion or immutable host republish did not materialize lazy-tree children"
    );
    ...
    const runtime::ActionDispatchOutcome query = surface.dispatch_action(
        ...
        "query",
        std::optional<std::string>("async.root"),
        runtime::Value{}
    );
    check(
        query.status == runtime::ActionDispatchStatus::handled,
        "async result query action was not handled"
    );
    static_cast<void>(surface.frame(1'066'666'668));
    const auto result_request = std::ranges::find_if(requests, [](const runtime::AsyncRequest& item) {
        return item.binding == "asyncResults";
    });
    check(result_request != requests.end(), "async result query did not reach the host adapter");
    check(
        application.async().post_succeed(
            result_request->id,
    ...
    check(
        result_state.field("status") != nullptr &&
    ...
    check(
        surface.tree().find_key("query.authored.async.0") != nullptr &&
    ...
    const auto start_query = [&surface, &requests, &query_frame_time](std::string payload) {
        ...
        check(
            outcome.status == runtime::ActionDispatchStatus::handled,
    ...
    check(
        application.async().post_succeed(empty_request, runtime::Value(std::vector<runtime::Value>{})),
    ...
    check(
        empty_state.field("status") != nullptr &&
    ...
    check(
        application.async().post_fail(failed_request, runtime::AsyncFailure{
            "The simulated source rejected the query.", "SIMULATED_FAILURE",
        }),
    ...
    check(
        failed_state.field("status") != nullptr &&
    ...
    static_cast<void>(surface.input().key(
        "A", ui::KeyModifiers{false, true, false, false}
    ));
    static_cast<void>(surface.input().text("empty"));
    static_cast<void>(surface.frame(query_frame_time += 16'666'667));
    static_cast<void>(surface.input().click("query.start"));
    check(
        !requests.empty() && requests.back().payload.string() != nullptr &&
...
overlay Collections {
  Panel(key: "collections.root", layout: { kind: "COLUMN", width: { weight: 1 }, height: { weight: 1 }, gap: 8, padding: 8 }) {
    ...
    const runtime::ActivationResult activation = application.compile_and_activate(
        compiler::ModuleSource{"collections.strata", source},
        no_imports,
        0U
    );
    if (!activation.activated()) {
    ...
    {
        ui::DescriptionBuilder temporary_builder(application);
        ui::DescriptionBuildResult detached = temporary_builder.build(
            runtime::LayerRole::overlay,
            "Collections"
        );
    ...
    check(
        derived_rows != detached_table_description->properties.end() &&
    ...
    const runtime::Value* tree_virtual_count = tree_layout != nullptr
        ? tree_layout->field("virtualItemCount")
        : nullptr;
    check(
        tree_items != detached_tree_description->properties.end() &&
    ...
    check(
        detached_grid_description->children->size() == 5U &&
            ...
            detached_grid_description->properties.at("$layout").value()
                    ->field("virtualItemMembers") == nullptr &&
            detached_grid_description->properties.at("$layout").value()
                    ->field("virtualItemExtents") == nullptr,
    ...
    check(
        first_tree_row == repeated_tree_row &&
            ...
            *first_tree_row->properties.at("$treeExpandable").value()->boolean() &&
            ...
            first_table_row->materialization_result !=
                second_table_row->materialization_result,
    ...
    check(
        initial_tree != nullptr && initial_root != nullptr && initial_folder != nullptr &&
    ...
    check(
        surface.layout().find(initial_root->identity()) != nullptr &&
    ...
    check(
        surface.tree().find_key("row.alpha.cell.score") != nullptr &&
    ...
    const std::uint64_t initial_table_identity =
        surface.tree().find_key("row.alpha")->identity();
    const std::uint64_t initial_grid_identity =
        surface.tree().find_key("grid.alpha")->identity();
    const ui::SurfaceFrame warm_frame = surface.frame(1'016'666'667);
    check(
        surface.tree().find_key("tree.root")->identity() == initial_root_identity &&
    ...
    const auto pointers = [&surface, &frame_time](
                              const std::initializer_list<ui::PointerInputEvent> events
                          ) {
    ...
    const ui::LayoutRecord* root_layout = root != nullptr
        ? surface.layout().find(root->identity()) : nullptr;
    ...
    check(
        std::ranges::any_of(expanded.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
    ...
    const ui::LayoutRecord* expanded_child_layout = surface.layout().find(
        surface.tree().find_key("tree.child")->identity()
    );
    const ui::LayoutRecord* expanded_folder_layout = surface.layout().find(
        surface.tree().find_key("tree.folder")->identity()
    );
    check(
        expanded_root_layout != nullptr && expanded_child_layout != nullptr &&
    ...
    const runtime::Value* tree_active = surface.tree().find_key("collections.tree")
        ->retained_value("strata.collection.active");
    check(
        tree_active != nullptr && tree_active->key() != nullptr &&
            tree_active->key()->value == "tree.child",
        "tree typeahead did not select its wrapped match; active=" +
            (tree_active != nullptr && tree_active->key() != nullptr
                ? tree_active->key()->value
                : std::string("<none>"))
    );
    ...
    const ui::LayoutRecord* folder_layout = tree_folder != nullptr
        ? surface.layout().find(tree_folder->identity()) : nullptr;
    ...
    check(
        surface.tree().find_key("tree.folder.child") != nullptr &&
            std::ranges::any_of(hover_expanded.lifecycle_input.events, [](const data::JsonValue& event) {
                const data::JsonValue* type = event.find("type");
                return type != nullptr && type->string() != nullptr &&
                    *type->string() == "tree-expansion-changed";
            }),
    ...
    const ui::LayoutRecord* row_alpha_layout = row_alpha != nullptr
        ? surface.layout().find(row_alpha->identity()) : nullptr;
    ...
    const runtime::Value* table_selected = surface.tree().find_key("collections.table")
        ->retained_value("strata.collection.selected");
    check(
        table_selected != nullptr && table_selected->list() != nullptr &&
    ...
    const ui::LayoutRecord* table_layout = table != nullptr
        ? surface.layout().find(table->identity()) : nullptr;
    ...
    const ui::Point divider{table_viewport.x + 140.0,
    ...
    const ui::InputOperationResult resized = pointers({
        ...
        ui::PointerInputEvent{
            ui::Point{divider.x + 24.0, divider.y}, ui::PointerEventType::move, 7, 0,
        },
        ui::PointerInputEvent{
            ui::Point{divider.x + 24.0, divider.y}, ui::PointerEventType::release, 7, 0,
        },
    });
    check(
        std::ranges::any_of(resized.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
                *type->string() == "table-column-width-changed";
        }) &&
            surface.tree().find_key("collections.table")
    ...
    const runtime::Value* table_order = surface.tree().find_key("collections.table")
        ->retained_value("strata.table.columnOrder");
    check(
        std::ranges::any_of(reordered.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
    ...
    const data::JsonValue* table_semantics = table != nullptr
        ? surface.semantics().find(table->identity()) : nullptr;
    const data::JsonValue* semantic_children_value = table_semantics != nullptr
        ? table_semantics->find("children") : nullptr;
    const data::JsonValue::Array* semantic_children = semantic_children_value != nullptr
        ? semantic_children_value->array() : nullptr;
    check(
        semantic_children != nullptr && semantic_children->size() >= 3U &&
    ...
    check(
        surface.tree().find_key("collections.table")
            ...
            surface.tree().find_key("collections.table")
                ->retained_value("strata.table.headerSession")->object() != nullptr,
        "table resize did not retain a cancellable header gesture"
    );
    static_cast<void>(surface.input().key("Escape"));
    static_cast<void>(surface.frame(frame_time += 16'666'667));
    const runtime::Value* cancelled_widths = surface.tree().find_key("collections.table")
        ->retained_value("strata.table.columnWidths");
    const auto cancelled_name = cancelled_widths != nullptr && cancelled_widths->list() != nullptr
        ? std::ranges::find_if(cancelled_widths->list()->values, [](const runtime::Value& value) {
              const runtime::Value* id = value.field("id");
              return id != nullptr && id->string() != nullptr && *id->string() == "name";
          })
    ...
    const ui::LayoutRecord* alpha_layout = grid_alpha != nullptr
        ? surface.layout().find(grid_alpha->identity()) : nullptr;
    const ui::LayoutRecord* beta_layout = grid_beta != nullptr
        ? surface.layout().find(grid_beta->identity()) : nullptr;
    ...
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        gap, ui::PointerEventType::press, 7, 0,
    }));
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        beta_center, ui::PointerEventType::move, 7, 0,
    }));
        ...
        for (const ui::RenderCommand& command : surface.render_commands().commands()) {
            ...
            if (rectangle == nullptr ||
                ...
                rectangle->border->color !=
                    ui::RenderColor{104U, 169U, 255U, 220U}) {
                continue;
            }
    ...
    const runtime::Value* marquee = surface.tree().find_key("collections.grid")
        ->retained_value("strata.collection.marquee");
    const runtime::Value* preview_selection = marquee != nullptr
        ? marquee->field("lastSelection") : nullptr;
    const runtime::Value* grid_selected = surface.tree().find_key("collections.grid")
        ->retained_value("strata.collection.selected");
    check(
        marquee_preview.operations.rebuilds == 0U &&
            (grid_selected == nullptr || grid_selected->list() == nullptr ||
             std::ranges::none_of(grid_selected->list()->values, [](const runtime::Value& value) {
                return value.key() != nullptr && value.key()->value == "grid.beta";
            })) && preview_selection != nullptr && preview_selection->list() != nullptr &&
            std::ranges::any_of(
                preview_selection->list()->values,
    ...
    check(
        marquee_continued.operations.rebuilds == 0U &&
            ...
            first_marquee_bounds.has_value() &&
            continued_marquee_bounds.has_value() &&
    ...
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        beta_center, ui::PointerEventType::release, 7, 0,
    }));
    const ui::SurfaceFrame marquee_commit = surface.frame(frame_time += 16'666'667);
    grid_selected = surface.tree().find_key("collections.grid")
        ->retained_value("strata.collection.selected");
    check(
        marquee_commit.operations.rebuilds == 1U && grid_selected != nullptr &&
            grid_selected->list() != nullptr &&
            std::ranges::any_of(grid_selected->list()->values, [](const runtime::Value& value) {
                return value.key() != nullptr && value.key()->value == "grid.beta";
            }),
    ...
    const ui::LayoutRecord* grid_layout = grid != nullptr
        ? surface.layout().find(grid->identity()) : nullptr;
    check(
        grid_layout != nullptr && grid_layout->viewport.has_value(),
    ...
    check(
        grid_layout != nullptr && grid_layout->scroll_offset.y > pre_wheel_scroll,
    ...
    grid_layout = grid != nullptr ? surface.layout().find(grid->identity()) : nullptr;
    check(
        grid_layout != nullptr && grid_layout->scroll_offset.y > initial_grid_scroll &&
    ...
    grid_selected = surface.tree().find_key("collections.grid")
        ->retained_value("strata.collection.selected");
    marquee = surface.tree().find_key("collections.grid")
        ->retained_value("strata.collection.marquee");
    check(
        grid_selected != nullptr && grid_selected->list() != nullptr &&
...
overlay LazyMotion {
  root Repeater(
    ...
    check(
        application.compile_and_activate(
            compiler::ModuleSource{"motion.strata", source}, no_imports, 0U
        ).activated(),
    ...
    check(
        counted != nullptr && focus_visible_trigger != nullptr &&
    ...
    check(
        counted->timing.fill_mode == ui::MotionFillMode::backwards &&
    ...
    check(
        delayed.running && delayed.computed.number(ui::MotionProperty::opacity).value_or(-1.0) == 0.0,
        "backwards fill did not present the initial keyframe during delay"
    );
    const auto alternating = player.advance(
        *counted, true, ui::MotionDirection::forward, 145'000'000, false
    );
    check_near(alternating.raw_progress, 0.75, "repeat reversal did not alternate iteration direction");
    check_near(
        alternating.computed.number(ui::MotionProperty::opacity).value_or(-1.0),
        0.75,
        "repeat reversal sampled the wrong authored value"
    );
    const auto unfilled_terminal = player.advance(
        *counted, true, ui::MotionDirection::forward, 320'000'000, false
    );
    check(
        unfilled_terminal.finished && unfilled_terminal.computed.values.empty(),
    ...
    static_cast<void>(retained_player.advance(
        retained_terminal, true, ui::MotionDirection::forward, 0, false
    ));
    const auto terminal = retained_player.advance(
        retained_terminal, true, ui::MotionDirection::forward, 320'000'000, false
    );
    check_near(
        terminal.computed.number(ui::MotionProperty::opacity).value_or(-1.0),
        1.0,
    ...
    static_cast<void>(even_player.advance(
        even_alternating, true, ui::MotionDirection::forward, 0, false
    ));
    const auto even_terminal = even_player.advance(
        even_alternating, true, ui::MotionDirection::forward, 220'000'000, false
    );
    check(
        even_terminal.finished && even_player.progress() ==
    ...
    static_cast<void>(reverse_even_player.advance(
        even_alternating, true, ui::MotionDirection::reverse, 0, false
    ));
    static_cast<void>(reverse_even_player.advance(
        even_alternating, true, ui::MotionDirection::reverse, 220'000'000, false
    ));
    check(
        reverse_even_player.progress() ==
    ...
    ui::Surface lazy_motion(
        "lazy-motion", application, runtime::LayerRole::overlay, "LazyMotion", environment
    );
    ...
    const auto enter_channel = [&lazy_motion](
                                   const ui::RetainedNode* row
                               ) -> const ui::MotionInspectionChannel* {
        const auto* channels = row != nullptr
            ? lazy_motion.motion().inspection_channels(row->identity())
            : nullptr;
        if (channels == nullptr) return nullptr;
        const auto found = std::ranges::find(
            *channels,
            std::string_view("strata.trigger.enter"),
    ...
    check(
        late_enter != nullptr && late_enter->running && late_enter->progress == 0.0 &&
    ...
    check(
        same_time_enter != nullptr && same_time_enter->progress == 0.0 &&
    ...
    check(
        advanced_enter != nullptr && advanced_enter->progress > 0.0 &&
            advanced_enter->progress < 1.0,
        "discovered lazy enter motion did not schedule a later temporal frame"
    );

    ui::Surface surface(
        "motion-progress", application, runtime::LayerRole::overlay, "Motion", environment
    );
    static_cast<void>(surface.frame(0));
    ...
    const auto progress_sample = [&surface, progress_node]() -> const ui::MotionInspectionChannel* {
        ...
        const auto found = std::ranges::find(
            *channels,
            std::string_view("strata.progress.indeterminate"),
            &ui::MotionInspectionChannel::id
        );
        return found != channels->end() ? &*found : nullptr;
    };
    ...
    check(
        progress_sample() != nullptr && progress_sample()->running,
        "indeterminate Progress did not retain a running motion timeline"
    );
    check_near(progress_sample()->progress, 0.25, "indeterminate Progress did not advance per frame");
    ...
    check_near(progress_sample()->progress, 0.25, "reduced motion changed the presented progress frame");
    ...
    ui::Surface interaction_motion(
        "interaction-motion", application, runtime::LayerRole::overlay,
    ...
    const ui::LayoutRecord* hover_layout = hover_node != nullptr
        ? interaction_motion.layout().find(hover_node->identity())
        : nullptr;
    ...
    static_cast<void>(interaction_motion.input().enqueue_pointer(ui::PointerInputEvent{
        hover_center, ui::PointerEventType::move, 0, 0,
    }));
    const ui::SurfaceFrame hover_started = interaction_motion.frame(1'310'000'000);
    const auto hover_sample = [&interaction_motion, hover_node]()
        -> const ui::MotionInspectionChannel* {
        const auto* channels = interaction_motion.motion().inspection_channels(
            hover_node->identity()
        );
        if (channels == nullptr) return nullptr;
        const auto found = std::ranges::find(
            *channels,
            std::string_view("motion.hover.channel"),
            &ui::MotionInspectionChannel::id
        );
        return found != channels->end() ? &*found : nullptr;
    };
    check(
        hover_sample() != nullptr && hover_sample()->running && hover_sample()->progress == 0.0 &&
    ...
    check_near(
        hover_sample()->progress,
        0.5,
        "post-input motion discovery advanced the interaction clock more than once"
    );

    const auto focus_visible_sample = [&interaction_motion, hover_node]()
        -> const ui::MotionInspectionChannel* {
        const auto* channels = interaction_motion.motion().inspection_channels(
            hover_node->identity()
        );
        ...
        const auto found = std::ranges::find(
            *channels,
            std::string_view("motion.focus-visible.channel"),
            &ui::MotionInspectionChannel::id
        );
        return found != channels->end() ? &*found : nullptr;
    };
    ...
    check(
        interaction_motion.input().focused(hover_node->identity()) &&
    ...
    ui::Surface reorder_motion(
        "reorder-motion", application, runtime::LayerRole::overlay,
        "ReorderMotion", environment
    );
    static_cast<void>(reorder_motion.frame(1'400'000'000));
    const ui::InputOperationResult reorder = reorder_motion.input().drag(
        "reorder.alpha", "reorder.container"
    );
    check(
        std::ranges::any_of(reorder.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            ...
            return type != nullptr && type->string() != nullptr && *type->string() == "drag" &&
                phase != nullptr && phase->string() != nullptr && *phase->string() == "drop" &&
                recipient != nullptr && recipient->string() != nullptr &&
                *recipient->string() == "target" && insertion != nullptr &&
                insertion->integer() != nullptr && *insertion->integer() == 2 &&
                before != nullptr && before->is_null() && after != nullptr &&
                after->string() != nullptr && *after->string() == "reorder.beta";
        }) && std::ranges::none_of(reorder.events, [](const data::JsonValue& event) {
            const data::JsonValue* phase = event.find("phase");
    ...
    check(
        reorder_repaint.operations.rebuilds == 0U,
        "retained reorder preview invalidated the declarative description"
    );

    ui::Surface verbose_reorder(
        "reorder-motion-verbose", application, runtime::LayerRole::overlay,
        "ReorderMotionVerbose", environment
    );
    static_cast<void>(verbose_reorder.frame(1'400'000'000));
    const ui::InputOperationResult verbose = verbose_reorder.input().drag(
        "reorder.verbose.alpha", "reorder.verbose.container"
    );
    check(
        std::ranges::any_of(verbose.events, [](const data::JsonValue& event) {
            const data::JsonValue* phase = event.find("phase");
            return phase != nullptr && phase->string() != nullptr &&
                *phase->string() == "move";
        }),
...
overlay Projection {
  root Frame() {
    ...
    const runtime::ActivationResult activation = application.compile_and_activate(
        compiler::ModuleSource{"projection.strata", source},
        no_imports,
        0U
    );
    ...
    ui::DescriptionBuilder builder(application);
    const ui::DescriptionBuildResult description = builder.build(
        runtime::LayerRole::overlay,
        "Projection"
    );
    ...
    check(
        tree.find_key("projection.named") != nullptr &&
    ...
    const auto variant = tree.find_key("projection.named")->description().properties.find("variant");
    check(
        variant != tree.find_key("projection.named")->description().properties.end() &&
            variant->second.value() != nullptr &&
            variant->second.value()->string() != nullptr &&
...
overlay Other { root Text(key: "cache.other", text: "other") }
    ...
    check(
        application.compile_and_activate(
            compiler::ModuleSource{"component-retained-cache.strata", source},
            no_imports,
            0U
        ).activated(),
    ...
    const auto find_description = [](
        ...
        if (node->key.has_value() && *node->key == key) return node;
    ...
    check(
        retained_split != nullptr && tree.set_retained_value(
            retained_split->identity(),
            "strata.gesture.splitRatio",
            runtime::Value(0.75)
        ),
    ...
    check(
        stable_before != nullptr && stable_after == stable_before,
        "an unrelated retained value invalidated a component cache entry"
    );
    check(
        split_before != nullptr && split_after != nullptr && split_after != split_before,
        "a component reused output after one of its observed retained values changed"
    );
    const runtime::Value* left_layout = split_after->children->at(0U)->properties.at("$layout").value();
    const runtime::Value* left_width = left_layout != nullptr ? left_layout->field("width") : nullptr;
    check(
        left_width != nullptr && left_width->field("weight") != nullptr &&
    ...
    check(
        alpha_before != nullptr && alpha_after == alpha_before,
        "an unrelated host snapshot generation invalidated a component cache entry"
    );
    check(
        beta_before != nullptr && beta_after != nullptr && beta_after != beta_before &&
    ...
    check(
        find_description(find_description, returned.root, "cache.host.alpha") == alpha_after &&
...
overlay PhasedInput {
  root Panel(
    ...
    check(
        application.compile_and_activate(
            compiler::ModuleSource{"phased-input.strata", source}, no_imports, 0U
        ).activated(),
    ...
    const auto phase_name = [](const ui::InputEventPhase phase) -> std::string_view {
        switch (phase) {
        case ui::InputEventPhase::capture: return "capture";
        case ui::InputEventPhase::target: return "target";
        case ui::InputEventPhase::bubble: return "bubble";
        case ui::InputEventPhase::advance: return "advance";
        case ui::InputEventPhase::after_layout: return "after-layout";
        }
        return "target";
    };
    ...
    focusable.event = [&](ui::BehaviorInputScope& scope) {
        ...
        if (dispatch != nullptr && dispatch->kind() == ui::InputEventKind::pointer_press) {
            pointer_press_route.push_back(
                "behavior:" + std::string(phase_name(dispatch->phase()))
            );
        } else if (dispatch != nullptr &&
                   dispatch->kind() == ui::InputEventKind::pointer_cancel) {
            pointer_cancel_route.push_back(
                "behavior:" + std::string(phase_name(dispatch->phase()))
            );
        }
        return false;
    ...
    const ui::LayoutRecord* child_layout = child != nullptr
        ? surface.layout().find(child->identity())
        : nullptr;
    ...
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        origin, ui::PointerEventType::press, 17, 0, {}, {}, 1'010'000'000,
    }));
    static_cast<void>(surface.frame(1'016'666'667));
    check(
        pointer_press_route == std::vector<std::string>{
            "behavior:capture", "panel:capture", "button:target",
            "behavior:bubble", "panel:bubble",
        },
    ...
    static_cast<void>(surface.input().enqueue(std::vector<ui::SurfaceInputEvent>{
        ui::PointerInputEvent{
            dragged, ui::PointerEventType::move, 17, 0, {}, {12.0, 0.0}, 1'020'000'000,
        },
        ui::PointerInputEvent{
            origin, ui::PointerEventType::move, 17, 0, {}, {-12.0, 0.0}, 1'030'000'000,
        },
        ui::PointerInputEvent{
            origin, ui::PointerEventType::release, 17, 0, {}, {}, 1'040'000'000,
        },
    }));
    const ui::SurfaceFrame release = surface.frame(1'050'000'000);
    check(
        release.operations.input_coalesced_moves == 1U,
    ...
    check(
        std::ranges::none_of(release.lifecycle_input.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
    ...
    static_cast<void>(surface.input().enqueue(std::vector<ui::SurfaceInputEvent>{
        ui::KeyInputEvent{
            "EnTeR", ui::KeyModifiers{true, false, true, false},
            ui::KeyEventType::release, 1'051'000'000,
        },
    }));
    const ui::SurfaceFrame key_release = surface.frame(1'052'000'000);
    check(
        routed_keys.size() == 1U && routed_keys.back().key == "EnTeR" &&
    ...
    check(
        std::ranges::none_of(key_release.lifecycle_input.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
                *type->string() == "activated";
        }),
    ...
    static_cast<void>(surface.input().enqueue(std::vector<ui::SurfaceInputEvent>{
        ui::KeyInputEvent{
            "EnTeR", {}, ui::KeyEventType::repeat, 1'053'000'000,
        },
    }));
    const ui::SurfaceFrame key_repeat = surface.frame(1'054'000'000);
    check(
        routed_keys.size() == 2U && routed_keys.back().type == ui::KeyEventType::repeat &&
            routed_keys.back().timestamp_nanos == 1'053'000'000,
        "queued key repeat was reconstructed as a press"
    );
    check(
        std::ranges::any_of(key_repeat.lifecycle_input.events, [](const data::JsonValue& event) {
            const data::JsonValue* type = event.find("type");
            return type != nullptr && type->string() != nullptr &&
                *type->string() == "activated";
        }),
    ...
    static_cast<void>(surface.input().enqueue(std::vector<ui::SurfaceInputEvent>{
        ui::NavigationInputEvent{
            "activate", ui::KeyEventType::release, {}, 1'055'000'000,
        },
    ...
    check(
        routed_keys.size() == 3U && routed_keys.back().key == "enter" &&
            routed_keys.back().type == ui::KeyEventType::release &&
    ...
    check(
        std::ranges::none_of(
            navigation_release.lifecycle_input.events,
    ...
    check(
        routed_text.size() == 1U && routed_text.front().text == "raw" &&
            routed_text.front().timestamp_nanos == 0 && routed_ime.size() == 1U &&
            routed_ime.front().text == "preedit" &&
            routed_ime.front().selection_start == 1U &&
            routed_ime.front().selection_end == 3U &&
            routed_ime.front().timestamp_nanos == 0,
        "queued text or IME generic dispatch reconstructed the raw envelope"
    );

    pointer_cancel_route.clear();
    static_cast<void>(surface.input().enqueue_pointer(ui::PointerInputEvent{
        origin, ui::PointerEventType::press, 23, 0, {}, {}, 1'060'000'000,
    }));
    ...
    check(
        pointer_cancel_route == std::vector<std::string>{
            "behavior:capture", "panel:capture", "button:target",
            "behavior:bubble", "panel:bubble",
        },
