...
[[nodiscard]] std::shared_ptr<const ui::DescriptionChildren> children(
    std::vector<std::shared_ptr<const ui::DescriptionNode>> values
) {
    return std::make_shared<const ui::EagerDescriptionChildren>(std::move(values));
}

[[nodiscard]] std::shared_ptr<const ui::DescriptionNode> node(
    std::string type,
    ...
    return ui::DescriptionNode::create(
        std::move(type),
        std::move(key),
        "/interaction-residual",
        "interaction residual fixture",
        std::move(properties),
...
[[nodiscard]] runtime::ExpressionValue layout(
    const double width,
    const double height,
...
[[nodiscard]] ui::DescriptionNode::Properties sized(
    const double width,
    const double height,
    std::string kind = "PANEL"
) {
...
[[nodiscard]] std::vector<std::string> semantic_actions(const data::JsonValue& semantics) {
    ...
    if (actions == nullptr || actions->array() == nullptr) return result;
    for (const data::JsonValue& action : *actions->array()) {
        if (action.string() != nullptr) result.push_back(*action.string());
    }
    return result;
}
...
public:
    ...
    InputFixture(
        ...
        application_("interaction-residual", bundle_),
        text_(ui::TextEngine::load_default_fonts(resources)),
        commands_(widgets_),
        input_(
            "interaction-residual",
            "interaction-residual/host-owner",
            application_,
            widgets_,
            behaviors_,
            status_,
            ...
            [this](
                ...
                const std::size_t utf16 = ui::text_layout_hit_offset(
                    shaped,
                    ui::Point{position.x - origin.x, position.y - origin.y}
                );
    ...
    void adopt(
        std::shared_ptr<const ui::DescriptionNode> description,
    ...
    [[nodiscard]] FrameResult frame(
        ...
        tree_.clear_dirty();
        return FrameResult{
            std::move(reconciliation), std::move(input), operations,
        };
    }

    [[nodiscard]] ui::InputOperationResult pointer(
        std::vector<ui::PointerInputEvent> events
    ) {
        std::vector<ui::SurfaceInputEvent> queued;
        queued.reserve(events.size());
        for (ui::PointerInputEvent& event : events) queued.emplace_back(std::move(event));
    ...
    [[nodiscard]] const ui::LayoutRecord& bounds(const std::string_view key) const {
        ...
        const ui::LayoutRecord* record = retained != nullptr
            ? layout_.result().find(retained->identity())
            : nullptr;
        if (record == nullptr) throw std::runtime_error("missing fixture layout record");
        return *record;
    }

    [[nodiscard]] ui::TextLayout text_layout(const std::string_view key) const {
        const ui::RetainedNode* retained = tree_.find_key(key);
        if (retained == nullptr) throw std::runtime_error("missing text-layout fixture node");
    ...
    [[nodiscard]] std::shared_ptr<const runtime::ActionValue> notification_action(
        ...
        if (contract == nullptr) throw std::runtime_error("notification action is unavailable");
        return std::make_shared<const runtime::ActionValue>(runtime::ActionValue{
            std::make_shared<const runtime::Action>(
                contract,
                object({{"message", runtime::Value(std::move(message))}})
            ),
            std::nullopt,
            {},
        });
...
[[nodiscard]] runtime::ExpressionValue rich_span(
    std::string text,
    ...
    return runtime::ExpressionValue(
        std::make_shared<const runtime::ExpressionObjectValue>(runtime::ExpressionObjectValue{
            std::move(materialized), std::move(fields),
        })
    );
}
...
void test_primary_pointer_focus_default(InputFixture& fixture) {
    fixture.adopt(node(
        "Panel", "focus.root", {
            ...
            node(
                "Panel", "focus.interactive-background", {}, sized(120.0, 40.0),
    ...
    check(
        control != nullptr && !fixture.input_.focus_visible(control->identity()),
    ...
    static_cast<void>(fixture.pointer({
        ui::PointerInputEvent{
            empty_background, ui::PointerEventType::press, pointer_id, 1,
        },
        ui::PointerInputEvent{
            empty_background, ui::PointerEventType::release, pointer_id++, 1,
        },
    }));
    check(
        fixture.input_.focused_key().has_value() &&
    ...
    const ui::InputOperationResult empty_press = fixture.pointer({
        ui::PointerInputEvent{
            empty_background, ui::PointerEventType::press, pointer_id, 0,
        },
        ui::PointerInputEvent{
            empty_background, ui::PointerEventType::release, pointer_id++, 0,
        },
    });
    check(
        !fixture.input_.focused_key().has_value() && !empty_press.events.empty(),
        "primary empty-background press did not blur the focused control"
    );
    check(
        fixture.invalidations_ == 0U,
        "background blur escaped the retained input-dirty path"
    );

    focus_control();
    const ui::Point interactive_background = center(
        fixture.bounds("focus.interactive-background").bounds
    );
    static_cast<void>(fixture.pointer({
        ui::PointerInputEvent{
            interactive_background, ui::PointerEventType::press, pointer_id, 0,
        },
        ui::PointerInputEvent{
            interactive_background, ui::PointerEventType::release, pointer_id++, 0,
        },
    }));
    check(
        !fixture.input_.focused_key().has_value(),
    ...
    check(
        fixture.input_.set_focus_containment("focus.root", containment),
...
void test_rich_text_press_arm(InputFixture& fixture) {
    ...
    rich.emplace(
        ...
        runtime::ExpressionValue(
            std::make_shared<const runtime::ExpressionListValue>(runtime::ExpressionListValue{
                materialized, std::move(expressed),
            })
        )
    );
    ...
    check(
        !fixture.input_.editor_snapshot(retained->identity()).has_value(),
        "selectable RichText leaked into the writable editor inspection projection"
    );
    const std::vector<ui::WidgetSubtarget> links = fixture.input_.subtargets(
        retained->identity()
    );
    const auto first_target = std::ranges::find(links, std::string("$link/0"), &ui::WidgetSubtarget::id);
    const auto second_target = std::ranges::find(links, std::string("$link/1"), &ui::WidgetSubtarget::id);
    check(
        first_target != links.end() && second_target != links.end() &&
    ...
    check(
        selection.has_value() && selection->selection_start != selection->selection_end,
    ...
    check(
        fixture.notifications_.size() == 1U &&
...
void test_static_text_state_partition(InputFixture& fixture) {
    ...
    opted_out.emplace(
        "selectionContainer", runtime::ExpressionValue(runtime::Value("document"))
    );
    ...
    for (const std::string_view key : {
        ...
        check(
            !fixture.input_.static_text_selection_snapshot(retained->identity()).has_value(),
    ...
    const auto select_ordinary = [&fixture, ordinary_point](const std::int32_t pointer_id) {
        static_cast<void>(fixture.pointer({
            ui::PointerInputEvent{
                ordinary_point, ui::PointerEventType::press, pointer_id, 0,
            },
            ui::PointerInputEvent{
                ordinary_point, ui::PointerEventType::release, pointer_id, 0,
            },
        }));
    };
    select_ordinary(11);
    ...
    check(
        ordinary_node != nullptr && fixture.input_.focused_key().has_value() &&
            *fixture.input_.focused_key() == "static.ordinary" &&
            fixture.input_.static_text_selection_snapshot(
                ordinary_node->identity()
            ).has_value(),
    ...
    check(
        ordinary_selection.has_value() && ordinary_selection->text == "ordinary title" &&
    ...
    check(
        opted_out_node != nullptr &&
            !fixture.input_.focused_key().has_value() &&
            !fixture.input_.static_text_selection_snapshot(
                opted_out_node->identity()
            ).has_value() &&
            !fixture.input_.static_text_selection_snapshot(
                ordinary_node->identity()
            ).has_value(),
    ...
    check(
        !fixture.input_.static_text_selection_snapshot(linked_node->identity()).has_value() &&
            !fixture.input_.static_text_selection_snapshot(
                ordinary_node->identity()
            ).has_value(),
...
void test_wrapped_static_text_navigation(InputFixture& fixture) {
    ...
    const auto point_on_line = [&fixture, key](
        const ui::TextLayout& layout,
        ...
        return ui::Point{
            ...
            record.content_bounds.y + layout.lines[line].y +
                layout.lines[line].height * 0.5,
        };
    };
    ...
    check(
        ui::utf16_offset_for_utf8_byte(content, selection().caret) ==
    ...
    const std::size_t initial_utf16 = ui::text_layout_line_offset_at_x(
        layout, vertical_line, requested_x
    );
    const double x_goal = ui::shaped_caret_x(
        layout.shaped, vertical_line, initial_utf16
    );
    ...
    adopt();
    layout = fixture.text_layout(key);
    const std::size_t shift_anchor_utf16 = ui::text_layout_line_offset_at_x(
        layout, vertical_line, requested_x
    );
    const double shift_x_goal = ui::shaped_caret_x(
        layout.shaped, vertical_line, shift_anchor_utf16
    );
    click(point_on_line(layout, vertical_line, requested_x), 33);
    static_cast<void>(fixture.input_.key(
        "down", ui::KeyModifiers{true, false, false, false}
    ));
    const ui::StaticTextSelectionSnapshot shifted = selection();
    check(
        ui::utf16_offset_for_utf8_byte(content, shifted.selection_start) ==
            shift_anchor_utf16 &&
            ui::utf16_offset_for_utf8_byte(content, shifted.selection_end) ==
                ui::text_layout_line_offset_at_x(
                    layout, vertical_line + 1U, shift_x_goal
                ),
...
void test_wrapped_editor_pointer_navigation(InputFixture& fixture) {
    ...
    const auto adopt = [&fixture, content, key] {
        ...
        }, sized(100.0, 200.0, "COLUMN")), 1.5);
    };
    const auto first_cluster_point = [&fixture, key](
        const ui::TextLayout& layout,
        const std::size_t line
    ) {
        const auto cluster = std::ranges::find_if(
            layout.shaped.clusters,
            [line](const font::ShapedCluster& candidate) {
                return candidate.line_index == line && !candidate.soft_wrap_gap;
            }
        );
        check(cluster != layout.shaped.clusters.end(), "wrapped editor line lost its first cluster");
        const ui::LayoutRecord& record = fixture.bounds(key);
        return ui::Point{
    ...
    click(point, 51);
    check(
        ui::utf16_offset_for_utf8_byte(content, snapshot().caret) ==
    ...
    check(
        ui::utf16_offset_for_utf8_byte(content, dragged.selection_start) ==
            layout.lines[clicked_line - 1U].text_start_offset &&
...
void test_static_text_container_owner_transition(InputFixture& fixture) {
    ...
    const ui::Point first_point{
        first_bounds.x + 4.0, first_bounds.y + first_bounds.height * 0.5,
    };
    const ui::Point second_point{
        second_bounds.x + 28.0, second_bounds.y + second_bounds.height * 0.5,
    };
    static_cast<void>(fixture.pointer({
    ...
    check(
        first_range.has_value() && second_range.has_value() &&
...
void test_choice_semantics(InputFixture& fixture) {
    ...
    authored_default.emplace("defaultSelectedId", runtime::ExpressionValue(runtime::Value("three")));
    ...
    disabled_select.emplace(
        "defaultSelectedId", runtime::ExpressionValue(runtime::Value("removed"))
    );
    ...
    stale_retained.emplace(
        "defaultSelectedId", runtime::ExpressionValue(runtime::Value("three"))
    );
    ...
    invalid_controlled.emplace(
        "selectedId", runtime::ExpressionValue(runtime::Value("removed"))
    );
    invalid_controlled.emplace(
        "defaultSelectedId", runtime::ExpressionValue(runtime::Value("three"))
    );
    fixture.adopt(node("Panel", "choice.root", {
    ...
    check(
        retained_tabs != nullptr && radio != nullptr && stale != nullptr && invalid != nullptr,
        "choice fixtures were not retained"
    );
    static_cast<void>(fixture.tree_.set_retained_value(
        retained_tabs->identity(), "$selectedId", runtime::Value("two"), ui::DirtyReason::properties
    ));
    static_cast<void>(fixture.tree_.set_retained_value(
    ...
    const auto require_choice = [&fixture](
                                    const std::string_view key,
                                    const std::string_view id,
                                    const std::size_t index
                                ) {
        const ui::RetainedNode* retained = fixture.tree_.find_key(key);
        const std::optional<ui::EffectiveChoice> selection = retained != nullptr
            ? ui::effective_choice(*retained)
            : std::nullopt;
        check(
            selection.has_value() && selection->id == id && selection->index == index,
    ...
    check(
        ui::choice_is_controlled(*invalid),
    ...
    static_cast<void>(semantics.update(
        fixture.tree_, fixture.commands_, fixture.input_, "interaction-residual"
    ));
    const data::JsonValue* controlled_semantics = semantics.find(
        fixture.tree_.find_key("choice.controlled")->identity()
    );
    const data::JsonValue* retained_semantics = semantics.find(retained_tabs->identity());
    const data::JsonValue* default_semantics = semantics.find(
        fixture.tree_.find_key("choice.default")->identity()
    );
    const data::JsonValue* radio_semantics = semantics.find(radio->identity());
    const data::JsonValue* absent_semantics = semantics.find(
        fixture.tree_.find_key("choice.absent-default")->identity()
    );
    const data::JsonValue* disabled_select_semantics = semantics.find(
        fixture.tree_.find_key("choice.disabled-select")->identity()
    );
    const data::JsonValue* disabled_radio_semantics = semantics.find(
        fixture.tree_.find_key("choice.disabled-radio")->identity()
    );
    ...
    check(
        controlled_semantics != nullptr && semantic_value_text(*controlled_semantics) == "one" &&
    ...
    const std::vector<ui::WidgetSubtarget> controlled_targets = fixture.input_.subtargets(
        controlled_tabs->identity()
    );
    const std::vector<ui::WidgetSubtarget> retained_targets = fixture.input_.subtargets(
        retained_tabs->identity()
    );
    check(
        controlled_targets.size() == 3U && retained_targets.size() == 3U,
        "choice fixtures did not expose their input rows"
    );
    static_cast<void>(fixture.pointer({
        ui::PointerInputEvent{center(controlled_targets[1].bounds), ui::PointerEventType::press, 4, 0},
        ui::PointerInputEvent{center(controlled_targets[1].bounds), ui::PointerEventType::release, 4, 0},
    }));
    static_cast<void>(fixture.pointer({
        ui::PointerInputEvent{center(retained_targets[2].bounds), ui::PointerEventType::press, 5, 0},
        ui::PointerInputEvent{center(retained_targets[2].bounds), ui::PointerEventType::release, 5, 0},
    }));
    const std::vector<ui::WidgetSubtarget> invalid_targets = fixture.input_.subtargets(
        invalid->identity()
    );
    ...
    static_cast<void>(fixture.pointer({
        ...
        ui::PointerInputEvent{center(invalid_targets[2].bounds), ui::PointerEventType::release, 6, 0},
    }));
    check(
        controlled_tabs->retained_value("$selectedId") == nullptr &&
...
void test_tooltip_disclosure(InputFixture& fixture) {
    ...
    check(
        ui::tooltip_show_delay_nanos(*owner) == 100'000'000 &&
    ...
    check(
        !ui::tooltip_disclosure_visible(*owner) && fixture.input_.requires_frame_advance(),
    ...
    const auto popup = std::ranges::find(
        targets, std::string("$tooltip"), &ui::WidgetSubtarget::id
    );
    check(
        popup != targets.end() && popup->detached &&
...
void test_manipulation_slop(InputFixture& fixture) {
    ...
    const auto behavior = [](std::string id, runtime::Value options) {
        return std::vector<ui::DescriptionBehavior>{ui::DescriptionBehavior{
            std::move(id), true, std::move(options), nullptr,
        }};
    };
    fixture.adopt(node("Panel", "manipulation.root", {
        node(
            "Probe", "manipulation.move", {
                node("Text", "manipulation.move.glyph", {}, sized(40.0, 20.0)),
            }, sized(160.0, 36.0),
            behavior("strata.movable", object({{"bounds", runtime::Value("none")}}))
        ),
        node(
            "Probe", "manipulation.resize", {
        ...
        node("Panel", "manipulation.pane", {
            node(
                "Probe", "manipulation.divider", {}, sized(12.0, 36.0),
                behavior("strata.split-handle", object({
                    {"paneKey", runtime::Value(runtime::KeyValue{"manipulation.pane"})},
                }))
            ),
    ...
    check(
        movable->retained_value("strata.movement.offset") == nullptr && clicks == 1U,
    ...
    check(
        resize->retained_value("strata.gesture.runtimeSize") == nullptr && clicks == 2U,
    ...
    check(
        pane->retained_value("strata.gesture.splitRatio") == nullptr && clicks == 3U,
...
void test_passive_descendant_activation(InputFixture& fixture) {
    const auto activate = [&fixture](std::string message) {
        return std::vector<ui::DescriptionBehavior>{ui::DescriptionBehavior{
            "strata.activate", true, runtime::Value{},
    ...
    static_cast<void>(fixture.pointer({
        ...
        ui::PointerInputEvent{
            ui::Point{label.x + 12.0, label.y}, ui::PointerEventType::move, 32, 0,
        },
        ui::PointerInputEvent{
            ui::Point{label.x + 12.0, label.y}, ui::PointerEventType::release, 32, 0,
        },
    }));
    check(
        fixture.notifications_.size() == notifications_before + 1U,
    ...
    static_cast<void>(fixture.pointer({
        ui::PointerInputEvent{
            ...
            ui::PointerEventType::press, 33, 0,
        },
        ui::PointerInputEvent{
            center(fixture.bounds("boundary.button").bounds),
            ui::PointerEventType::release, 33, 0,
        },
    }));
    check(
        fixture.notifications_.size() == notifications_before + 3U &&
            fixture.input_.focused_key().has_value() &&
    ...
    fixture.adopt(node(
        "Panel", "included.card", {
            ...
            ui::DescriptionBehavior{
                "strata.activate", true,
                object({{"includeDescendants", runtime::Value(true)}}), nullptr,
            },
        }
    ));
    ...
    check(
        std::ranges::count_if(included_result.events, [](const data::JsonValue& event) {
...
void test_banner_semantics(InputFixture& fixture) {
    const auto banner = [&fixture](
                            std::string key,
                            const bool action,
                            const bool dismissible,
                            const bool collapsed = false
                        ) {
        ui::DescriptionNode::Properties properties = collapsed
            ? sized(0.0, 0.0, "SPACER")
            : sized(280.0, 46.0);
        ...
        if (action && !collapsed) {
            properties.emplace(
                "action",
                runtime::ExpressionValue(fixture.notification_action(key + " action"))
            );
    ...
    const auto actions = [&fixture, &semantics](const std::string_view key) {
        const ui::RetainedNode* retained = fixture.tree_.find_key(key);
        const data::JsonValue* value = retained != nullptr
            ? semantics.find(retained->identity())
            : nullptr;
        return value != nullptr ? semantic_actions(*value) : std::vector<std::string>{};
    };
    check(
        actions("banner.none") == std::vector<std::string>{"focus"} &&
            actions("banner.action") == std::vector<std::string>{"activate", "focus"} &&
            actions("banner.dismiss") == std::vector<std::string>{"dismiss", "focus"} &&
            actions("banner.both") ==
                std::vector<std::string>{"activate", "dismiss", "focus"},
        "Banner semantics advertised actions absent from its conditional input targets"
    );
    const std::vector<ui::WidgetSubtarget> targets = fixture.input_.subtargets(
        fixture.tree_.find_key("banner.both")->identity()
    );
    check(
        std::ranges::find(targets, std::string("$action"), &ui::WidgetSubtarget::id) !=
                targets.end() &&
    ...
    const auto focus_banner = [&fixture, &pointer_id](const std::string_view key) {
        ...
        check(
            fixture.input_.focused_key().has_value() &&
                *fixture.input_.focused_key() == key,
    ...
    const auto action_target = std::ranges::find(
        targets, std::string(ui::banner_action_subtarget), &ui::WidgetSubtarget::id
    );
    ...
    check(
        fixture.notifications_.size() == notification_count + 1U,
    ...
    static_cast<void>(fixture.input_.key("space"));
    check(
        dismiss->retained_value("$dismissed") != nullptr &&
    ...
    static_cast<void>(fixture.input_.key("enter"));
    check(
        both->retained_value("$dismissed") != nullptr &&
    ...
    check(
        fixture.notifications_.size() == notification_count + 3U,
    ...
    check(
        !ui::project_banner(*dismiss).active &&
            fixture.bounds("banner.dismiss").bounds.empty() &&
    ...
    check(
        !fixture.input_.focused_key().has_value() && !dismissed_frame.input.events.empty(),
    ...
    for (std::size_t traversal = 0U; traversal < 4U; ++traversal) {
        ...
        check(
            focused.has_value() && *focused != "banner.dismiss" && *focused != "banner.both",
    ...
    check(
        !settled.reconciliation.changed() &&
            settled.layout.measured_nodes == 0U &&
            settled.layout.arranged_nodes == 0U &&
            settled.input.events.empty() &&
            !semantics.update(
                fixture.tree_, fixture.commands_, fixture.input_, "interaction-residual"
            ),
